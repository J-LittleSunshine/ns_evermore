...
class NsRuntimeProtocolError(NsRuntimeError):
    ...
    numeric_code = 201000
    default_message = "Runtime protocol or codec error."


class NsRuntimeAuthError(NsRuntimeError):
    code = "RUNTIME_AUTH_ERROR"
    numeric_code = 202000
    default_message = "Runtime authentication or authorization error."


class NsRuntimeRoutingError(NsRuntimeError):
    code = "RUNTIME_ROUTING_ERROR"
    numeric_code = 203000
    default_message = "Runtime routing, sharding, or load balancing error."


class NsRuntimeConnectionError(NsRuntimeError):
    code = "RUNTIME_CONNECTION_ERROR"
    numeric_code = 204000
    default_message = "Runtime connection, node, or heartbeat error."


class NsRuntimeMessageError(NsRuntimeError):
...
class NsRuntimeMessageStoreError(NsRuntimeError):
    code = "RUNTIME_MESSAGE_STORE_ERROR"
    numeric_code = 207000
    default_message = "Runtime message store error."


class NsRuntimePluginError(NsRuntimeError):
    code = "RUNTIME_PLUGIN_ERROR"
    numeric_code = 208000
    default_message = "Runtime plugin or extension error."


class NsRuntimeAdminError(NsRuntimeError):
    code = "RUNTIME_ADMIN_ERROR"
    numeric_code = 209000
    default_message = "Runtime admin API or admin protocol error."
