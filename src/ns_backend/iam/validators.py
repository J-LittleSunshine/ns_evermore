...
from ns_backend.backend.common import BaseValidator

from .constants import DATA_SCOPE_VALUES
...
class DepartmentValidator(BaseValidator):
    ...
    allowed_fields = ("company_id", "subsidiary_id", "parent_id", "department_code", "department_name", "status")
    enum_fields = {"status": (0, 1)}


class PermissionValidator(BaseValidator):
    required_fields = ("permission_code", "permission_name", "permission_type")
    allowed_fields = ("permission_code", "permission_name", "permission_type", "parent_id", "status")
    enum_fields = {"permission_type": ("MENU", "ACTION", "DATA"), "status": (0, 1)}
...
class RoleValidator(BaseValidator):
    ...
    enum_fields = {"role_scope": ("PERSONAL", "ENTERPRISE"), "status": (0, 1)}


class UserValidator(BaseValidator):
    required_fields = ("username", "password", "user_type")
    allowed_fields = ("username", "password", "email", "phone", "display_name", "user_type", "company_id", "subsidiary_id", "department_id", "is_active", "is_staff", "is_superuser")
    enum_fields = {"user_type": ("PERSONAL", "ENTERPRISE"), "is_active": (0, 1), "is_staff": (0, 1), "is_superuser": (0, 1)}
...
class RolePermissionValidator(BaseValidator):
    ...
    enum_fields = {"data_scope": DATA_SCOPE_VALUES}
...
class UserPermissionValidator(BaseValidator):
    ...
    enum_fields = {"effect": ("ALLOW", "DENY"), "data_scope": DATA_SCOPE_VALUES}
