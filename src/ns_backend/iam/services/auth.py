...
from ns_backend.iam.errors import (
    IamAccountLockedError,
    IamCredentialInvalidError,
    IamPasswordEmptyError,
    IamRefreshTokenEmptyError,
    IamRefreshTokenInvalidOrExpiredError,
    IamRefreshTokenUserMismatchError,
...
from ns_backend.iam.utils import (
...
class RefreshTokenRotationService:
    ...
    def rotate(cls, *, user_id: int, refresh_jti: str, refresh_token_hash: str) -> TokenRotationResult:
        return UserTokenRotationRepository.rotate(
            user_id=user_id,
...
class AuthService:
    ...
    async def clear_login_failure(username: str, user_id: int) -> None:
        try:
            ...
            logger.warning("login failure clear failed", exc_info=True, extra={
                "username": username,
                "user_id": user_id,
    ...
    def get_device_payload(cls, *, data: dict[str, Any], username: str, client_ip: str | None = None, user_agent: str | None = None) -> dict[str, str]:
        ...
        if not fingerprint:
            fingerprint = cls.build_fallback_fingerprint(
                username=username,
                client_ip=client_ip,
                user_agent=user_agent,
            )
    ...
    @staticmethod
    def build_fallback_fingerprint(*, username: str, client_ip: str | None, user_agent: str | None) -> str:
        return "|".join(
            [
                username,
                client_ip or "",
    ...
    @classmethod
    async def login(cls, *, data: dict[str, Any], request: "Request") -> AuthLoginResult:
        username = str(data.get("username", "") or "").strip()
        password_payload = str(data.get("password", "") or "").strip()

        if not username:
            raise IamUsernameEmptyError()
        ...
        if not user:
            ...
            raise IamCredentialInvalidError()
        ...
        now = timezone.now()
        device_payload = cls.get_device_payload(
            data=data,
            username=username,
            client_ip=client_ip,
            user_agent=user_agent,
        ...
        return AuthLoginResult(
            ...
            data={
                ...
                "user": cls.build_current_user_payload(user),
            },
        )
    ...
    async def refresh(cls, *, data: dict[str, Any]) -> dict[str, Any]:
        ...
        if not refresh_token:
            raise IamRefreshTokenEmptyError()
        ...
        result = await sync_to_async(RefreshTokenRotationService.rotate, thread_sensitive=True)(user_id=user_id, refresh_jti=refresh_jti, refresh_token_hash=refresh_token_hash)
    ...
    @classmethod
    async def logout(cls, *, data: dict[str, Any], request: "Request") -> dict[str, Any]:
        ...
        if refresh_token:
            ...
            if payload:
                ...
                if refresh_user_id != user.id:
                    raise IamRefreshTokenUserMismatchError()

                refresh_token_hash = JwtService.hash_token(refresh_token)
                updated_count = await UserTokenRepository.revoke_refresh_token(
                    user_id=user.id,
                    refresh_jti=refresh_jti,
                    refresh_token_hash=refresh_token_hash,
                    revoked_at=now,
