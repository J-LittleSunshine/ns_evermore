...
from ns_backend.iam.schemas import TenantContext, DataScopeResult, AuditEvent, PermissionSpec
...
class IamCrudService:
    ...
    async def list_items(cls, *, page: int | str | None = 1, page_size: int | str | None = 20, tenant_context: TenantContext | None = None) -> dict[str, Any]:
        ...
        return await BaseRepository.list_items(model_class=cls.model_class, fields=cls.list_fields, page=page, page_size=page_size, tenant_filter=tenant_filter)

    @classmethod
    async def detail_item(cls, *, item_id: int | str | None, tenant_context: TenantContext | None = None) -> dict[str, Any]:
        tenant_filter = cls.get_tenant_filter(tenant_context=tenant_context)
        return await BaseRepository.detail_item(model_class=cls.model_class, item_id=item_id, fields=cls.detail_fields, tenant_filter=tenant_filter)
    ...
    async def create_item(cls, *, data: dict[str, Any], operator_id: int | None = None, tenant_context: TenantContext | None = None) -> dict[str, Any]:
        ...
        return await BaseRepository.create_item_with_audit(model_class=cls.model_class, data=validated_data, operator_id=operator_id, tenant_create_values=tenant_create_values)
    ...
    async def update_item(cls, *, item_id: int | str | None, data: dict[str, Any], operator_id: int | None = None, tenant_context: TenantContext | None = None) -> None:
        ...
        tenant_filter = cls.get_tenant_filter(tenant_context=tenant_context)
        await BaseRepository.update_item_with_audit(model_class=cls.model_class, item_id=item_id, data=validated_data, operator_id=operator_id, tenant_filter=tenant_filter)

    @classmethod
    async def delete_item(cls, *, item_id: int | str | None, tenant_context: TenantContext | None = None) -> None:
        tenant_filter = cls.get_tenant_filter(tenant_context=tenant_context)
        await BaseRepository.delete_item_by_id(model_class=cls.model_class, item_id=item_id, tenant_filter=tenant_filter)
...
class UserCrudService(IamCrudService):
    ...
    async def update_item(cls, *, item_id: int | str | None, data: dict[str, Any], operator_id: int | None = None, tenant_context: TenantContext | None = None) -> None:
        tenant_filter = cls.get_tenant_filter(tenant_context=tenant_context)
        item = await BaseRepository.get_required_by_id(
            model_class=cls.model_class,
            item_id=item_id,
        ...
        validated_data = cls.validate_update_data(data)
        await BaseRepository.update_item_with_audit(
            model_class=cls.model_class,
            item_id=item_id,
            data=validated_data,
            operator_id=operator_id,
    ...
    async def reset_password(cls, *, item_id: int | str | None, password: str | None, operator_id: int | None = None, tenant_context: TenantContext | None = None) -> None:
        ...
        now = timezone.now()
        await BaseRepository.update_item(
            instance=item,
            data={
