...
class UserDeviceRepository:
    ...
    async def get_by_user_and_fingerprint(*, user_id: int, fingerprint_hash: str) -> IamUserDevice | None:
        return await IamUserDevice.objects.filter(user_id=user_id, fingerprint_hash=fingerprint_hash).afirst()

    @staticmethod
    async def create_device(data: dict[str, Any]) -> IamUserDevice:
        return await IamUserDevice.objects.acreate(**data)

    @staticmethod
    async def update_device(device: IamUserDevice, data: dict[str, Any]) -> None:
        for field, value in data.items():
            setattr(device, field, value)
        await device.asave(update_fields=list(data.keys()))
