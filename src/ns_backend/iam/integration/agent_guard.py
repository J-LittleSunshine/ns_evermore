...
class AgentToolAuthorizationGuard:
    ...
    def normalize_resource_id(cls, resource_id: Any) -> str:
        return cls.normalize_required_text(
            resource_id,
            "resource_id",
        )
    ...
    def register_tool_action(cls, *, tool_name: str, resource_type: str, action_code: str, permission_code: str | None = None) -> dict[str, str | None]:
        normalized_tool_name = cls.normalize_required_text(
            tool_name,
            "tool_name",
        )
        normalized_resource_type = cls.normalize_required_text(
            resource_type,
            "resource_type",
        ).lower()
        normalized_action_code = cls.normalize_required_text(
            action_code,
            "action_code",
        ).lower()
    ...
    def normalize_tool_mapping(cls, *, tool_name: str, tool_mapping: dict[str, Any]) -> dict[str, str | None]:
        resource_type = cls.normalize_required_text(
            tool_mapping.get("resource_type"),
            "resource_type",
        ).lower()
        action_code = cls.normalize_required_text(
            tool_mapping.get("action_code"),
            "action_code",
        ).lower()
    ...
    async def provision_mapping_if_needed(cls, *, tool_name: str, mapping: dict[str, str | None]) -> None:
        ...
        async with cls._PROVISION_LOCK:
            ...
            await IamSyncService.batch_sync_resources(
                data={
                    "resources": [
                        {
                            "resource_type": resource_type,
            ...
            await IamSyncService.sync_permission(
                data={
    ...
    async def get_tool_mapping(cls, tool_name: str) -> dict[str, str | None]:
        ...
        normalized_mapping = cls.normalize_tool_mapping(
            tool_name=normalized_tool_name,
            tool_mapping=tool_mapping,
        )

        await cls.provision_mapping_if_needed(
            tool_name=normalized_tool_name,
    ...
    async def ensure_tool_allowed(cls, *, user: Any, tool_name: str, resource_id: str, context: dict[str, Any] | None = None, trace_id: str | None = None) -> dict[str, Any]:
        ...
        decision = await AccessDecisionService.check_with_audit(
            user=user,
            data={
