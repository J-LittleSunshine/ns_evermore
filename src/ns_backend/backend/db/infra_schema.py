...
from ns_backend.backend.db.sql import validate_infra_domain, ensure_sql_file_exists

if TYPE_CHECKING:
    pass

_CREATE_TABLE_NAME_PATTERN = re.compile(r"^\s*create\s+table\s+(?:if\s+not\s+exists\s+)?(?P<name>`[^`]+`|\"[^\"]+\"|\[[^]]+]|[a-zA-Z_][\w$.]*)", flags=re.IGNORECASE)
...
def _get_existing_tables(db_alias: str) -> tuple[str, ...]:
    ...
    return tuple(sorted({str(name) for name in table_names if str(name).strip()}))
