...
class GrantRepository:
    ...
    async def create_user_role(data: dict) -> IamUserRole:
        try:
            ...
            raise BusinessError(f"Failed to bind user role: {exc}", NsErrorCode.BIND_USER_ROLE_FAILED)
    ...
    async def create_role_permission(data: dict) -> IamRolePermission:
        try:
            ...
            raise BusinessError(f"Failed to grant role permission: {exc}", NsErrorCode.GRANT_ROLE_PERMISSION_FAILED)
    ...
    async def create_user_permission(data: dict) -> IamUserPermission:
        try:
            ...
            raise BusinessError(f"Failed to grant user permission: {exc}", NsErrorCode.GRANT_USER_PERMISSION_FAILED)
    ...
    async def create_department_permission(data: dict) -> IamDepartmentPermission:
        try:
            ...
            raise BusinessError(f"Failed to grant department permission: {exc}", NsErrorCode.GRANT_DEPARTMENT_PERMISSION_FAILED)
    ...
    async def create_subsidiary_permission(data: dict) -> IamSubsidiaryPermission:
        try:
            ...
            raise BusinessError(f"Failed to grant subsidiary permission: {exc}", NsErrorCode.GRANT_SUBSIDIARY_PERMISSION_FAILED)
