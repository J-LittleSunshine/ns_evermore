...
class SessionService:
	...
	def ensure_session_state_available(session):
		...
		if session.revoked_at:
			raise BusinessError("Session has been revoked", NsErrorCode.SESSION_REVOKED)

		if session.expired_at <= timezone.now():
			raise BusinessError("Session has expired", NsErrorCode.SESSION_EXPIRED)
	...
	async def revoke_user_sessions(cls, user_id: int) -> int:
		if not user_id:
			raise BusinessError("user_id cannot be empty", NsErrorCode.USER_ID_EMPTY)
	...
	async def revoke_device_sessions(cls, device_id: int) -> int:
		if not device_id:
			raise BusinessError("device_id cannot be empty", NsErrorCode.DEVICE_ID_EMPTY)
