...
class PermissionRegistry:
    """Simple permission spec registry for builtin-only synchronization."""
    ...
    def register_many(cls, specs: list[PermissionSpec] | tuple[PermissionSpec, ...]) -> None:
        """Register multiple permission specs."""
        for spec in specs:
            cls.register(spec)

    @classmethod
    def list_specs(cls) -> list[PermissionSpec]:
        """Return registered specs in deterministic order."""
        return sorted(cls._items, key=lambda item: item.code)

    @classmethod
    def clear(cls) -> None:
        """Clear registry state before a new sync run."""
        cls._items = []
