...
class PermissionService:
    ...
    1. Validate user state and permission code.
    2. Orchestrate permission grant repositories.
    3. Apply DENY-over-ALLOW and ancestor inheritance rules.
    """
    ...
    async def get_active_permission_ids_with_ancestors(cls, permission_code: str) -> list[int]:
        """Return active permission id and active ancestor ids from leaf to root."""
        ...
        for _ in range(cls.MAX_ANCESTOR_DEPTH):
            ...
            next_permission = await PermissionRepository.get_active_permission_by_id(current.parent_id)
    ...
    async def has_permission(cls, user: Any, permission_code: str) -> bool:
        """Check whether a user owns an effective permission code."""
    ...
    async def _has_direct_effect(cls, *, subject_type: str, subject_id: int, permission_ids: list[int], effect: str, now) -> bool:
        """Check direct grant effect by subject type."""
        if subject_type == cls.SUBJECT_USER:
            return await PermissionRepository.has_user_effect(user_id=subject_id, permission_ids=permission_ids, effect=effect, now=now)

        if subject_type == cls.SUBJECT_DEPARTMENT:
            return await PermissionRepository.has_department_effect(department_id=subject_id, permission_ids=permission_ids, effect=effect, now=now)

        if subject_type == cls.SUBJECT_SUBSIDIARY:
            return await PermissionRepository.has_subsidiary_effect(subsidiary_id=subject_id, permission_ids=permission_ids, effect=effect, now=now)

        return False

    @classmethod
    async def _has_role_allow(cls, *, user_id: int, permission_ids: list[int], now, role_scope: str, company_id: int | None) -> bool:
        """Check whether user has role-level allow permission."""
        return await PermissionRepository.has_role_allow(user_id=user_id, permission_ids=permission_ids, now=now, role_scope=role_scope, company_id=company_id)

    @classmethod
    async def _has_personal_permission(cls, *, user: Any, permission_ids: list[int], now) -> bool:
        """Check personal-user permission with DENY precedence."""
    ...
    async def _has_enterprise_permission(cls, *, user: Any, permission_ids: list[int], now) -> bool:
        """Check enterprise-user permission with DENY precedence."""
        ...
        has_user_deny = await cls._has_direct_effect(subject_type=cls.SUBJECT_USER, subject_id=user_id, permission_ids=permission_ids, effect=PERMISSION_EFFECT_DENY, now=now)
        has_department_deny = False
        has_subsidiary_deny = False

        if department_id:
            has_department_deny = await cls._has_direct_effect(subject_type=cls.SUBJECT_DEPARTMENT, subject_id=department_id, permission_ids=permission_ids, effect=PERMISSION_EFFECT_DENY, now=now)

        if subsidiary_id:
            has_subsidiary_deny = await cls._has_direct_effect(subject_type=cls.SUBJECT_SUBSIDIARY, subject_id=subsidiary_id, permission_ids=permission_ids, effect=PERMISSION_EFFECT_DENY, now=now)
        ...
        if department_id:
            has_department_allow = await cls._has_direct_effect(subject_type=cls.SUBJECT_DEPARTMENT, subject_id=department_id, permission_ids=permission_ids, effect=PERMISSION_EFFECT_ALLOW, now=now)

        if subsidiary_id:
            has_subsidiary_allow = await cls._has_direct_effect(subject_type=cls.SUBJECT_SUBSIDIARY, subject_id=subsidiary_id, permission_ids=permission_ids, effect=PERMISSION_EFFECT_ALLOW, now=now)
    ...
    async def list_permission_codes(cls, user: Any) -> list[str]:
        """List all effective permission codes for a user."""
    ...
    async def list_menu_tree(cls, user: Any) -> list[dict[str, Any]]:
        """List menu permission tree for a user."""
        active_permissions = await cls._list_active_permissions()
        effective_ids = await cls.resolve_effective_permission_ids(user=user, active_permissions=active_permissions)
    ...
    async def resolve_effective_permission_ids(cls, *, user: Any, active_permissions: list[dict[str, Any]] | None = None) -> set[int]:
        """Resolve effective permission ids using allow/deny grants and ancestor inheritance."""
        if not user or not bool(getattr(user, "is_active", False)):
            return set()
        ...
        if user_type == USER_TYPE_ENTERPRISE:
            ...
            if department_id:
                deny_ids.update(await cls._list_department_permission_ids(department_id, now, effect=PERMISSION_EFFECT_DENY))
                allow_ids.update(await cls._list_department_permission_ids(department_id, now, effect=PERMISSION_EFFECT_ALLOW))

            if subsidiary_id:
                deny_ids.update(await cls._list_subsidiary_permission_ids(subsidiary_id, now, effect=PERMISSION_EFFECT_DENY))
                allow_ids.update(await cls._list_subsidiary_permission_ids(subsidiary_id, now, effect=PERMISSION_EFFECT_ALLOW))
    ...
    async def _list_user_permission_ids(user_id: int, now, *, effect: str) -> set[int]:
        ...
        return await PermissionRepository.list_user_permission_ids(user_id=user_id, now=now, effect=effect)
    ...
    async def _list_department_permission_ids(department_id: int, now, *, effect: str) -> set[int]:
        ...
        return await PermissionRepository.list_department_permission_ids(department_id=department_id, now=now, effect=effect)
    ...
    async def _list_subsidiary_permission_ids(subsidiary_id: int, now, *, effect: str) -> set[int]:
        ...
        return await PermissionRepository.list_subsidiary_permission_ids(subsidiary_id=subsidiary_id, now=now, effect=effect)
    ...
    async def _list_role_permission_ids(*, user_id: int, now, role_scope: str, company_id: int | None) -> set[int]:
        ...
        return await PermissionRepository.list_role_permission_ids(user_id=user_id, now=now, role_scope=role_scope, company_id=company_id)

    @classmethod
    def expand_effective_permission_ids(cls, *, active_permissions: list[dict[str, Any]], allow_ids: set[int], deny_ids: set[int]) -> set[int]:
        """Expand direct grants to child permissions; any deny in chain blocks the permission."""
    ...
    def get_permission_chain_ids(cls, *, permission_id: int, permission_map: dict[int, dict[str, Any]]) -> list[int]:
        """Return current permission id and ancestor ids from a permission map."""
        chain_ids: list[int] = []
    ...
    def build_menu_tree(permissions: list[dict[str, Any]], allowed_ids: set[int]) -> list[dict[str, Any]]:
        """Build visible menu tree and include allowed parent menu chain."""
        menu_permissions = {item["id"]: item for item in permissions if item.get("permission_type") == "MENU"}
        ...
        def build_node(_node_id: int, _path_ids: set[int]) -> dict[str, Any]:
            ...
            child_ids = sorted(children_map.get(_node_id, []), key=lambda _child_id: menu_permissions[_child_id]["permission_code"])
        ...
        remaining_ids = sorted([node_id for node_id in included_ids if node_id not in built_ids], key=lambda item_id: menu_permissions[item_id]["permission_code"])
