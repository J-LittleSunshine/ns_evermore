...
from ns_backend.iam.constants import ROLE_SCOPE_ENTERPRISE, ROLE_SCOPE_PERSONAL
...
class PermissionRepository:
    ...
    async def get_active_permission_by_code(permission_code: str) -> IamPermission | None:
        ...
        return await IamPermission.objects.using(db_alias).filter(status=1, permission_code=permission_code).only("id", "parent_id").afirst()

    @staticmethod
    async def get_active_permission_by_id(permission_id: int) -> IamPermission | None:
        """Load active permission node by primary key."""
        db_alias = BaseRepository.resolve_db_alias(model_class=IamPermission)
        return await IamPermission.objects.using(db_alias).filter(id=permission_id, status=1).only("id", "parent_id").afirst()

    @staticmethod
    async def list_active_permissions() -> list[dict[str, Any]]:
        """List active permissions for effective permission calculation."""
        db_alias = BaseRepository.resolve_db_alias(model_class=IamPermission)
        queryset = IamPermission.objects.using(db_alias).filter(status=1).values(
            "id",
            "permission_code",
            "permission_name",
            "permission_type",
    ...
    async def list_role_permission_ids(cls, *, user_id: int, now, role_scope: str, company_id: int | None) -> set[int]:
        ...
        role_ids = cls._build_user_role_ids_query(user_id=user_id, role_scope=role_scope, company_id=company_id)
        db_alias = BaseRepository.resolve_db_alias(model_class=IamRolePermission)
        queryset = IamRolePermission.objects.using(db_alias).filter(cls.valid_time_filter(now), role_id__in=role_ids, permission__status=1).values_list("permission_id", flat=True)
        return {item async for item in queryset}
