...
class AuthorizeViewSet(IamRequestViewSet):
    ...
    async def check(self, request, *args, **kwargs):
        decision = await AccessDecisionService.check(user=request.current_user, data=request.data, trace_id=self._trace_id(request))
        return self.success_response(decision)

    async def batch_check(self, request, *args, **kwargs):
        result = await AccessDecisionService.batch_check(user=request.current_user, data=request.data, trace_id=self._trace_id(request))
