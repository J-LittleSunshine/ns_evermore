...
class PasswordTransportService:
    ...
    Production default is plain. HTTPS protects transport, while Django password
    hashers protect storage. rsa_oaep is only a compliance transport wrapper.
    """
    ...
    def resolve(cls, password_payload: str) -> str:
        """Return raw password according to configured transport mode."""
        ...
        if mode == cls.MODE_PLAIN:
            ...
            raise BusinessError("password transport mode is invalid", NsErrorCode.PASSWORD_TRANSPORT_CONFIG_INVALID)
    ...
    def validate_payload_basic(cls, password_payload: str) -> None:
        """Validate password payload shape and length without decrypting."""
        cls._validate_payload(password_payload)
    ...
    def _validate_payload(cls, password_payload: str) -> None:
        ...
        if len(password_payload) > max_length:
            raise BusinessError("password payload is invalid", NsErrorCode.PASSWORD_TRANSPORT_INVALID)
    ...
    def _validate_raw_password(cls, raw_password: str) -> None:
        ...
        if len(raw_password) > max_length:
            raise BusinessError("password is invalid", NsErrorCode.PASSWORD_TRANSPORT_INVALID)
    ...
    def decrypt_rsa_oaep(cls, password_payload: str) -> str:
        ...
        try:
            ...
            raise BusinessError("password payload is invalid", NsErrorCode.PASSWORD_TRANSPORT_INVALID) from exc
        ...
        try:
            ...
            raise BusinessError("password decrypt failed", NsErrorCode.PASSWORD_TRANSPORT_DECRYPT_FAILED) from exc
    ...
    def _load_private_key(cls):
        ...
        try:
            ...
            raise BusinessError("password transport private key is invalid", NsErrorCode.PASSWORD_TRANSPORT_CONFIG_INVALID) from exc
    ...
    def _load_private_key_bytes() -> bytes:
        ...
        if key_file:
            try:
                ...
                raise BusinessError("password transport private key file is invalid", NsErrorCode.PASSWORD_TRANSPORT_CONFIG_INVALID) from exc

        raise BusinessError("password transport private key is not configured", NsErrorCode.PASSWORD_TRANSPORT_CONFIG_INVALID)
