...
class SqlWalCacheBackend(BaseCacheBackend):
    ...
    def get(self, key: str, default: object | None = None) -> object | None:
        ...
        with self._lock:
            row = self._connection.execute(
                f"SELECT value, expire_at FROM {self._table} WHERE cache_key = ?", (
                    cache_key,
                )
            ).fetchone()
            if row is None:
            ...
            if expire_at is not None and float(expire_at) <= now:
                self._connection.execute(
                    f"DELETE FROM {self._table} WHERE cache_key = ?", (
                        cache_key,
                    )
                )
            ...
            self._connection.execute(
                f"UPDATE {self._table} SET accessed_at = ? WHERE cache_key = ?", (
                    now,
                    cache_key
                )
            )
    ...
    def add(self, key: str, value: object, timeout: int | None | DefaultCacheTimeout = NS_CACHE_DEFAULT_TIMEOUT) -> bool:
        ...
        with self._lock:
            ...
            exists = self._connection.execute(
                f"SELECT 1 FROM {self._table} WHERE cache_key = ?", (
                    cache_key,
                )
            ).fetchone()
    ...
    def expire(self, key: str, timeout: int) -> bool:
        ...
        with self._lock:
            self._delete_expired_key_locked(cache_key, now)
            cursor = self._connection.execute(
                f"UPDATE {self._table} SET expire_at = ?, updated_at = ? WHERE cache_key = ?", (
                    expire_at,
                    now,
                    cache_key
                )
    ...
    def persist(self, key: str) -> bool:
        ...
        with self._lock:
            self._delete_expired_key_locked(cache_key, now)
            cursor = self._connection.execute(
                f"UPDATE {self._table} SET expire_at = NULL, updated_at = ? WHERE cache_key = ?", (
                    now,
                    cache_key
                )
            )
    ...
    def ttl(self, key: str) -> int:
        ...
        with self._lock:
            row = self._connection.execute(
                f"SELECT expire_at FROM {self._table} WHERE cache_key = ?", (
                    cache_key,
                )
            ).fetchone()
            if row is None:
    ...
    def _delete_expired_key_locked(self, cache_key: str, now: float) -> None:
        ...
        self._connection.execute(
            f"DELETE FROM {self._table} WHERE cache_key = ? AND expire_at IS NOT NULL AND expire_at <= ?", (
                cache_key,
                now
            )
        )
    ...
    def _delete_expired_locked(self, now: float) -> None:
        ...
        self._connection.execute(
            f"DELETE FROM {self._table} WHERE expire_at IS NOT NULL AND expire_at <= ?", (
                now,
            )
        )
...
class RedisCompatibleCacheBackend(BaseCacheBackend):
    ...
    def get_many(self, keys: list[str]) -> dict[str, object]:
        ...
        for (
                key,
                _
        ), payload in zip(entries, payloads):
            if payload is None:
                continue
    ...
    def set_many(self, data: dict[str, object], timeout: int | None | DefaultCacheTimeout = NS_CACHE_DEFAULT_TIMEOUT) -> list[str]:
        ...
        return [key for (
            ...
            _,
            _
        ), result in zip(entries, results) if not bool(result)]
