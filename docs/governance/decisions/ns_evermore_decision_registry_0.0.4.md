...
- **Supersedes:** `0.0.3` as current working-tree registry
...
Current Architecture Constraint authority is defined by the current Constraint Index and Global Architecture State; this Registry provides decision-classification context only.
...
## 3. Historical Z0 DADs

`Z0-DAD-001..010` remain historically accepted through Z0 Global Acceptance. Where their governance implementation details were superseded by Unified Governance/current Global State, current governance wins while historical evidence remains recoverable.
...
Future Architecture / Design / Implementation Planning / IWP / Codex sessions consume current Unified Governance, Global State and current Constraint Index rather than relying on obsolete standalone governance artifacts or prior chat context.

No session may use directory structure, framework placement, library/provider implementation, transport representation, data ingestion, extension origin, commercial state, technology exception, dependency placement, package composition, or code organization to bypass the accepted decision-authority model.
