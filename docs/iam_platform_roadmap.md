# IAM 平台化总览与路线图（Master Roadmap）

- 版本：v2.1
- 日期：2026-06-02
- 适用仓库：`G:\PythonProject\ns_evermore`
- 主要代码域：`src/ns_backend/iam`、`src/ns_backend/backend`、`sql/create/iam`

配套分层文档入口：`docs/iam/README.md`
...
3. 当前项目未正式生产发布，SQL 以 `sql/create/iam/*.sql` 直接扩展为准，暂不维护 `sql/upgrade/iam/<version>/`。
...
- 依赖：无
- 文件：
  - 更新 `docs/iam/l1-foundation/api-and-permission-contract.md`
...
- 文件：
  ...
  - 更新 `docs/iam/l2-capabilities/data-scope-contract.md`
- 表：复用现有授权表
...
- 依赖：P0-04
- 文件：
  - 更新 `docs/iam/l3-integration/acceptance-checklist.md`
  - 更新 `docs/iam/l3-integration/smoke-requests.http`（或同等请求集）
- 表：无
- API：全量抽样
- 具体改法：
  1. 固化 20+ 条核心接口 smoke。
...
- 文件：
  ...
  - 更新 `docs/iam/l3-integration/knowledge-and-agent-integration.md`
- 表：复用 ACL/Policy/审计表
...
- 文件：
  - 更新 `docs/iam/l3-integration/acceptance-checklist.md`
  - 更新 `docs/iam/l3-integration/perf-baseline.md`
- 表：无新增
- API：全链路
- 具体改法：
  1. 建立六大能力验收矩阵。
