# ns_evermore Project Architecture — Candidate Revision 0.0.1
...
Top-level Source-of-Truth / actual-state ownership topology
...
- Project Owner decisions `Z2-MDE-001..016` persisted in `docs/governance/decisions/`.
...
Future runtime/deployment design MAY realize one Product Component through multiple physical/runtime units or co-locate multiple responsibilities where allowed, but physical placement MUST NOT silently rewrite Product Component identity or semantic authority.
...
```text
Native Tenant semantics and canonical Tenant SoT
Native IAM semantics
Unified Policy semantics
Native Organization semantics and Organization-system governance
Business Application Construction / Runtime backend semantics
...
| Principal capability domain | Primary top-level semantic ownership | Major execution/interaction placement |
|---|---|---|
| Business Application Construction / Runtime | `ns_server` native platform application semantics | `ns_server` backend + `ns_web` UI/builder; may compose other domains |
| Automation Construction / Execution | `ns_server` Automation Definition / Workflow semantics | `ns_web` builder; `ns_runtime` scheduling/dispatch; `ns_node` local execution |
| AI Agent Runtime / Tooling | `ns_agent` AI Agent semantic authority | `ns_agent` runtime/tooling; may consume Knowledge and invoke other domains |
| Enterprise Data / Knowledge / Foundational ETL | `ns_server` native Data/Knowledge/ETL semantics | `ns_server` data/knowledge/ETL backend; `ns_web` management/visualization; consumed by other domains |
...
→ managed runtime configuration authority / canonical desired-state SoT = ns_server
...
### Z2-DAD-013 — Governing lifecycle separation
...
### Z2-DAD-014 — Governance dependencies
...
### Z2-DAD-015 — Automation semantic/execution topology
...
### Z2-DAD-016 — Agent semantic/execution topology
...
### Z2-DAD-017 — Data / Knowledge topology
...
### Z2-DAD-018 — Configuration topology

```text
Component-local bootstrap configuration
→ local per Product Component
→ common loader MAY come from Shared Foundation

Managed Runtime Configuration
→ governance / canonical desired-state SoT = ns_server
...
### Z2-DAD-019 — Distributed bounded factual ownership
...
### Z2-DAD-020 — Organization factual authority
...
### Z2-DAD-021 — Data / Knowledge factual authority
...
## 14. Offline / Degraded Correctness

### Z2-DAD-022 — Offline semantics are first-class
...
## 15. Extension / Re-delivery Boundary

### Z2-DAD-023 — Extension does not escalate authority
...
## 16. Responsibility / Authority / SoT Matrix
...
| Native IAM semantics | `ns_server` | `DEFERRED_TO_LATER_AUTHORIZED_DESIGN` for native/external factual federation details | `ns_server` governance; external IdP integration later | Authentication provider != IAM authority |
| Unified Policy semantics | `ns_server` | `DEFERRED_TO_LATER_AUTHORIZED_DESIGN` for persistence/evaluation-state details | `ns_server` governance; enforcement distributed as applicable | Policy permit != Admission |
...
| Business Application platform semantics | `ns_server` | application/platform definition state under `ns_server`; customer business factual SoTs remain independent | `ns_server` backend + `ns_web` builder/UI | Application composition != domain subordination |
| Automation Definition / Workflow semantics | `ns_server` | definition state under applicable `ns_server` Automation authority | `ns_web` build; `ns_runtime` coordinate; `ns_node` execute | Execution != definition authority |
| AI Agent semantics | `ns_agent` | Agent definition state under `ns_agent`; detailed runtime-state partition later | `ns_agent` runtime/tooling; `ns_web` UI | Model/provider != Agent authority |
...
| Formal Artifact Acceptance | `ns_server` | accepted-artifact governance state under `ns_server` | storage/registry implementation later | Semantic certification != acceptance |
| Formal Execution Admission | `ns_server` | admission governance state under `ns_server` | `ns_runtime` consumes outcome for coordination | Admission != dispatch |
| Platform Security / Trust semantics | `ns_server` | trust governance state representation deferred | enforcement/evidence distributed | Crypto valid != platform trusted |
...
## 17. Product Component Responsibility / Authority Summary

| Product Component | Owns at Project Architecture level | Explicitly does not own by implication |
|---|---|---|
| `ns_server` | Tenant, IAM, Policy, Organization native semantics; Business App and Automation platform semantics; Data/Knowledge/ETL semantics; Artifact Acceptance; Admission; Platform Trust; runtime-config desired governance | Agent semantics; local execution facts; all runtime facts; all external enterprise factual SoTs; universal semantic authority |
| `ns_runtime` | communication/runtime coordination responsibility; bounded coordination actual-state | business/domain definitions; Artifact Acceptance; Admission; local protected effects; universal runtime truth |
| `ns_node` | local execution responsibility; bounded local source/effect facts | Task/Workflow definition; Policy; Artifact Acceptance; Admission; universal business/runtime authority |
| `ns_agent` | Agent semantic authority; Agent runtime/tooling; bounded Agent-runtime facts | Knowledge/Data SoT by consumption; invoked Business/Automation authority; Platform Trust/Policy/Admission authority |
| `ns_web` | human-facing UI/builder/management/control-plane interaction responsibility | canonical business/config/runtime state; domain semantic authority by editing/display |

---

## 18. Explicit Deferred Architecture Questions
...
## 19. Constraint Conformance Summary
...
Major SoT / actual-state topology
