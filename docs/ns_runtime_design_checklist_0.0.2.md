...
- production StateStore provider 与物理域 lease 只在 broker 进程打开。物理域 identity 至少绑定 backend、规范 endpoint、database、namespace 与 principal/credential reference，且不得包含可替换以绕过唯一性的 runtime ID；同一物理域的第二 broker 必须跨进程拒绝，崩溃后依靠 OS lock 自动释放。
