...
- production `IamClient` 不得暴露可由普通模块调用的 factory、proof 或自由 signer；production graph 只能在 bootstrap 的完整组装区间内一次性建立。普通 `NsHttpClientOwner` 不得签发 IAM handle；最终 client 必须绑定唯一 narrow HTTP handle，`object.__new__`、copy、subclass、method override、伪造 filename/frame 或 contract-test adapter 均不得满足 production adapter 校验。
- production IAM HTTP provenance 必须绑定 `NsAsyncHttpClient.base_url`、request/httpx base URL、scheme/host/port/path prefix、timeout/default routing/auth headers、TLS context、proxy、主 transport、mount/proxy transport与 handler identity。IAM path 使用 exact allowlist；执行请求直接使用初始化时固定的 transport callable 和绝对 backend URL，使 await 期间临时替换并恢复 client transport/base URL/mount 不能改变实际请求去向。
...
- raw production StateStore 不得保存 repository owner、scope 私钥或 repository creation API；composition 必须一次性安装固定 repository set 后永久关闭创建。production repository 不持有签发 closure/private key，只向 Store 的窄 scope endpoint 提交自身固定 binding；Store 按 scope 实例快照与 canonical policy ID/version 校验。复制、字段替换、属性遍历、name-mangled lookup、伪造 validator 或二次 composition不得恢复或扩展权限。
...
- 对 production IAM decision、payload decision 与 StateStore repository authority，Python 进程内的下划线字段、随机 token、closure、frame/path、frozen dataclass、mutable validator 或调用点约定都不得作为安全边界。production 私钥、IAM HTTP transport、raw production StateStore、scope issuer 与 resource-policy authority 必须位于 bootstrap 以 `multiprocessing` spawn 建立的独立 broker 进程；普通 runtime 进程只能持有 broker 公钥、instance ID、固定 operation/role 的 IPC proxy 和 broker-signed typed result。
- IAM broker IPC 只允许 `introspect`、`runtime_access_check`、`permission_snapshot`、`payload_validate`、`payload_revalidate` 固定操作。每个结果必须以 broker 独占的非对称私钥绑定 broker instance、operation、request fingerprint、backend decision、permission snapshot/version、tenant/target/message type、issued/expiry、nonce/sequence；跨 broker、跨 request/operation、过期或签名错误一律拒绝，主进程不得重新签发 production decision。
- production StateStore provider 与物理域 lease 只在 broker 进程打开。物理域 identity 至少绑定 backend、规范 scheme/host/port、database、namespace 与稳定 ACL principal/username；不得绑定 runtime ID、环境变量名、password 文件路径、调用方自定义 credential reference 或 secret 明文。同一物理域即使改用不同 credential 输入渠道，第二 broker 也必须跨进程拒绝；崩溃后依靠 OS lock 自动释放。
- admission、scheduler、payload、registry、audit repository proxy 必须携带 broker-signed opaque handle，固定 broker instance、lifecycle generation、role 与 runtime/tenant/audit dimension。IPC 服务端同时复验 handle 与 role-operation/resource policy；主进程不取得 `StateAccessScope`、production private issuer、raw Store 或 mutable production validator。contract-test composition只能使用明确的 deterministic/in-memory provider，不得接受 Redis/Valkey endpoint或连接网络。
- broker crash、IPC close/timeout、签名错误或 instance change 必须 fail closed：IAM返回 unavailable/denied，StateStore write返回 indeterminate，禁止本地降级签发、delivery重试或WRITE_UNCERTAIN重发。broker不是第二 RuntimeService、TaskSupervisor、event loop owner或shutdown coordinator；其内部事件循环只服务隔离的authority IPC并由既有bootstrap/shutdown生命周期拥有。
- production broker握手不得信任broker自报公钥。部署trust root公钥必须预置在broker构建中，root私钥只能由最外层launcher通过一次性继承FD提供；broker child必须先验证该私钥对应固定root，再用root签发包含instance ID、session公钥、runtime、generation、时效和nonce的instance certificate。普通Python模块不得通过config、环境明文、公钥替换或自由starter建立新的production trust root。
- authority IPC应用层禁止pickle和`Connection.send/recv`，只能使用`send_bytes/recv_bytes`承载有帧上限的canonical UTF-8 JSON。每个operation必须有exact schema与typed codec，拒绝重复/未知字段、非有限数、超长string/container/frame及非JSON对象；StateTransaction wire必须完整覆盖mutations、record assertions、ordered-index mutations/assertions与log appends，并由broker用内部repository scope重建。
- P10/P11业务服务只依赖admission、scheduler、payload、registry、audit窄持久化Protocol；production实现为fixed-role broker proxy，contract-test实现属于独立realm。业务对象不得接收production raw Store、StateStoreRepository或StateAccessScope；ordered-index cursor wire必须绑定member、score与exact index name/bucket。
- production channel身份必须永久绑定由预置root签发且仍有效的instance certificate；certificate必须绑定realm、instance、session key、runtime、lifecycle generation、issued/expiry与nonce。production、contract-test、integration-test必须使用互不接受的exact channel、IAM proxy、repository proxy和StateStore proxy类型；修改realm字符串、class attribute、certificate、session key、instance、handle或用`object.__new__`补字段不得跨realm建立production authority。
- broker的IAM和StateStore每个response（成功、typed error、read、transaction、index、append和health）都必须由certificate中的session key签名，并绑定instance/generation、request ID与sequence、operation、handle ID/role、canonical request fingerprint、完整result/error、response sequence、issued_at与nonce。runtime必须先验证root certificate chain和response signature，再解码业务DTO；unsigned、字段修改、跨operation/handle/broker/request replay及重复、跳号、倒退sequence一律fail closed，已发送write统一indeterminate。
- delivery persistence transaction fingerprint必须使用canonical结构而非`repr()`，覆盖partition、mutation顺序与kind、key、全部assertion、document metadata及payload SHA-256、ordered-index mutation/assertion和log append。transaction result必须逐mutation验证record顺序、key/document/delete语义，并验证log position为正整数且数量精确；同shape但不同payload/assertion/index/log、copy/replace/字段修改与cross-request replay不得复用。
- IPC异常后的channel关闭状态与child已reap状态必须分离；timeout、EOF、decode/signature failure或OS error后统一close connection并执行bounded join、terminate、join、kill、join，`close()`即使channel已拒绝新请求也必须幂等确认child退出，使physical-domain lease可立即重新获取。
- StateStore实际credential由受信launcher解析后以一次性FD传递实际password bytes或broker-only opaque credential；broker消费并尽力清零临时buffer，主RuntimeContext、sanitized config、普通环境与业务对象不得保留可解析source或secret。physical-domain唯一性只使用稳定principal身份，不得以credential reference改名绕过。
- production root certificate chain的最终验证必须位于不导入processor、delivery、routing或plugin业务模块的独立OS进程attestor中；主runtime解释器里的root global、realm/type helper、certificate对象、channel字段或对象identity只能是待验证数据，不得成为最终production trust root。
- 每次broker request必须在单一序列化临界区创建attestor-approved immutable authority snapshot，绑定connection generation、broker/session/certificate、handle/role及request/response sequence和fingerprint。response必须由attestor对该snapshot验签；请求期间替换connection、certificate、public key、instance/runtime或generation必须fail closed。
- deployment root只用于签发长期有界delegation；broker session certificate必须在到期前以delegated key和attestor-approved rotation ticket轮换。rotation必须原子替换session generation与全部role handles，旧handle/result/response立即失效，现有业务proxy不得因固定300秒certificate而永久unavailable。
- IPC `send_bytes()`调用开始前必须标记send-attempted；调用开始后的任何OS/transport异常都必须reap broker child并废弃channel sequence。已开始发送的StateStore write一律indeterminate，不能降格为确定未写、不能重试delivery。
- P11 authority边界的强制威胁模型是：普通runtime业务模块即使直接使用自己持有的role IPC endpoint并手工构造合法wire，也不能让broker执行超出该endpoint固定IAM/StateStore角色的operation或resource policy；production私钥、raw Store、IAM transport、resource policy和最终role判定必须留在隔离broker，跨role、跨endpoint、跨broker与跨generation调用由broker拒绝。
- P11不宣称防御主runtime进程已经遭到任意代码执行的完全攻陷；攻击者在该解释器内任意替换业务方法、调用栈或本地Python返回值可以伪造本地行为，但不能据此取得其他role的OS endpoint或让隔离broker执行越权操作。proxy exact type、`_ROLE`、`_repository_proxy_binding`及RuntimeDependencySlots类型检查只能防止误配，不能作为broker authority边界或完成证据。
- root签发的delegation必须同时绑定broker instance/runtime、delegation public key、attestor instance/public key、允许用途、时效和nonce。broker只验证该delegation绑定attestor签发的request/rotation ticket；attestor只批准明确绑定自身instance/public key的delegation。父进程传入任意32字节key、攻击者自建exact attestor client或格式正确的ticket均不能建立production批准。
- production bootstrap必须为`iam`、`admission`、`scheduler`、`payload`、`registry`、`audit`和`lifecycle`分别创建独立OS IPC endpoint。broker按接收connection固定role并执行operation/resource allowlist；request wire不得接受caller-provided role或handle。每个稳定proxy只持有自身endpoint，production主对象不得保留全部role handle/channel表、`current_handle(role)`或generic `request(handle=..., operation=...)`入口。
- 多role并发请求不得使旧generation response与新generation rotation交叉验签。由于当前broker本身是单执行队列，主进程process custodian可用不携带role/handle/authority的调度锁串行化各独立endpoint的send、rotation与attestor response verification；该锁不得持有endpoint集合，也不得重新提供generic channel。
- attestor死亡、approved identity失效、rotation或response attestation失败必须停止接受请求并立即close/bounded join/terminate/kill/reap broker、关闭attestor，使physical-domain lease可重新取得。StateStore write只有在`send_bytes()`已尝试后才映射为indeterminate；preflight拒绝仍为unavailable/capability error。broker公开session identity必须动态反映attestor批准的当前generation、session key及fingerprint，不能保存启动快照。
