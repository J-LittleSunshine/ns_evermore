...
## 2026-07-30 authority/attestor 稳定性与 service cleanup lease（当前记录）

### P01—P11 剩余全仓不稳定与 cleanup owner 审查关闭

- 起始事实：分支`codex/ns-runtime-implementation`；基准及起始HEAD均为`58929b3d674ef4318d9fd0094d71bc267ab7767b`；起始`git status --short --branch`仅显示`## codex/ns-runtime-implementation...origin/codex/ns-runtime-implementation`，工作树clean。本轮未切换/创建分支，未commit、push或创建PR。
- authority竞态：删除`session_ttl_seconds=0.6`、`asyncio.sleep(0.42)`及精确generation墙钟假设。rotation以当前certificate的`issued_at`/`expires_at`计算最后三分之一barrier，并以实际generation增长驱动3次切换；每次切换后20个并发缓存授权必须全部绑定当前generation且不增加backend调用。event-loop用例由`threading.Event`标记worker进入点，有限ticker在该barrier后推进3个loop turn，再显式释放worker；barrier到达与operation提前完成作为两个future竞争，不再用无界`asyncio.sleep(0)`轮询或固定`time.sleep(0.25)`。
- timeout与诊断：contract/integration starter新增显式test-only broker operation timeout，非timeout测试使用30秒宽裕值；production仍禁止该override，默认`max(iam_timeout, state_timeout) + 2`及所有fail-closed语义不变。只有非production test realm启用诊断；reap前记录broker/attestor alive、exitcode、operation、role、request stage、rotation generation、timeout stage及固定allowlist failure code，每次RPC清除旧诊断。禁止并未记录credential、payload、FD、handle、certificate、endpoint对象或异常原文。
- 压力中间失败与归因：三方法首轮压力在外层第4轮无界等待；thread dump证明executor worker已经空闲、broker/attestor已退出，而测试loop仍在零延迟ticker中自调度。有限ticker后的复核又在第9轮暴露worker-entry轮询同类问题；改成barrier/operation双future后不再hang。随后event-loop独立压力的第18个进程捕获首个真实错误：`iteration=6 / iam / attestor_verify_state_response`，reap前broker与attestor均alive、无timeout，排除certificate rotation边界、broker/attestor预先退出、cleanup提前发生及broker IPC timeout；其后broker`exitcode=0`是fail-closed端点关闭后的有界reap结果。增加清零旧状态及固定failure-code诊断后，独立30轮与联合20轮均无再现；没有以该单项复跑替代全仓验收。
- 最终authority压力：上述3个测试方法以完全相同组合从第1轮重新计数并连续执行20轮，全部`Ran 3 tests ... OK`，共60个test-method executions。内部累计至少60次实际generation切换、400次整消息授权及1600次四角色barrier operation；每次operation同时断言ticker推进、operation按契约完成、broker/attestor仍alive，每轮末显式`broker.close()`并断言broker与attestor process均不alive且有exitcode、custodian/attestor均reaped、全部7条channel connection关闭、无endpoint close resource或Pipe残留。
- service lifecycle owner：StateStore成功open后、`service.start()`尝试前，composition把transport、task supervisor、logger及共享broker-backed StateStore/repository graph一次性移交给service。`_stop_service_until_cleanup_complete()`连续无进展时抛出稳定cleanup-incomplete错误，显式持有绑定原event loop的close-only lease及`cleanup_pending`、`service_stopped`、owner-active安全事实，并以cause保留原operation failure。只有`cleanup_pending=False`且service为`STOPPED`时才释放`resources.service_lifecycle_owner`；同一loop解除失败条件后lease只继续未完成phase。outer同步cleanup不在新`asyncio.run()`中重试service.stop，也不形成第二owner或重复关闭service已经接管的资源；process-level`BaseException`优先级保持。
- 保持边界：detached/inherited FD每个整数仍只允许一次close attempt；StateStore成功后才closed，main/bootstrap/composition cleanup及inline seal仍有界。production certificate、attestation、IAM receipt、StateStore response和WRITE_UNCERTAIN语义未放宽；没有retry、吞错或无限重试。P12继续`BLOCKED / NOT_STARTED / F0`，未实现ACK/NACK/Defer、retry、DLQ、cluster ownership或production `task.dispatch`。
- 定向authority：`PYTHONPATH=src python3 -m unittest -q tests.test_runtime_authority_bootstrap tests.test_runtime_authority_broker`最终为`Ran 67 tests in 100.602s, OK`。
- 定向main/service/shutdown/transport：`PYTHONPATH=src python3 -m unittest -q tests.test_runtime_main tests.test_runtime_service tests.test_runtime_shutdown tests.test_runtime_transport_backpressure tests.test_runtime_transport_conformance tests.test_runtime_transport_contracts tests.test_runtime_transport_errors tests.test_runtime_transport_identity tests.test_runtime_transport_lifecycle tests.test_runtime_transport_metrics tests.test_runtime_transport_registry tests.test_runtime_transport_websocket_tcp`为`Ran 126 tests in 12.183s, OK (skipped=6)`。
- 定向StateStore与真实Redis：`PYTHONPATH=src python3 -m unittest -q tests.test_state_store tests.test_runtime_state_store tests.test_redis_state_store_provider`为`Ran 45 tests in 0.846s, OK`；最终强制真实Redis命令`NS_RUNTIME_REQUIRE_REDIS_INTEGRATION=1 PYTHONPATH=src python3 -m unittest -q tests.test_redis_state_store_integration`为`Ran 23 tests in 79.923s, OK`，无skip。全仓中曾有一轮在`test_timeout_and_unavailable_are_typed`的正常`store.open()`阶段因30ms测试deadline出现1个timeout error；测试改为以1秒完成健康open barrier后才把operation deadline收紧到30ms并注入`CLIENT PAUSE`，保留read timeout与indeterminate write语义，完整真实Redis模块随后通过。
- 定向IAM/routing/delivery：`PYTHONPATH=src python3 -m unittest -q tests.test_backend_runtime_iam tests.test_runtime_iam_authorization tests.test_runtime_iam_client tests.test_runtime_iam_credential_recovery tests.test_runtime_routing tests.test_runtime_routing_contracts tests.test_runtime_delivery_admission tests.test_runtime_delivery_scheduling`为`Ran 132 tests in 77.569s, OK`。
- docs与静态门禁：文档落盘前`PYTHONPATH=src python3 -m unittest -q tests.test_runtime_documentation_security tests.test_requirements tests.test_runtime_bootstrap tests.test_runtime_processor_boundaries`为`Ran 23 tests in 3.794s, OK`；最终文档落盘后为`Ran 23 tests in 4.152s, OK`。`PYTHONPATH=src python3 -m compileall -q src tests`及`git diff --check`均通过，Git只输出仓库行尾转换提示。
- 全仓失败及重新计数：完全相同命令`env -u PYTHONASYNCIODEBUG NS_RUNTIME_REQUIRE_REDIS_INTEGRATION=1 PYTHONPATH=src python3 -m unittest discover -s tests -t . -p 'test_*.py' -q`在Redis测试最终修复前先为`Ran 985 tests in 287.085s, OK (skipped=7)`，下一轮为`Ran 985 tests in 290.504s, FAILED (errors=1, skipped=7)`，唯一错误即上述健康open与30ms deadline竞争；因此该序列全部作废，没有以单项复跑计为后续轮次。
- 最终连续三轮全仓：Redis测试修复后从第1轮重新计数，以上完全相同命令依次为`Ran 985 tests in 315.252s, OK (skipped=7)`、`Ran 985 tests in 317.633s, OK (skipped=7)`、`Ran 985 tests in 311.907s, OK (skipped=7)`。三轮均设置`NS_RUNTIME_REQUIRE_REDIS_INTEGRATION=1`并实际执行真实Redis，三轮均0 failed。7项skip中6项需要仓库外deployment production authority private key，1项为当前Linux不适用的Windows event-loop policy。
- 修改文件：`src/ns_runtime/authority_attestor.py`、`src/ns_runtime/authority_broker.py`、`src/ns_runtime/main.py`；`tests/test_redis_state_store_integration.py`、`tests/test_runtime_authority_broker.py`、`tests/test_runtime_iam_client.py`、`tests/test_runtime_main.py`；ADR、实施计划及本验收日志，共10个代码、测试及相关文档文件。
- 未验证：当前commit`58929b3d674ef4318d9fd0094d71bc267ab7767b`及本次未提交候选没有可归属的远端绿色CI，保持`UNVERIFIED`。没有deployment production authority private key，production正向main/真实IAM/transport仍未验证；Valkey server、Redis Sentinel/Cluster、replica/failover、真实Windows及uvloop全树未验证。旧Redis凭据继续记录为“仓库值已移除，服务端轮换待人工确认”。
- 结束事实：HEAD仍为`58929b3d674ef4318d9fd0094d71bc267ab7767b`；最终`git status --short --branch`保持原分支跟踪行并仅列出上述10个modified文件。未切换/创建分支，未commit、push或创建PR。
