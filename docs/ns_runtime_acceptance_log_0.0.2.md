...
- 工作包：`P02-FIX-03 修复真实 main() 启动路径绕过 RSP-1 的配置错误归一化与目录副作用`。
- 状态：`VERIFIED`。
- 完成时间：`2026-07-20T15:50:43+08:00`。
- 原缺陷：真实 `main()` 在进入 `RuntimeStartupPreflight.prepare()` 前调用无显式路径的 `NsConfig.load()`。该调用既会通过 CFG-1 的既有兼容语义提前执行 `ensure_runtime_dirs()`，又会按目标环境先抛出 startup security 类 `NsConfigError`，导致生产明文、关闭强制 TLS、非生产禁止明文和生产 SQLite 无法进入 RSP-1 的稳定 `RUNTIME_STARTUP_SECURITY_ERROR` 边界。
- 修改文件：更新 `src/ns_runtime/startup.py`、`src/ns_runtime/main.py`、`tests/test_runtime_main.py`、实施计划、acceptance log 和 [ADR-023](ns_runtime_architecture_decisions_0.0.2.md#adr-023)。没有修改 `src/ns_common/config/`、`src/ns_common/paths.py`、`CFG-1` facade/默认值/校验/错误合同、配置示例、五份 requirements、RuntimeContext、RuntimeService、backend 或设计边界文档；没有开始 P02-W05/W06。
- `RSP-1` 校准：新增 `RuntimeStartupPreflight.load_config_snapshot()`，只接受显式配置文件路径；`main()` 先解析环境，再用 `get_default_config_path(environment)` 取得路径并以 `config_path=` 加载，因此不进入 `NsConfig.load(config_path=None)` 的目录准备分支。配置加载和 preflight 的 `context.config.validate()` 二次防御共同调用同一 `_raise_normalized_startup_config_error()`；四类安全配置错误使用固定 field/reason 映射，普通配置错误原对象、code 与 details 保持不变。`main()` 的 keyword-only environment/config path/directories/preflight 接线只用于显式 composition root 与隔离测试，无参模块入口行为保持不变。
- 真实 main 负向测试：所有用例均使用显式临时 JSON 配置和临时 `RuntimeStartupDirectories`。prod 明文与 prod SQLite 均返回 `RUNTIME_STARTUP_SECURITY_ERROR`；关闭 `require_tls_in_prod` 和非生产禁止明文也返回对应稳定 reason；非法 listen port 保持 `NS_CONFIG_ERROR`。缺失 `websockets` 返回 `NS_DEPENDENCY_ERROR`，TLS probe 失败返回 startup security error，未来 transport 返回 `RUNTIME_TRANSPORT_DISABLED`；上述失败全部确认临时 runtime root 不存在、event-loop policy setter 未调用、RuntimeService 未构造。默认配置路径分支另以 probe 断言未调用 `ensure_runtime_dirs()`。
- 测试结果：runtime 环境 `tests.test_runtime_startup tests.test_runtime_main` 为 `Ran 25, OK`（startup 14、main 11）；W01 main、W02/FIX-01 service、W03/FIX-02 context 与 W04/FIX-03 startup 联合为 `Ran 58, OK`。P01/runtime + P02 非 Django cache 联合回归为 `Ran 314, OK (skipped=1)`；backend 环境根目录全量为 `Ran 325, OK (skipped=1)`。唯一跳过为 WSL 下 Windows 专用 event-loop policy 用例。
- 静态与隔离检查：全树 `compileall`、runtime/backend 两套 `pip check` 和 `git diff --check` 通过。新增代码没有 listener/socket/server、transport session、StateStore/Redis/Valkey client、HTTP client/exporter、signal、后台 task/thread 或角色状态；失败测试不操作仓库真实 data/etc/log/tmp。
