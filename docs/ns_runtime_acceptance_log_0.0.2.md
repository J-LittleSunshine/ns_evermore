...
- 未验证：起始commit`5b7bd1d760e7ea0b5df863ee04d4ee93eaf0f426`及本次提交候选没有可归属的远端绿色CI，保持`UNVERIFIED`。没有deployment production authority private key，production正向main/真实IAM/transport仍未验证；Valkey server、Redis Sentinel/Cluster、replica/failover、真实Windows及uvloop全树未验证。旧Redis凭据继续记录为“仓库值已移除，服务端轮换待人工确认”。
