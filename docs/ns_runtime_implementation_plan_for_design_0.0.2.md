...
> 当前状态校准时间：`2026-07-22T00:00:22+08:00`
...
| 字段 | 当前值 |
|---|---|
| 当前阶段 | `P07 Processor、插件、事件与审计` |
| 当前工作包 | `P07-W01` |
| 当前工作包状态 | `NOT_STARTED` |
| 当前阶段状态 | `NOT_STARTED / F0` |
| 最近已验证阶段 | `P06 IAM、安全上下文与 backend 合同` |
| 最近已验证工作包 | `P06-R08 backend 恢复后的六项安全重验` |
| 下一阶段 | `P07 Processor、插件、事件与审计`（`P07-W01`，`NOT_STARTED`） |
...
| `src/ns_runtime` 实际能力 | `VERIFIED` | P02-P05 生命周期、协议、transport 与 logical connection/session 保持冻结；P06 新增显式 production `IamClient`、最小 PermissionSnapshot/auth_context、strict/cache 鉴权、tenant 前置硬校验、TTL/version/invalidation、本地 AES-GCM 节点凭证缓存、backend unavailable 降级矩阵和恢复六项重验。P07 generic processor/durable audit、P08 StateStore、DeliveryRecord、路由/投递和 cluster/management 执行链路仍未开始。 |
...
| 当前本地测试基线 | `VERIFIED` | P06 专项 `Ran 20, OK`；P05/P06/config/dependency/main 联合 `Ran 114, OK`；根目录全量 `Ran 676, OK (skipped=1)`。全树 `compileall`、`git diff --check` 与 P06 全局 client、禁用 P07/P08 类型、敏感凭证边界扫描通过。当前解释器 `pip check` 仅报告工作区外系统包 `pygobject` 缺少 `pycairo`，不属于项目依赖清单且未影响上述测试。 |
| 当前状态校准时间 | `VERIFIED` | `2026-07-22T00:00:22+08:00` |
...
| Session、握手与恢复 | `src/ns_runtime/connection/`、`src/ns_runtime/iam/` | SC-1 logical lifecycle 保持冻结；production composition 已显式注入 `IamClient` 和 P01 HTTP owner，handshake/reauth/resume 统一 fail-closed。IAM-R1 增加最小 PermissionSnapshot、strict/cache 鉴权、tenant 前置硬校验、TTL/version/invalidation、AES-GCM 节点凭证缓存、backend unavailable 矩阵和恢复六项重验，不包含 P07 processor pipeline 或 P08 lease/fencing 实现 | P05 connection 联合回归 114/114；P06 runtime 专项 15/15；根目录全量 676/676 | `F3` | P05 保持 `VERIFIED`；P06 `VERIFIED` |
| Processor 与插件 | `src/ns_runtime/connection/processors.py` 仅 P05 lightweight 实现 | heartbeat/drain/reauth 通过 canonical `processor_key` 的不可变 connection lifecycle registry；无 P07 generic pipeline、plugin/event bus、permission/rate-limit/routing/final audit 或 P08 durable storage | processor/schema/feature conformance 与 transport bypass 回归 | P05 lightweight `F3`；generic `F0` | P05 `VERIFIED`；P07 `NOT_STARTED` |
...
|---|---|
| 当前阻塞项 | 无 |
| 下一工作包 | `P07` 保持 `NOT_STARTED`；不得在新的显式指令前实现 generic processor pipeline |
...
- P03 当时冻结 `ENV-1` 时，除标准 runtime.error 外所有 message family 确实保持 feature disabled；该历史 acceptance 事实不变。P05-W01至W14实现了 connection lifecycle 领域能力，`P05-FIX-01` 已正式激活对应 9 项 canonical connection contracts、收回 exact wire schema、建立 P05 lightweight processor 与 logical composition。P06+ 的 task/delivery/stream/control/cluster/config/dead-letter/replay/cancel/hold/status 继续 fail-closed；P07/P08 前仍无通用 pipeline/强审计，P08/P10/P12/P17 前无 StateStore、DeliveryRecord、ACK 状态提交或集群权威，见 [ADR-028](ns_runtime_architecture_decisions_0.0.2.md#adr-028) 与 [ADR-030](ns_runtime_architecture_decisions_0.0.2.md#adr-030)。
