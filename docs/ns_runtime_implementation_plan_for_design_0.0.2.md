...
| 当前工作包 | `P01-W14 安全化 HTTP 错误响应与 response sanitizer` |
| 当前工作包状态 | `NOT_STARTED` |
| 当前阶段状态 | `IN_PROGRESS` |
| 最近已验证阶段 | `P00 本地仓库基线与实施账本`（2026-07-16） |
| 最近已验证工作包 | `P01-W13 重构 HTTP client 创建方式与 owner 生命周期` |
...
| HTTP 基础设施 | `src/ns_common/http_client.py` | `NsHttpClientFactory` 每次创建独立 caller-owned client；`NsHttpClientOwner` 提供 open/closing/closed、反向关闭、并发幂等、取消/失败后保留所有权与可重试回收；旧全局 getter/close 只作兼容边界；当前 HTTP 错误仍保留原始 body preview，待 W14 安全化 | WSL `tests.test_http_client` 7/7；P01/runtime `Ran 183, OK (skipped=1)`；backend 根回归 `Ran 194, OK (skipped=1)` | lifecycle `F2`，response security `F1` | P01-W13 `VERIFIED`，P01-W14 `NOT_STARTED` |
...
| `HTTP-1` | `NsHttpClientFactory` 独立 caller-owned 创建、`NsHttpClientOwner` open/closing/closed 状态、同 loop 并发幂等反向关闭、取消/失败所有权保留与 legacy 全局 getter/close 隔离 | P01 | `VERIFIED` |
...
|---|---|
| 当前阻塞项 | 无 |
| 下一工作包 | `P01-W14 安全化 HTTP 错误响应与 response sanitizer`（`NOT_STARTED`） |
...
- P01-W13 已完成显式 HTTP factory/owner 生命周期并保留 legacy getter 兼容边界，见 [ADR-016](ns_runtime_architecture_decisions_0.0.2.md#adr-016)。P01-W14 尚未安全化 body preview、response URL 和可注入 response sanitizer；P01-W15 至 P01-W17 也尚未完成。`src/ns_runtime`、强一致 StateStore、Envelope、transport、session、delivery、cluster 和 management 仍按后续阶段保持禁用或 F0。
