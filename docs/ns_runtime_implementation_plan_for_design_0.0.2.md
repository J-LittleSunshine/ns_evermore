...
> 当前状态校准时间：`2026-07-17T14:55:21+08:00`
...
| 最近已验证工作包 | `P01-W15 建立 MetricsSink、TraceSink、DiagnosticSnapshotSink 接口和内存测试实现` |
...
| `src/ns_common` 实际能力 | `VERIFIED` | 存在配置、路径、异步生命周期、UTC/单调/可控时钟、统一标识符、固定/指数/jitter 退避、不可变 retry 预算、统一 sanitizer、日志、HTTP、普通 cache、完整 `RUNTIME_*` 错误覆盖矩阵以及 metrics/trace/diagnostic sink 契约和内存实现；缺少 P01 要求的 testing 公共能力 |
...
| 当前本地测试基线 | `VERIFIED` | 最近一次本地验收：observability 专项 13/13；P01/runtime 联合 `Ran 203, OK (skipped=1)`；backend 环境根目录全量 `Ran 214, OK (skipped=1)`，跳过项均为 WSL 下同一 Windows 专用用例；`compileall`、runtime/backend 两套环境 `pip check`、common/observability facade 唯一导出、冷启动导入、exporter/runtime 依赖扫描、生产源码测试文件与仓库虚拟环境扫描和 `git diff --check` 通过；历史命令见 acceptance log |
| 当前状态校准时间 | `VERIFIED` | `2026-07-17T14:55:21+08:00` |
...
| 可观测与故障测试 | `src/ns_common/logger.py`、`src/ns_common/observability.py` | 已有日志、不可变脱敏 metrics/trace/diagnostic 记录、显式 sink 协议、有界线程安全内存实现和 35 个标准指标名预留；尚无 runtime 采集器、外部 exporter 或故障注入框架 | `tests/test_observability.py` 13/13；P01/runtime 联合 203 项；backend 根回归 214 项 | 公共观测基础 `F2`；runtime 观测 `F0` | `IN_PROGRESS`，P01-W15 `VERIFIED`，完整 runtime 观测与故障注入待 P02/P20 |
...
| `COM-009` | Metrics、trace 和 diagnostic sink 接口 | `ns_common.observability` | `VERIFIED` | P01 |
...
| `P01-W15` | `VERIFIED` | 建立 MetricsSink、TraceSink、DiagnosticSnapshotSink 接口和内存测试实现；接口不得强制 HTTP exporter | runtime 后续模块可显式注入 sink |
...
| `OBS-1` | UTC、不可变、脱敏且严格 JSON-safe 的 metric/trace/diagnostic 记录；同步本地 `record()` 与异步 `flush()/aclose()` sink 协议；35 个标准指标名、高基数 metric 标签禁区和有界线程安全内存实现 | P01 | `VERIFIED` |
...
- P01-W15 已冻结不绑定 exporter 的 metrics/trace/diagnostic sink、脱敏不可变记录、标准指标名、高基数标签禁区和有界内存实现，见 [ADR-018](ns_runtime_architecture_decisions_0.0.2.md#adr-018)。当前只完成公共观测基础，不代表 runtime 指标采集、HTTP/OTLP exporter 或 P20 故障注入已经可用；P01-W16/P01-W17 尚未完成。`src/ns_runtime`、强一致 StateStore、Envelope、transport、session、delivery、cluster 和 management 仍按后续阶段保持禁用或 F0。
