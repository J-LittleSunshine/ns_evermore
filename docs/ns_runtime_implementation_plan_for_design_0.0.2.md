...
> 当前状态校准时间：`2026-07-17T22:08:57+08:00`
...
| 当前工作包 | `P02-W02 建立 RuntimeService 生命周期状态机` |
| 当前工作包状态 | `NOT_STARTED` |
| 当前阶段状态 | `IN_PROGRESS` |
| 最近已验证阶段 | `P01 ns_common 公共基础设施加固` |
| 最近已验证工作包 | `P02-W01 建立 src/ns_runtime 独立组件和唯一进程入口` |
...
| `src/ns_runtime` 实际能力 | `VERIFIED` | 已建立独立 package 与无导入副作用的唯一 `main.py` 进程入口；可通过 `python -m ns_runtime.main` 启动并以状态 0 退出；RuntimeService、协议、transport、session、processor、路由、delivery、stream、cluster、management 与 observability 仍为 F0 |
...
| 当前本地测试基线 | `VERIFIED` | 最近一次本地验收：P02-W01 专项 3/3；P01/runtime 联合加入口回归 `Ran 259, OK (skipped=1)`；backend 环境根目录全量 `Ran 270, OK (skipped=1)`，跳过项均为 WSL 下同一 Windows 专用用例；五份依赖清单内容未改变，既有四类仓库外全新环境安装证据保持有效，runtime/backend 持久环境 `pip check` 和真实 standalone Redis namespace 回归通过；`compileall`、模块冷启动/外部工作目录启动、生产源码测试文件、仓库虚拟环境和 `git diff --check` 门禁通过，命令见 acceptance log |
| 当前状态校准时间 | `VERIFIED` | `2026-07-17T22:08:57+08:00` |
...
| runtime 进程与事件循环 | `src/ns_runtime/__init__.py`、`src/ns_runtime/main.py` | 独立 package 与唯一模块进程入口已建立，package 导入不安装 event loop policy 或启动服务，模块入口可确定性退出；RuntimeService、loop 集成和运行上下文尚未实现 | `tests.test_runtime_main` 3/3；P01/runtime 联合加入口回归 259 项；backend 根回归 270 项 | 进程入口 `F1`；service、loop 集成与上下文 `F0` | P02-W01 `VERIFIED`；P02 `IN_PROGRESS` |
...
|---|---|
| 当前阻塞项 | 无 |
| 下一工作包 | `P02-W02 建立 RuntimeService 生命周期状态机`（`NOT_STARTED`） |
...
- P01-W16/P01-FIX-09 已完成 `ns_common.testing` 校准：Factory 的 OPEN/CLOSING/CLOSED 状态机、Factory close 屏障和可重试资源所有权均已验证；reservation 只在真实 socket close 成功后转移自身所有权，直接 release 与 Factory close 共享 reservation 锁。Factory 不拥有 Redis client 或服务进程。P01-W17 的五层依赖清单保持 `VERIFIED`；Redis/Valkey 驱动仍只属于测试层，pyperf/psutil 只属于压测层，QUIC/WebTransport 依赖继续留给 P21。P02-W01 已建立 `src/ns_runtime` 与唯一模块入口；RuntimeService、Envelope、transport、session、delivery、cluster 和 management 仍按后续工作包保持禁用或 F0。
