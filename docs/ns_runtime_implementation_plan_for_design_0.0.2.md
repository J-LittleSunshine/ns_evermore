...
| 字段 | 当前值 |
|---|---|
| 当前阶段 | `P08 强一致 State Store 与原子原语` |
| 当前工作包 | `P08-W01` |
| 当前工作包状态 | `NOT_STARTED` |
| 当前阶段状态 | `NOT_STARTED / F0` |
| 最近已验证阶段 | `P07 Processor 流水线、插件、事件与审计` |
| 最近已验证工作包 | `P07-W10 existing processors pipeline integration` |
| 下一阶段 | `P08 强一致 State Store 与原子原语`（`P08-W01`，`NOT_STARTED`） |
...
| `src/ns_common` 实际能力 | `VERIFIED` | 存在配置、路径、异步生命周期、UTC/单调/可控时钟、统一标识符、重试、sanitizer、日志、HTTP、普通 cache、完整 `RUNTIME_*` 错误、metrics/trace/diagnostic sink 与隔离测试工厂。`ns_common` 和 `ns_common.config` facade 继续导出同一权威 `ns_config`，但只在调用方显式访问该兼容属性时线程安全地执行一次 `NsConfig.load()`；裸 package/submodule import 不再初始化全局配置。cache/logger/Django cache 也只在实际需要全局配置时解析该属性。`NsConfig.load(config_path=None)` 的默认路径与 `ensure_runtime_dirs()` 兼容语义保持不变；可选多进程日志依赖继续按实际使用延迟加载。 |
...
| `src/ns_runtime` 实际能力 | `VERIFIED` | P02-P06 生命周期、协议、transport、logical session 与 IAM-R1 保持冻结；P07 新增 PC-1 ProcessorContext、固定八阶段 pipeline、实例 registry、timeout/cancellation/error isolation、typed final audit、typed EventBus 和本地受信插件边界，并把 heartbeat/health/reauth/drain/feature-disabled 接入唯一 ingress。P08 StateStore、DeliveryRecord、可靠投递、RoutingPlan 与 cluster/management 实际执行仍未开始。 |
...
| 当前本地测试基线 | `VERIFIED` | P07 最终证据专项 `Ran 22, OK`；P07 与 connection/session 相关联合 `Ran 93, OK`；runtime 边界全量 `Ran 687, OK (skipped=1)`；backend 全量 `Ran 698, OK (skipped=49)`。全树 `compileall`、两环境 `pip check`、冷导入、`git diff --check`、P08/可靠投递、第二 owner、global processor state 与敏感事件/审计字段扫描通过。 |
| 当前状态校准时间 | `VERIFIED` | `2026-07-22T08:49:30+08:00` |
...
| 公共异常与错误注册 | `src/ns_common/exceptions/` | 73 个异常按显式领域模块登记；冻结 `NsErrorDefinition`、稳定 severity/category、显式不可变注册表和 19 域/66 码 `RUNTIME_*` 覆盖矩阵；20 个独立人工冻结设计场景包含 feature disabled；13 个 NACK reason 全部映射到细粒度 `RUNTIME_*` code；通用错误和未证明幂等的 processor timeout 采用保守策略，查询不做 MRO fallback；原 facade、构造、继承与四字段序列化兼容 | `tests/test_exceptions.py` 26/26 基线及 W05 联合专项；73 项完整策略矩阵及类/code/numeric_code 唯一；覆盖矩阵、独立场景、NACK 映射、冷启动导入和依赖扫描通过 | `F2` | `VERIFIED`，P01 合同由 P02-W05 增补统一功能禁用错误并完成全量回归 |
...
| Processor 与插件 | `src/ns_runtime/processor/`、`src/ns_runtime/connection/processors.py` | PC-1 normalized context 和显式依赖；AuditSink/EventBus 保持静态显式类型与 composition 注入；安全、授权、限流接口、幂等接口、审计标记、路由接口、message、response 显式固定八阶段；`MessageProcessor` binding 只能由 `MessageProcessorStageProcessor` execution boundary 进入标准 stage map；实例 registry 按 type/stage/version/feature flag 冲突检测；typed final audit；EventBus 以不可变 handle 管理 subscribe/unsubscribe，in-flight publish 使用快照语义；仅本地 allowlist plugin metadata/namespace 权限边界。P08 strong storage、真实 limiter/idempotency/routing、远程 plugin runtime 均不存在 | P07 最终专项 22/22；connection/session 相关联合 93/93；runtime 687/687；backend 698/698 | `F3` | P07 risks 已关闭且 evidence 完整，`VERIFIED`；PC-1 冻结 |
...
| `P07-W06` | `VERIFIED` | 高风险控制审计接口标记为强一致，普通 processor 审计先使用显式 sink；P08 后接 state store | 不把强审计误标为 best-effort |
...
| `ERR-1` | NsErrorDefinition、稳定 severity/category、显式不可变错误注册表、19 域/66 码 `RUNTIME_*` 完整覆盖矩阵、20 个独立人工冻结设计场景（含 `RUNTIME_FEATURE_DISABLED`）、全 `RUNTIME_*` NACK 精确映射、精确类型保守策略、code/numeric_code/类唯一性与原异常序列化兼容 | P01/P02 | `VERIFIED` |
...
| `RTC-1` | `ns_runtime.context.RuntimeContext` 的冻结关键字构造与显式依赖身份：NsConfig、Clock、Logger、MetricsSink、TraceSink、TaskSupervisor 为必需依赖；有限冻结 `RuntimeDependencySlots` 只为 DiagnosticSnapshotSink 与 NsHttpClientOwner 提供可选类型化槽位；RuntimeService 必须构造注入同一 context；无默认依赖创建、全局 context、字符串 registry 或 ambient getter；模块冷导入与合法/非法构造期类型校验不得因类型引用加载 `ns_common` 全局配置、文件系统、logger 或 HTTP 链；有限规范类型边界使用真实类对象判断且不提供依赖实例 locator | P02 | `VERIFIED` |
...
| `RSD-1` | `RuntimeShutdownCoordinator` 与 `RuntimeService` 必须共享同一 `RuntimeContext` 身份；首次 SIGINT/SIGTERM/显式/critical task 原因胜出并同步关闭 transport admission gate，shutdown 固定按 stop transport admission、stop logical admission/read loops、drain/close logical connections、drain/close transport sessions/adapters、cancel supervisor tasks、flush/close sinks、close 显式 HTTP owner、写安全有界摘要、close owned logger 的顺序执行。P04/P05 只通过类型化 `TransportLifecycleOwner`/`LogicalConnectionLifecycleOwner` 兼容扩展既有 coordinator，不新增 signal/shutdown/loop/supervisor owner。signal registration 精确恢复原 handler且幂等；普通资源失败只记录 phase/resource/error type并继续，进程级异常穿透；重复关闭返回同一冻结报告 | P02/P04/P05 | `VERIFIED` |
...
| `PC-1` | normalized ProcessorContext、固定八阶段 pipeline、实例 registry、supervised timeout/cancellation、typed final audit/EventBus 与 local trusted plugin boundary；限流、幂等、routing 只冻结接口，strong audit 只保存 requirement，P08 后接 authority store | P07 | `VERIFIED` |
...
|---|---|
| 当前阻塞项 | 无 |
| 下一工作包 | `P08-W01` 保持 `NOT_STARTED`；不得在新的显式指令前实现 StateStore、Redis/Valkey authority、lease、fencing、CAS 或可靠投递 |
