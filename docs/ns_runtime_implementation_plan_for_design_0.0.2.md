...
| 字段 | 当前值 |
|---|---|
| 当前阶段 | `P04 Transport 抽象与 WebSocket/TCP Adapter` |
| 当前工作包 | `P04-W01` |
| 当前工作包状态 | `NOT_STARTED` |
| 当前阶段状态 | `NOT_STARTED` |
| 最近已验证阶段 | `P03 Envelope 协议层与类型注册表` |
| 最近已验证工作包 | `P03-FIX-02 StreamGroup 严格数组与 extension namespace wire 语法边界` |
| 下一阶段 | `P05 连接、会话、握手、心跳与 Resume` |
...
验收记录：`P03-W01` 至 `P03-W11`、`P03-FIX-01`、`P03-FIX-02` 与阶段出口见 [acceptance log](ns_runtime_acceptance_log_0.0.2.md#p03-w01)；完整协议边界登记为 `ENV-1`，长期决策见 [ADR-028](ns_runtime_architecture_decisions_0.0.2.md#adr-028)。
...
| P04 | Transport 抽象与 WebSocket/TCP | `IN_PROGRESS` | F2 | P03 |
...
- P02-W04/P02-FIX-03 已冻结并校准 `RSP-1`：`ns_runtime._bootstrap` 重用权威配置/错误对象且不触发 global config；`main.py` 显式解析配置路径并复用唯一安全错误归一化，冷进程 dependency/security 失败时不初始化 global、调用 ensure、创建目录、安装 policy 或加载 service。P02-W05 在 preflight 成功后才以显式配置与显式 log root 构造带 Sanitizer 的 `NsLogger`，不恢复 global config 副作用。preflight 仍不证明 listener、StateStore、IAM、角色协调或生产证书可用；P04 前没有 adapter/listener，完整 TLS 材料仍归 P20，见 [ADR-023](ns_runtime_architecture_decisions_0.0.2.md#adr-023) 和 [ADR-024](ns_runtime_architecture_decisions_0.0.2.md#adr-024)。
...
- P03 已冻结 `ENV-1`，但注册不等于业务可用：除标准 runtime.error 构造外，connection/task/delivery/stream/control/cluster/config/dead-letter/replay/cancel/hold/status 全部继续 feature disabled。P04 前无 adapter/listener，P05/P06 前无 session/IAM 权威建立，P07/P08 前无通用 pipeline/强审计，P08/P10/P12/P17 前无 StateStore、DeliveryRecord、ACK 状态提交或集群权威；不得把 canonical bytes、registry metadata 或 best-effort audit log 解释为这些能力已实现，见 [ADR-028](ns_runtime_architecture_decisions_0.0.2.md#adr-028)。
