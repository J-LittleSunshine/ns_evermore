...
> 当前状态校准时间：`2026-07-17T16:44:27+08:00`
...
| 当前工作包 | `P01-W16 建立 ns_common.testing 测试工厂` |
| 当前工作包状态 | `NOT_STARTED` |
| 当前阶段状态 | `IN_PROGRESS` |
| 最近已验证阶段 | `P00 本地仓库基线与实施账本`（2026-07-16） |
| 最近已验证工作包 | `P01-FIX-08 加固观测 sink 故障隔离、metric 基数门禁和完整记录大小限制`；`P01-W15` 已恢复 `VERIFIED` |
...
| `src/ns_common` 实际能力 | `VERIFIED` | 存在配置、路径、异步生命周期、UTC/单调/可控时钟、统一标识符、固定/指数/jitter 退避、不可变 retry 预算、统一 sanitizer、日志、HTTP、普通 cache、完整 `RUNTIME_*` 错误覆盖矩阵，以及具备关闭故障隔离、35 项有限 metric definition 和完整 record 字节门禁的 metrics/trace/diagnostic sink 契约与内存实现；缺少 P01 要求的 testing 公共能力 |
...
| 当前本地测试基线 | `VERIFIED` | 最近一次本地验收：observability 专项 23/23；P01/runtime 联合 `Ran 213, OK (skipped=1)`；backend 环境根目录全量 `Ran 224, OK (skipped=1)`，跳过项均为 WSL 下同一 Windows 专用用例；`compileall`、runtime/backend 两套环境 `pip check`、190 项 common/43 项 observability facade 唯一导出与对象身份、35 项标准 metric definition、完整 record 字节限制、冷启动导入、导入循环、exporter/runtime/global sink 依赖、生产源码测试文件与仓库虚拟环境扫描和 `git diff --check` 通过；历史命令见 acceptance log |
| 当前状态校准时间 | `VERIFIED` | `2026-07-17T16:44:27+08:00` |
...
| 可观测与故障测试 | `src/ns_common/logger.py`、`src/ns_common/observability.py` | 已有日志、不可变脱敏 metrics/trace/diagnostic 记录、显式 sink 协议、关闭后原子拒绝与 dropped/rejected 分离、有界线程安全内存实现、35 项不可变标准 metric definition、有限 attribute schema、高基数别名防御和完整公开 record 字节门禁；尚无 runtime 采集器、外部 exporter 或故障注入框架 | `tests.test_observability` 23/23；P01/runtime 联合 213 项；backend 根回归 224 项；静态/环境门禁通过 | 公共观测基础 `F2`；runtime 观测 `F0` | `VERIFIED`，P01-W15/P01-FIX-08 已完成，P01-W16 `NOT_STARTED`，完整 runtime 观测与故障注入待 P02/P20 |
...
|---|---|
| 当前阻塞项 | 无 |
| 下一工作包 | `P01-W16 建立 ns_common.testing 测试工厂`（`NOT_STARTED`） |
...
- P01-W15/P01-FIX-08 已冻结不绑定 exporter 的 metrics/trace/diagnostic sink、关闭后的 best-effort 拒绝、`dropped_count`/`rejected_count` 分离、脱敏不可变记录、35 项显式有限 metric definition、高基数别名防御和完整公开 record 字节门禁，见 [ADR-018](ns_runtime_architecture_decisions_0.0.2.md#adr-018)。该公共基础不代表 runtime 指标采集、HTTP/OTLP exporter 或 P20 故障注入已经可用；P01-W16/P01-W17 尚未完成。`src/ns_runtime`、强一致 StateStore、Envelope、transport、session、delivery、cluster 和 management 仍按后续阶段保持禁用或 F0。
