...
| 最近已验证工作包 | `P10-W14 response emission isolation` |
...
| 生产状态存储目标 | P08 backend-neutral StateStore contract已冻结；P10新增delivery admission authority与StateStore原子adapter，无具体provider时typed unavailable，普通cache/transient memory不能替代；Redis/Valkey仍为生产provider目标 |
...
| `src/ns_common` 实际能力 | `VERIFIED` | 既有配置、路径、异步生命周期、时间、标识符、重试、sanitizer、日志、HTTP、普通 cache、错误、观测与隔离测试能力保持；P08 新增 backend-neutral StateStore authority/namespace/version/CAS/transaction/lifecycle/failure contract与11个稳定错误，不包含具体 provider、lease/fencing或可靠投递状态。cache与StateStore语义严格分离。 |
...
| `src/ns_runtime` 实际能力 | `VERIFIED` | P02-P09冻结能力保持；P10新增仅消费PC-1 stage-six typed ResolvedRoutingPlan的DR-1 admission、可信policy、inline/payload_ref证据、tenant dedup、Summary/prepared DeliveryRecord StateStore原子初始化和轻量typed response。无provider时unavailable，不含P11发送或P12 ACK能力。 |
| 配置示例与依赖清单 | `VERIFIED` | `etc/ns_config.example.json` 已包含 backend/cache/log、完整 runtime 总组及 17 个强类型 runtime 细分组；依赖按 `requirements-common.txt`、`requirements-backend.txt`、`requirements-runtime.txt`、`requirements-runtime-test.txt`、`requirements-runtime-benchmark.txt` 五层无环清单隔离 |
...
| 强一致状态存储 | `src/ns_common/state_store/`、`src/ns_runtime/state_authority.py` | backend-neutral authority/namespace/capability/revision/CAS/transaction/lifecycle/failure contract与strong audit authority chain；无具体provider、lease/fencing/cluster/delivery/routing | P08专项13项、runtime composition/audit 9项、runtime全量709项、backend全量720项、cold import与禁止项扫描 | `F3` contract only | P08 `VERIFIED`；provider能力明确deferred |
...
| 可靠投递 | `src/ns_runtime/delivery/` | DR-1受理policy、payload evidence、tenant dedup、MessageDeliverySummary、prepared DeliveryRecord、StateStore首批原子建图、每500条CAS推进、initializing-scope失败取消与轻量response；不注册task.dispatch production processor，不发送 | P10专项17项、P08-P10/PC-1联合、双loop及backend全量；详见acceptance log | `F3`（local only） | P10-W01至W14 `VERIFIED`；P11-W01 `BLOCKED` |
...
* Username：`ns`
* Password：`Ns981127*`
* Connection URL：`redis://ns:Ns981127*@127.0.0.1:6379/0`
...
| `COM-010` | 强一致 state store 抽象 | `ns_common.state_store` | `VERIFIED`（backend-neutral authority/namespace/capability/revision/CAS/transaction/lifecycle/failure contract；P10登记tenant-scoped delivery admission authority与p10 capability组合；仍无具体provider） | P08/P10 |
| `COM-011` | Redis/Valkey state store adapter 与 Lua runner | `ns_common.state_store` | `DEFERRED` | P08 后续 provider 阶段；P08 禁止实现 |
