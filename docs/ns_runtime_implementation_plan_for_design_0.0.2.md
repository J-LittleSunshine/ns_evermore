...
| 字段 | 当前值 |
|---|---|
| 当前阶段 | `P09 RoutingPlan 与本地路由` |
| 当前工作包 | `P09-W01` |
| 当前工作包状态 | `NOT_STARTED` |
| 当前阶段状态 | `NOT_STARTED / F0` |
| 最近已验证阶段 | `P08 StateStore 抽象契约与 Authority Boundary` |
| 最近已验证工作包 | `P08-W08 Contract Verification` |
| 下一阶段 | `P09 RoutingPlan 与本地路由`（`P09-W01`，`NOT_STARTED`） |
| 当前阻塞项 | 无；当前 WSL 验证使用 `/home/ns/.virtualenvs/ns_runtime` 与 `/home/ns/.virtualenvs/ns_backend` 隔离环境 |
| 设计基线版本 | `0.0.2` |
| wire codec | `json.v1` |
...
| 生产状态存储目标 | P08 backend-neutral StateStore contract 已冻结；无具体 provider，Redis/Valkey仍为后续生产目标，普通 cache不能替代 |
...
| `src/ns_runtime` 实际能力 | `VERIFIED` | P02-P07 既有 lifecycle、transport、session、IAM与processor owner保持冻结；P08通过RuntimeContext显式slot、RuntimeService ready gate和既有RuntimeShutdownCoordinator接入可选StateStore，并新增StrongAuditAuthorityService链路。无默认provider、Processor直接Store访问、第二lifecycle owner、DeliveryRecord、RoutingPlan或cluster执行。 |
...
| 当前本地测试基线 | `VERIFIED` | P08与相关runtime专项 `Ran 102, OK`；runtime边界全量 `Ran 709, OK (skipped=1)`；backend全量 `Ran 720, OK (skipped=49)`。全树compileall、两环境pip check、cold import、diff check、state ownership与P08禁止项扫描通过。 |
| 当前状态校准时间 | `VERIFIED` | `2026-07-22T10:31:58+08:00` |
...
| 公共异常与错误注册 | `src/ns_common/exceptions/` | 84 个异常按显式领域模块登记；冻结 `NsErrorDefinition`、稳定 severity/category、显式不可变注册表和 20 域/77 码 `RUNTIME_*` 覆盖矩阵；P08新增11个StateStore严格失败错误；既有设计场景与NACK映射不变，查询不做MRO fallback | `tests/test_exceptions.py` 26/26；84项完整策略矩阵及类/code/numeric_code唯一；覆盖矩阵、冷启动导入和依赖扫描通过 | `F2` | `VERIFIED`，ERR-1由P08扩展StateStore错误域 |
...
| Envelope 与类型注册 | `src/ns_runtime/protocol/` | ENV-1 grouped `json.v1`、raw/normalized authority、resource limits、schema/version/registry/extensions、canonical bytes、ERR-1 与单一 feature/direction/processor dispatch metadata；P05 后续正式激活 9 项 connection lifecycle contract 并冻结 exact schema，其余 40 项继续 disabled | P03 54项专项及 P01-P05 联合回归 | `F2` | `VERIFIED`，P03 合同冻结、P05 activation 校准 |
...
| Session、握手与恢复 | `src/ns_runtime/connection/`、`src/ns_runtime/iam/` | SC-1 logical lifecycle 保持冻结；production composition 已显式注入 `IamClient` 和 P01 HTTP owner，handshake/reauth/resume 统一 fail-closed，P07 后续补充 principal type continuity 门禁。IAM-R1 增加最小 PermissionSnapshot、strict/cache 鉴权、tenant 前置硬校验、TTL/version/invalidation、AES-GCM 节点凭证缓存、backend unavailable 矩阵和恢复六项重验，不包含 P08 lease/fencing 实现 | P05 connection 联合回归 114/114；P06 runtime 专项 15/15；根目录全量 676/676；P07 principal drift 负向回归通过 | `F3` | P05/P06 保持 `VERIFIED`；P07 continuity 已验证 |
| Processor 与插件 | `src/ns_runtime/processor/`、`src/ns_runtime/connection/processors.py` | PC-1 normalized context 和显式依赖；AuditSink/EventBus 保持静态显式类型与 composition 注入；安全、授权、限流接口、幂等接口、审计标记、路由接口、message、response 显式固定八阶段；实例 registry、typed final audit、EventBus subscription和本地allowlist plugin边界保持。P08新增AuditSink到StrongAuditAuthorityService的间接链路，Processor不导入StateStore；仍无真实limiter/idempotency/routing或远程plugin runtime | P07专项与connection/session回归保持；P08 runtime/audit专项9项、runtime全量709项、backend全量720项 | `F3` | P07 PC-1与P08 strong audit authority均`VERIFIED` |
...
| `ERR-1` | NsErrorDefinition、稳定 severity/category、显式不可变错误注册表、20 域/77 码 `RUNTIME_*` 完整覆盖矩阵、20 个独立人工冻结设计场景（含 `RUNTIME_FEATURE_DISABLED`）、全 `RUNTIME_*` NACK 精确映射、精确类型保守策略、code/numeric_code/类唯一性与原异常序列化兼容；P08增加11个StateStore严格失败错误 | P01/P02/P08 | `VERIFIED` |
...
| `ENV-1` | `ns_runtime.protocol` transport-independent `json.v1`：固定 12 分组与 strict raw/normalized authority boundary；1 MiB/32 depth/65536 string/4096 container/100000 node/signed-64/finite-number limits；base + exact type schema；major-strict explicit compatibility/schema matrix；50 项/13 族不可变 registry 及完整 feature/direction/processor dispatch metadata；direct extension namespace registry；ERR-1 + SEC-1 标准 runtime.error；sorted compact UTF-8 canonical bytes 与 SHA-256。P03 冻结时 49 项非 error 类型均 disabled；P05-FIX-01 后 9 项 connection lifecycle contract enabled，余 40 项统一返回 `RUNTIME_FEATURE_DISABLED`，无 callback/stub success | P03/P05 | `VERIFIED` |
...
| `SC-1` | frozen/slots/kw-only SessionContext：logical connection/session/epoch、identity/tenant/component、negotiated protocol/schema/codec/capability、permission ref/digest/version、IAM mode/issued/expiry/resume eligibility、established/created time；不含token、permission mapping、IAM raw、transport/path/peer、task/sink/client/callback。P05 logical owner 只从 canonical validated hello 建立该 context；ACTIVE read loop 对 current session/epoch hard check 后以 canonical processor_key 调度 heartbeat/drain/reauth，resume IAM 重验后 epoch+1，reauth 原子替换 authority/index | P05 | `VERIFIED` |
...
| `PC-1` | normalized ProcessorContext、固定八阶段pipeline、实例registry、supervised timeout/cancellation、typed final audit/EventBus与local trusted plugin boundary；限流、幂等、routing只冻结接口；strong audit经AuditSink间接路由到P08 authority service，Processor contract不含StateStore | P07/P08 | `VERIFIED` |
...
|---|---|
| 当前阻塞项 | 无 |
| 下一工作包 | `P09-W01` 保持 `NOT_STARTED`；不得因P08 contract存在而提前实现可靠投递、routing target selection、具体StateStore provider、lease/fencing或cluster |
