...
| 强一致状态存储 | `src/ns_common/state_store/`、`src/ns_runtime/state_authority.py` | P08 authority map与strong audit chain保持；P11追加typed paginated scan capability，Redis transaction Lua原子维护namespace/object-type有序索引；delivery scan、claim、状态/attempt/summary迁移均经同一authority与revision/state-version冲突边界 | StateStore/P11/配置67项；provider与真实Redis integration 24项；全量816项见acceptance log | `F3`（standalone local evidence） | Redis standalone P11 authority path `VERIFIED`；Valkey driver compatibility `IMPLEMENTED`；Valkey server integration `UNVERIFIED` |
...
| 可靠投递 | `src/ns_runtime/delivery/` | P10 DR-1/prepared初始化与RP-1 plan保持；P11权威激活、三层水位、priority、原子claim、本地owner lease/risk、发送前重校验、queued -> sending + DeliveryAttempt、write timeout、success -> ack_waiting及typed write_failed已实现。无Router调用、target重选或transport-success成功语义 | P11 scheduler 12项、真实Redis多worker claim与冲突、全量816项通过 | `F3`（local only） | P11-W01至W11 `VERIFIED`；只到ack_waiting，P12仍`BLOCKED/F0` |
