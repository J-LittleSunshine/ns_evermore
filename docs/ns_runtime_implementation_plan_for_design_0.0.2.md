...
> 当前状态校准时间：`2026-07-20T15:53:32+08:00`
...
| 最近已验证工作包 | `P02-FIX-03 修复真实 main() 配置错误归一化与目录副作用` |
...
| `src/ns_common` 实际能力 | `VERIFIED` | 存在配置、路径、异步生命周期、UTC/单调/可控时钟、统一标识符、固定/指数/jitter 退避、不可变 retry 预算、统一 sanitizer、日志、HTTP、普通 cache、完整 `RUNTIME_*` 错误覆盖矩阵、具备关闭故障隔离/35 项有限 metric definition/完整 record 字节门禁的 metrics/trace/diagnostic sink，以及实例隔离且使用 OPEN/CLOSING/CLOSED 可重试并发关闭屏障的临时目录/配置/可控 clock/内存 sink/保留端口/真实 Redis namespace 测试工厂；可选多进程日志依赖与 cache logger 均按实际使用延迟加载 |
...
| `src/ns_runtime` 实际能力 | `VERIFIED` | 已建立独立 package、保持冷导入无副作用的唯一 `main.py` 进程入口、created/starting/running/stopping/stopped/failed 六态一次性 `RuntimeService`，以及冻结、显式注入的 `RuntimeContext`。同步 `RuntimeStartupPreflight` 严格解析 local/dev/test/prod；`load_config_snapshot()` 只接受显式配置路径，复用不可变配置校验并与 preflight 二次校验共用唯一 startup security 错误归一化；只允许当前 `websocket_tcp` 配置基线，检查 `websockets`、服务端 TLS context 能力、data/etc/log/tmp 与 SQLite parent 可访问性，生产仅接受 Redis/Valkey 配置，所有检查通过后才选择或安装 event-loop policy。冻结结果不公开 URL、真实路径或底层异常。main 显式解析默认配置文件路径后加载快照，不触发 CFG-1 无路径加载的目录语义；随后构造 context、执行 preflight，再启动并停止无监听 RuntimeService。缺失依赖、TLS 失败或非法 transport 均不准备目录、不安装 policy、不构造 service。尚未创建 listener、transport adapter、StateStore、HTTP client/exporter、角色状态、信号关闭编排、loop lag collector、协议或消息链路。 |
...
| 当前本地测试基线 | `VERIFIED` | 最近一次本地验收：P02-FIX-03 startup/main 专项 25/25，W01 入口、W02/FIX-01 生命周期、W03/FIX-02 context 与 W04/FIX-03 startup 联合 58/58；runtime 环境 P01/P02 非 Django cache 联合回归 `Ran 314, OK (skipped=1)`，backend 环境根目录全量 `Ran 325, OK (skipped=1)`；唯一跳过为 WSL 下 Windows 专用 event-loop policy 用例。真实 main 负向测试确认四类 startup security 配置归一化、普通配置错误保持、缺失 `websockets`/TLS/非法 transport 不准备目录、不安装 policy、不构造 service；两套持久环境 `pip check`、`compileall` 和 `git diff --check` 门禁通过，命令见 acceptance log。 |
| 当前状态校准时间 | `VERIFIED` | `2026-07-20T15:53:32+08:00` |
...
| `P02-FIX-03` | `VERIFIED` | 修复真实 main() 配置加载绕过 RSP-1 错误归一化与目录顺序 | 显式配置路径加载；安全配置错误统一，依赖/TLS/transport 失败无目录、policy 或 service 副作用 |
...
- `P02-FIX-03`：见 [acceptance log](ns_runtime_acceptance_log_0.0.2.md#p02-fix-03)；真实 composition root 的显式配置加载、统一安全错误归一化与无目录副作用边界已校准进 `RSP-1` 和 [ADR-023](ns_runtime_architecture_decisions_0.0.2.md#adr-023)。
...
| `RSP-1` | `ns_runtime.startup.RuntimeStartupPreflight` 的同步启动边界：`load_config_snapshot()` 强制使用显式配置路径并与 `validate()`/`prepare()` 共用唯一 startup security 配置错误归一化；普通配置错误保持 CFG-1 稳定类型。`validate()` 不替换 policy，`prepare()` 只在严格环境、完整配置、安全限制、transport feature gate、固定 Python 依赖、服务端 TLS 能力和显式目录全部通过后安装 event-loop policy；结果不公开 URL、真实路径或底层异常；当前只接纳 `websocket_tcp` 并检查 `websockets`，不创建 listener/adapter/StateStore/client/exporter；`ns_runtime.main` 显式解析默认配置路径，保持冷导入无副作用并按冻结顺序运行无监听 RuntimeService | P02 | `VERIFIED` |
...
- P02-W04/P02-FIX-03 已冻结并校准 `RSP-1`：`main.py` 显式解析默认配置文件路径后调用 `load_config_snapshot()`，不进入 CFG-1 无路径加载的目录创建语义；配置加载和 preflight 二次验证共用唯一 startup security 错误归一化。依赖、TLS 或 transport 失败时不准备目录、不安装 policy、不构造 service；成功后才运行一次无监听生命周期。入口模块冷导入仍不加载 `ns_common`、startup/context/service、uvloop 或 websockets。preflight 不证明 WebSocket listener、Redis/Valkey StateStore、IAM、角色协调或生产证书可用；P04 前没有 adapter/listener，完整 TLS 材料与策略仍归 P20。main 当前仍注入普通 `logging.Logger`；W05 开始实际使用 runtime logger 前必须完成生产安全日志接线。backend 环境按 `DEP-1` 继续验证缺失 `websockets` 的 fail-closed 分支，见 [ADR-023](ns_runtime_architecture_decisions_0.0.2.md#adr-023)。
