...
| 可靠投递 | `src/ns_runtime/delivery/` | DR-1受理policy、payload evidence、tenant dedup、MessageDeliverySummary、prepared DeliveryRecord、StateStore原子初始化、initializing-scope取消与轻量response；不注册task.dispatch production processor，不发送 | P10专项16项、P08-P10/PC-1联合、双loop及backend全量；详见acceptance log | `F3`（local only） | P10-W01至W14 `VERIFIED`；P11-W01 `BLOCKED` |
...
| `P10-W06` | `VERIFIED` | 建立 `message_id + target_fingerprint` tenant 去重登记 | dedup与Summary/delivery同一StateStore transaction，并发唯一winner |
...
| `P10-W09` | `VERIFIED` | 关键消息 summary + 初始 prepared delivery 原子写入 | backend-neutral transaction同成同败；indeterminate单独表达，无orphan/dedup残留声明 |
| `P10-W10` | `VERIFIED` | 大 fanout超过5000 targets时分片；bucket 1000、初始化批次500 | 4999/5000/5001真实RP-1 plan边界验证，仅5001产生6 shard |
...
| `P10-W12` | `VERIFIED` | 初始化失败批量取消已prepared，记录not_initialized | 唯一initializing-scope取消纯迁移批量关闭created并记录未创建target；终态不可重复取消 |
