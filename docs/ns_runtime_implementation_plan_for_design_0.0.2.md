...
> 当前状态校准时间：`2026-07-17T22:58:14+08:00`
...
| 最近已验证工作包 | `P02-W02 建立 RuntimeService 生命周期状态机` |
...
| `src/ns_runtime` 实际能力 | `VERIFIED` | 已建立独立 package、无导入副作用的唯一 `main.py` 进程入口，以及 created/starting/running/stopping/stopped/failed 六态 `RuntimeService`；service 的 start/stop 串行绑定单一 event loop，hook 普通异常或取消进入 failed，非法迁移返回稳定 `NsStateError`；main 尚未接入 service，RuntimeContext、协议、transport、session、processor、路由、delivery、stream、cluster、management 与 observability 仍为 F0 |
...
| 当前本地测试基线 | `VERIFIED` | 最近一次本地验收：P02-W02 专项 10/10、W01+W02 入口/生命周期专项 13/13；P01/runtime 联合加 P02 回归 `Ran 269, OK (skipped=1)`；backend 环境根目录全量 `Ran 280, OK (skipped=1)`，跳过项均为 WSL 下同一 Windows 专用用例；五份依赖清单内容未改变，既有四类仓库外全新环境安装证据保持有效，runtime/backend 持久环境 `pip check` 和真实 standalone Redis namespace 回归通过；`compileall`、冷导入/无后台任务、生产源码测试文件、仓库虚拟环境和 `git diff --check` 门禁通过，命令见 acceptance log |
| 当前状态校准时间 | `VERIFIED` | `2026-07-17T22:58:14+08:00` |
...
| runtime 进程与事件循环 | `src/ns_runtime/__init__.py`、`src/ns_runtime/main.py`、`src/ns_runtime/service.py` | 独立 package 与唯一模块进程入口保持无副作用；`RuntimeServiceState` 冻结六态，`RuntimeService.start()/stop()` 串行执行并暴露 starting/stopping，正常路径到 running/stopped，hook 异常或取消到 failed，stopped/failed 为终态，跨 loop 与非法迁移使用稳定 `NsStateError`；main 集成、运行上下文、启动校验和信号关闭尚未实现 | `tests.test_runtime_main` 3/3、`tests.test_runtime_service` 10/10；P01/runtime 联合加 P02 回归 269 项；backend 根回归 280 项 | 进程入口 `F1`；service 生命周期 `F2`；loop 集成与上下文 `F0` | P02-W01/W02 `VERIFIED`；P02 `IN_PROGRESS` |
...
- `P02-W02`：见 [acceptance log](ns_runtime_acceptance_log_0.0.2.md#p02-w02)；RuntimeService 六态生命周期契约登记为 `RSL-1`。
...
| `RSL-1` | `ns_runtime.service.RuntimeService` 的 created/starting/running/stopping/stopped/failed 六态一次性生命周期；start/stop 同 loop 串行，hook 异常或取消进入 failed，stopped/failed 为终态，非法迁移返回固定 `NS_STATE_ERROR` 与结构化状态 details | P02 | `VERIFIED` |
...
- P02-W02 已冻结 `RuntimeService` 六态一次性生命周期和稳定非法迁移错误；当前 `main.py` 尚未创建或运行 service，service 也尚未持有配置、clock、logger、sink、task supervisor 或 client。上述显式依赖统一留给 P02-W03，启动校验、角色、信号关闭和 loop 观测继续留给 P02-W04 至 W08。
