...
| 最近已验证工作包 | `P05-W12 non-resumable security close` |
...
| `P05-W13` | `IN_PROGRESS` | 建立 connection.reauth、accepted/rejected 语义和 session 到期策略接口 | 续期失败不会无限信任旧权限 |
...
|---|---|
| 当前阻塞项 | 无 |
| 下一工作包 | `P05-W13 reauth and session expiry policy`（`IN_PROGRESS`） |
