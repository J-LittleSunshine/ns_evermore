...
> 当前状态校准时间：`2026-07-20T18:01:53+08:00`
...
| 当前工作包 | `P02-W05 初始角色状态与未完成能力门禁` |
| 当前工作包状态 | `NOT_STARTED` |
| 当前阶段状态 | `IN_PROGRESS` |
| 最近已验证阶段 | `P01 ns_common 公共基础设施加固` |
| 最近已验证工作包 | `P02-FIX-03 修复真实 main() 配置加载与冷进程 bootstrap 副作用` |
...
| `src/ns_runtime` 实际能力 | `VERIFIED` | 已建立独立 package、保持冷导入无副作用的唯一 `main.py`、六态一次性 `RuntimeService` 与冻结显式 `RuntimeContext`。内部 `ns_runtime._bootstrap` 只重导出权威默认配置路径、`NsConfig`、startup 错误类型和路径常量，不初始化 `ns_config` 或准备目录；`main()` 首次冷调用经该边界显式加载配置。`RuntimeStartupPreflight.load_config_snapshot()` 与 preflight 二次校验共用唯一安全错误归一化；transport admission、`websockets`、TLS 能力、显式目录与 policy 保持冻结顺序。独立解释器证明 dependency/security 失败前 global config 未初始化、`ensure_runtime_dirs` 未调用、仓库与显式目录均未创建、policy 未安装且 service 未加载。尚未创建 listener、transport adapter/session、StateStore、HTTP client/exporter、角色状态、信号关闭编排、loop lag collector、协议或消息链路。 |
...
| 当前本地测试基线 | `VERIFIED` | 最近一次本地验收：main/startup/独立解释器 bootstrap 专项 27/27，W01、W02/FIX-01、W03/FIX-02 与 W04/FIX-03 联合 60/60；runtime 环境 P01/P02 非 Django cache 联合回归 `Ran 317, OK (skipped=1)`，backend 环境根目录全量 `Ran 328, OK (skipped=1)`；唯一跳过为 WSL 下 Windows 专用 event-loop policy 用例。两条真正冷进程分别验证 `NS_DEPENDENCY_ERROR` 与 `RUNTIME_STARTUP_SECURITY_ERROR`，且 global `ns_config`、ensure、目录、policy、service 均零副作用；CFG-1 facade/cache/logger/testing 兼容专项 67/67。两套 `pip check`、`compileall` 和 `git diff --check` 通过，命令见 acceptance log。 |
| 当前状态校准时间 | `VERIFIED` | `2026-07-20T18:01:53+08:00` |
...
| 公共异常与错误注册 | `src/ns_common/exceptions/` | 72 个异常按显式领域模块登记；冻结 `NsErrorDefinition`、稳定 severity/category、显式不可变注册表和 18 域/65 码 `RUNTIME_*` 覆盖矩阵；19 个独立人工冻结设计场景阻断 registry/matrix 自证闭环；13 个 NACK reason 全部映射到细粒度 `RUNTIME_*` code；通用错误和未证明幂等的 processor timeout 采用保守策略，查询不做 MRO fallback；原 facade、构造、继承与四字段序列化兼容 | `tests/test_exceptions.py` 26/26；72 项完整策略矩阵及类/code/numeric_code 唯一；覆盖矩阵、独立场景、NACK 映射、冷启动导入和依赖扫描通过 | `F2` | `VERIFIED`，P01-W11/P01-FIX-06/P01-W12/P01-FIX-07 已完成 |
...
| `LOG-1` | NsLogger 单向注入/拥有 Sanitizer；JSON/text/color 在输出前处理消息、格式参数、extra、异常和安全 traceback 元数据，不直接序列化原始对象；调用方 extra 不得覆盖权威日志字段，冲突值只进入 `extra_fields`；可选多进程 handler 只在实际使用时加载 | P01 | `VERIFIED` |
| `ERR-1` | NsErrorDefinition、稳定 severity/category、显式不可变错误注册表、18 域/65 码 `RUNTIME_*` 完整覆盖矩阵、19 个独立人工冻结设计场景、全 `RUNTIME_*` NACK 精确映射、精确类型保守策略、code/numeric_code/类唯一性与原异常序列化兼容 | P01 | `VERIFIED` |
...
|---|---|
| 当前阻塞项 | 无 |
| 下一工作包 | `P02-W05 初始角色状态支持 singleton、sub_node、standby_master、active_master 配置值；实际协调能力保持 feature disabled`（`NOT_STARTED`） |
...
- P01-W12 与 P01-FIX-07 已完成 18 域/65 码 `RUNTIME_*` 覆盖矩阵、19 个独立人工冻结设计场景和 13 个 NACK reason 的细粒度映射；新增 `RUNTIME_PROTOCOL_VIOLATION`，processor timeout 默认不再自动重试，单条 protocol parse error 默认不再无条件断连。既有 71 个异常未重编号，原类名、模块、继承、构造、details 和四字段序列化保持兼容。策略 metadata 只作用于精确异常类型且不是无条件执行指令；`safe_detail` 只是策略元数据，实际输出仍必须经过 sanitizer/logger/未来错误 Envelope 边界，见 [ADR-015](ns_runtime_architecture_decisions_0.0.2.md#adr-015)。
...
- P02-W04/P02-FIX-03 已冻结并校准 `RSP-1`：`ns_runtime._bootstrap` 重用权威配置/错误对象且不触发 global config；`ns_common` 两层 facade 及 cache/logger 仅在显式实际使用时解析 `ns_config`，其真实类型、身份和无参 load 兼容语义保持。`main.py` 显式解析配置路径并复用唯一安全错误归一化；冷进程 dependency/security 失败时不初始化 global、调用 ensure、创建仓库或显式目录、安装 policy、加载 service。入口模块冷导入仍不加载 `ns_common`。preflight 不证明 listener、StateStore、IAM、角色协调或生产证书可用；P04 前没有 adapter/listener，完整 TLS 材料仍归 P20。main 当前仍注入普通 `logging.Logger`；W05 开始实际使用 runtime logger 前必须完成生产安全日志接线，见 [ADR-023](ns_runtime_architecture_decisions_0.0.2.md#adr-023)。
