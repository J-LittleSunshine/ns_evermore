...
| `P04-W10` | `IN_PROGRESS` | 记录通用 transport 指标：连接数、握手耗时、收发字节、队列深度、发送失败、关闭原因、背压时长 | 指标标签不含高基数 ID |
...
| `RSD-1` | `RuntimeShutdownCoordinator` 与 `RuntimeService` 必须共享同一 `RuntimeContext` 身份；首次 SIGINT/SIGTERM/显式/critical task 原因胜出并立即关闭本地接入门禁，shutdown 按停止新任务与取消后台任务、flush sink、close sink、close 显式 HTTP owner、写安全有界摘要、close owned logger 的顺序执行。signal registration 在 asyncio 与 fallback 路径安装前保存 SIGINT/SIGTERM handler，close 先撤销 loop handler再精确恢复原对象且幂等。普通资源失败只记录 phase/resource/error type 并继续，进程级异常穿透；超时只摘要化记录未完成任务，重复关闭返回同一冻结报告。P02 不含 transport listener/drain、连接迁移、Envelope 或管理旁路 | P02 | `VERIFIED` |
...
- P02-W06 已冻结 `RSD-1`：SIGINT、SIGTERM、显式 stop 与无监听自检复用同一 coordinator；首次原因胜出，关闭顺序、普通失败隔离、安全摘要、logger ownership 和 context 身份均已固定，见 [ADR-025](ns_runtime_architecture_decisions_0.0.2.md#adr-025)。当前“停止接入”仅关闭本地 admission gate，TaskSupervisor 进入 closing 后拒绝新任务；P04 首个 listener 出现后必须通过类型化 hook 扩展 transport drain，不能把本阶段结果解释为连接 draining、通知重连、delivery 转移或管理通道已实现。
