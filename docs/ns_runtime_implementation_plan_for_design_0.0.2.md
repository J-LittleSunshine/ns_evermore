...
| 最近已验证工作包 | `P09-W10 Failure Model` |
| 下一阶段 | `P10 受理、Summary、去重与 Payload Reference`（`P10-W01`，`NOT_STARTED`） |
| 当前阻塞项 | 无；本轮 WSL 验证使用 `/home/ns/.virtualenvs/ns_runtime_p09` 与 `/home/ns/.virtualenvs/ns_backend_p09` 隔离环境 |
| 设计基线版本 | `0.0.2` |
| wire codec | `json.v1` |
...
| RoutingPlan 与调度 | `src/ns_runtime/routing/` | RP-1单进程本地routing：一次index snapshot、target expansion、IAM tenant边界、静态/动态过滤、确定性fallback.v1评分、六种strategy、五种rebind与deeply immutable plan/failure。ordinary recorder仅接收安全投影；strong authority仅冻结接口且默认unavailable | routing专项、5001 fanout、determinism/immutability/security与禁止边界扫描；Linux双loop全量731项 | `F3`（local only） | P09-W01至W10 `VERIFIED`；remote/master routing与strong persistence未启用 |
...
| `RP-1` | 单快照本地target expansion、IAM tenant boundary、静态/动态过滤、single/all/broadcast/quorum/all_required/weighted_subset、五种rebind、`runtime_fallback/fallback.v1`确定性评分，以及deeply immutable ResolvedRoutingPlan/RoutingFailureReport/SafeRoutingProjection。plan version只来自显式previous context；ordinary recorder为安全摘要接口；strong authority仅冻结接口且无provider时unavailable；无StateStore直连、transport写入、delivery/ACK/retry、stale/remote/master routing | P09 | `VERIFIED` |
...
|---|---|
| 当前阻塞项 | 无 |
| 下一工作包 | `P10-W01` 保持 `NOT_STARTED`；不得因RP-1存在而提前实现DeliveryRecord/attempt、ACK/NACK/Defer、retry/queue/timer、strong RoutingPlan provider、remote/master routing、lease/fencing或cluster |
