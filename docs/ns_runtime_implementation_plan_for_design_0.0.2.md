...
| 当前工作包 | `P03-W02 inbound raw 与 normalized model 分离` |
| 当前工作包状态 | `IN_PROGRESS` |
| 当前阶段状态 | `IN_PROGRESS` |
...
| `P03-W02` | `IN_PROGRESS` | 建立 inbound raw model 与 normalized model 分离；inbound 不允许 source/auth_context；normalized 由 runtime 注入 | 伪造测试全部拒绝 |
...
验收记录：`P03-W01` 见 [acceptance log](ns_runtime_acceptance_log_0.0.2.md#p03-w01)；分组模型边界登记为 `ENV-1`，长期决策见 [ADR-028](ns_runtime_architecture_decisions_0.0.2.md#adr-028)。
...
|---|---|
| 当前阻塞项 | 无 |
| 下一工作包 | `P03-W02 inbound raw 与 normalized model 分离`（`IN_PROGRESS`） |
