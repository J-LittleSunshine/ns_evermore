...
| 可靠投递 | `src/ns_runtime/delivery/` | P10 DR-1/prepared初始化与RP-1 plan保持；P11权威分页激活、三层水位、原子claim、持久last_fencing/owner_epoch、lease/risk、发送前source/auth/protocol/payload实时复验、queued -> sending + DeliveryAttempt、indeterminate reconcile、success -> ack_waiting及typed waiting/failure/uncertain已实现。无Router调用、target重选或transport-success成功语义 | P10/P11定向45项、真实Redis15项、asyncio836/uvloop825/backend821项通过 | `F3`（local only） | P11-W01至W11及P11-FIX-02 `VERIFIED`；只到ack_waiting，P12仍`BLOCKED/F0` |
