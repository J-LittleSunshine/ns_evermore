...
> 当前状态校准时间：`2026-07-20T18:43:02+08:00`
...
| 当前工作包 | `P02-W07 event loop lag 与 implementation 指标` |
| 当前工作包状态 | `IN_PROGRESS` |
| 当前阶段状态 | `IN_PROGRESS` |
| 最近已验证阶段 | `P01 ns_common 公共基础设施加固` |
| 最近已验证工作包 | `P02-W06 SIGINT/SIGTERM 优雅关闭编排` |
...
| `src/ns_runtime` 实际能力 | `VERIFIED` | 已建立独立 package、保持冷导入无副作用的唯一 `main.py`、六态一次性 `RuntimeService`、冻结显式 `RuntimeContext`、同步 startup preflight、只读初始角色/健康/能力快照，以及 SIGINT/SIGTERM 共用的本地优雅关闭编排。关闭顺序固定为停止本地接入、关闭 TaskSupervisor 新任务入口并取消后台任务、flush/close sink、关闭显式 HTTP owner、输出安全摘要、关闭 owned logger；重复关闭返回同一冻结报告，超时与普通清理失败可观测。singleton、sub_node、standby_master、active_master 四个配置角色均可装载；transitioning/draining 与健康域保持分离，但尚无角色或健康迁移 API。transport、cluster_coordination、delivery 均由统一 `RUNTIME_FEATURE_DISABLED` 门禁审计后拒绝，active_master 配置值不授予选主、lease、fencing 或协调写权限。尚未创建 listener、transport adapter/session、StateStore、HTTP client/exporter、loop lag collector、协议或消息链路。 |
...
| 当前本地测试基线 | `VERIFIED` | P02-W06 shutdown/service/main/context/bootstrap/logger 专项 `Ran 66, OK`；runtime 环境非 Django cache 全量 `Ran 330, OK (skipped=1)`，backend 环境根目录全量 `Ran 341, OK (skipped=1)`；唯一跳过为 WSL 下 Windows 专用 event-loop policy 用例。真实 SIGTERM 注册/恢复、关闭超时、失败隔离、资源顺序、context 身份和摘要零原始任务名泄露均已验证；既有 dependency/security 冷进程零副作用回归保持通过。两套 `pip check`、`compileall` 和 `git diff --check` 通过，命令见 acceptance log。 |
| 当前状态校准时间 | `VERIFIED` | `2026-07-20T18:43:02+08:00` |
...
| runtime 进程与事件循环 | `src/ns_runtime/__init__.py`、`src/ns_runtime/main.py`、`src/ns_runtime/context.py`、`src/ns_runtime/service.py`、`src/ns_runtime/startup.py`、`src/ns_runtime/shutdown.py` | package/入口冷导入无启动副作用；`RuntimeServiceState` 六态一次性生命周期和显式 `RuntimeContext` 保持冻结；startup preflight 在 event-loop policy 与任何未来 listener 前完成严格校验；RuntimeShutdownCoordinator 复用同一 context，统一 SIGINT/SIGTERM/显式停止路径，按固定顺序停止本地接入与新任务、取消后台任务、flush/close sink、关闭显式 client、输出安全摘要并关闭 owned logger；无监听入口自检后通过同一 coordinator 退出 | shutdown/service/main/context/bootstrap/logger 66/66；runtime 联合 330 项、backend 根回归 341 项，均仅 1 个 Windows 专用跳过 | 进程入口、composition root、startup、service/context、角色与信号关闭 `F2`；loop 观测 `F0` | P02-W01 至 W06（含 FIX）`VERIFIED`；P02 `IN_PROGRESS` |
...
| `P02-W07` | `IN_PROGRESS` | 建立 event loop lag 采样和 implementation 指标 | asyncio/uvloop 指标可读取 |
...
|---|---|
| 当前阻塞项 | 无 |
| 下一工作包 | `P02-W07 event loop lag 采样和 implementation 指标`（`IN_PROGRESS`） |
