...
- 禁止在仓库目录内创建 `.venv`、`venv` 或其他虚拟环境；禁止为本项目绕过 `S:\PythonVenv` 下的共享隔离环境安装依赖。
...
| 当前工作包 | `P01-W03 建立 runtime 细分配置组` |
| 当前阶段状态 | `IN_PROGRESS` |
...
| `src/ns_common` 实际能力 | `VERIFIED` | 存在配置、路径、日志、HTTP、普通 cache 与部分 runtime 错误类；缺少 P01 要求的异步生命周期、时钟、ID、retry、sanitizer、observability、testing 等公共能力 |
...
| 配置示例与依赖清单 | `VERIFIED` | `etc/ns_config.example.json` 仅含 backend/cache/log；`requirements-runtime.txt` 与 `requirements-backend.txt` 存在，无独立测试依赖清单 |
...
|---|---|---|---|---|---|
| 配置基础设施 | `src/ns_common/config.py` | backend/cache/log/runtime 总组的深度不可变快照、组 metadata、严格加载/序列化、兼容别名、固定来源优先级、backend 覆盖和已验证快照恢复已完成；runtime 细分组待 W03 | `tests/test_config.py` 24/24；根测试回归 35/35 | `F2` | `IN_PROGRESS`，P01-W01/W02 `VERIFIED`，进入 P01-W03 |
| 路径与生命周期 | `src/ns_common/paths.py` | 仓库固定路径常量与目录创建；无 event loop selector、TaskSupervisor 或优雅关闭基础 | 源码审查 | `F1` | 路径已实现，生命周期待 P01 |
| 日志与脱敏 | `src/ns_common/logger.py` | JSON/text/彩色 formatter、轮转与全局 logger map；未统一脱敏，multiprocessing 导入依赖缺失会失败 | 基线 loader error；源码审查 | `F1` | `IN_PROGRESS`，待 P01 sanitizer 接入 |
...
|---|---|---|---|---|
| `COM-001` | 不可变配置快照、配置来源和组版本元数据 | `ns_common.config` | `VERIFIED` | P01 |
| `COM-002` | event loop 选择、任务监督和优雅关闭基础 | `ns_common.async_runtime` | `NOT_STARTED` | P01-P02 |
| `COM-003` | 单调时钟、UTC 时间和可控测试时钟 | `ns_common.time` | `NOT_STARTED` | P01 |
| `COM-004` | 标识符生成和格式校验 | `ns_common.identifiers` | `NOT_STARTED` | P01 |
| `COM-005` | 指数退避、jitter 和预算基础类型 | `ns_common.retry` | `NOT_STARTED` | P01 |
| `COM-006` | 统一 redaction/sanitizer | `ns_common.security` | `NOT_STARTED` | P01 |
...
- Ubuntu/Linux：标准 asyncio 与 uvloop 双模式，Redis/Valkey standalone 集成测试。
...
| `P01-W03` | `NOT_STARTED` | 建立 runtime 配置组：event_loop、transport、wire_codec、protocol、security、iam、state_store、routing、delivery、worker、pool、tenant_quota、cluster、recovery、observability、logging、debug | 示例配置完整；每组有独立校验和生效模式 |
| `P01-W04` | `NOT_STARTED` | 建立标准 asyncio event loop selector；支持 auto、asyncio、uvloop；auto 在 Linux 优先 uvloop，在 Windows 使用 asyncio；显式 uvloop 不可用时启动失败 | 平台矩阵测试通过；运行中变更被判定为 restart_required |
| `P01-W05` | `NOT_STARTED` | 建立统一 TaskSupervisor，负责命名任务、异常收集、取消顺序、关闭超时和未完成任务报告 | 任务异常不会静默丢失；关闭后无悬挂任务 |
| `P01-W06` | `NOT_STARTED` | 建立 Clock 接口：UTC wall clock、monotonic clock、测试可控 clock | timeout、lease、retry 测试不依赖 sleep 漂移 |
| `P01-W07` | `NOT_STARTED` | 建立 ID 生成和校验规则，覆盖 runtime_id、connection_id、session_id、message_id、summary_id、delivery_id、stream_id、plan_id、operation_id | 格式稳定；无空值；并发生成无冲突 |
| `P01-W08` | `NOT_STARTED` | 建立通用 retry/backoff 类型，覆盖固定、指数、jitter、预算和下一执行时间计算 | 边界值、预算耗尽和确定性测试通过 |
| `P01-W09` | `NOT_STARTED` | 建立统一 sanitizer，按字段名、路径和对象类型脱敏 token、payload、auth_context、capabilities、fencing_token、签名 URL、peer address 和证书摘要 | 日志、错误和审计输入的敏感值全部替换或摘要化 |
...
| 契约编号 | 契约 | 所属阶段 | 状态 |
|---|---|---|---|
| `CFG-1` | NsConfig 不可变快照、兼容入口与配置组 metadata | P01 | `IN_PROGRESS` |
