...
| 当前工作包状态 | `IN_PROGRESS` |
| 当前阶段状态 | `IN_PROGRESS` |
| 最近已验证阶段 | `P04 Transport 抽象与 WebSocket/TCP Adapter` |
| 最近已验证工作包 | `P05-W13 reauth and session expiry policy` |
| 下一阶段 | `P06 IAM、安全上下文与 backend 合同` |
| 当前阻塞项 | 无；当前 WSL 验证使用 `/home/ns/.virtualenvs/ns_runtime` 与 `/home/ns/.virtualenvs/ns_backend` 隔离环境 |
| 设计基线版本 | `0.0.2` |
| wire codec | `json.v1` |
| 当前正式 transport | `websocket_tcp` adapter 已达到 `VERIFIED/F2`；P05 握手前仍不得进入业务 active 状态 |
...
| `src/ns_runtime` 实际能力 | `VERIFIED` | P02 生命周期能力保持；新增 transport-independent `ns_runtime.protocol`：严格 grouped `json.v1` Envelope、raw/normalized 权威分离、资源有界 codec、叠加 schema、1.0 兼容矩阵、50 项内置注册、extension namespace、ERR-1 标准错误、canonical bytes/checksum 与 49 项统一 feature-disabled processor。只有 runtime.error 构造能力 enabled；尚未创建 listener、transport adapter/session、IAM client、StateStore、DeliveryRecord、路由/投递/cluster/management 执行链路。 |
...
| 当前本地测试基线 | `VERIFIED` | P03 专项 `Ran 52, OK`；runtime 环境排除按 DEP-1 不安装的 Django cache 后 P01+P02+P03 联合 `Ran 401, OK (skipped=1)`；backend 环境根目录全量 `Ran 412, OK (skipped=1)`。唯一跳过为 WSL 下 Windows 专用 event-loop policy；两套 `pip check`、两套全树 `compileall` 和 `git diff --check` 通过。 |
| 当前状态校准时间 | `VERIFIED` | `2026-07-20T23:38:07+08:00` |
...
| `P05-W14` | `IN_PROGRESS` | 会话快照异步输出；resume/kick/security close 强审计接口预留 | 普通 heartbeat 不进入强一致链路 |
...
| `SC-1` | SessionContext 和 logical connection | P05 | `IN_PROGRESS` |
...
| P05 | 连接、会话、握手、心跳、Resume | `IN_PROGRESS` | F1 | P04 |
...
|---|---|
| 当前阻塞项 | 无 |
| 下一工作包 | `P05-W14 async safe session snapshot and audit aggregation`（`IN_PROGRESS`） |
