...
> 当前状态校准时间：`2026-07-20T19:11:35+08:00`
...
| 当前本地测试基线 | `VERIFIED` | P02-W08 startup/diagnostic/main 专项 `Ran 35, OK`；runtime 环境非 Django cache 全量 `Ran 345, OK (skipped=1)`，backend 环境根目录全量 `Ran 356, OK (skipped=1)`；唯一跳过为 WSL 下 Windows 专用 event-loop policy 用例。两套 `pip check`、全树 `compileall` 和 `git diff --check` 通过，命令见 acceptance log。 |
| 当前状态校准时间 | `VERIFIED` | `2026-07-20T19:11:35+08:00` |
...
| `RSD-1` | `RuntimeShutdownCoordinator` 与 `RuntimeService` 必须共享同一 `RuntimeContext` 身份；首次 SIGINT/SIGTERM/显式原因胜出并立即关闭本地接入门禁，shutdown 按停止新任务与取消后台任务、flush sink、close sink、close 显式 HTTP owner、写安全有界摘要、close owned logger 的顺序执行。普通资源失败只记录 phase/resource/error type 并继续，进程级异常穿透；超时只摘要化记录未完成任务，重复关闭返回同一冻结报告。P02 不含 transport listener/drain、连接迁移、Envelope 或管理旁路 | P02 | `VERIFIED` |
| `RLO-1` | `RuntimeEventLoopMonitor` 必须与 RuntimeService 共享同一 context，并使用 RSP-1 已选定的 asyncio/uvloop 实现；1 秒单调 deadline 采样不追赶旧 deadline，lag 历史最多 1024 项并以 nearest-rank 计算 P95/P99。冻结 snapshot 覆盖 implementation、lag、slow-threshold observation、pending/cancelled task、executor queue、probe/metric failure；8 个 OBS-1 标准指标仅使用既有有限 attributes。普通 clock/sink/probe 失败不终止 service，未知 count 为 None 且不写失真指标；monitor 由 TaskSupervisor 拥有并在 sink 关闭前取消，不创建 exporter、线程、端口或强一致记录 | P02 | `VERIFIED` |
