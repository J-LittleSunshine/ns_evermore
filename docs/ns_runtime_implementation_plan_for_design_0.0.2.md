...
> 当前状态校准时间：`2026-07-16T21:32:53+08:00`
...
| 最近已验证工作包 | `P01-FIX-04 修复 sanitizer digest 路径资源消耗问题` |
...
| 当前本地测试基线 | `VERIFIED` | 最近一次本地验收：sanitizer 30/30、runtime 联合 131/131、根目录全量 142/142；`compileall`、runtime/backend 两套环境 `pip check` 和 `git diff --check` 通过；历史命令见 acceptance log |
| 当前状态校准时间 | `VERIFIED` | `2026-07-16T21:32:53+08:00` |
...
| 日志与脱敏 | `src/ns_common/security.py`、`src/ns_common/logger.py` | sanitizer 已支持安全字段名/后缀/路径规则、Mapping key 脱敏与稳定冲突键、嵌套映射、dataclass、普通对象、异常 details、完整认证/Cookie 头、URL/自由文本、fail-closed、严格 JSON-safe、循环与深度门禁；digest 使用有界稳定规范化并限制深度、节点、容器、字符串、bytes 和结果字节；peer/client/remote address 直接完全脱敏；logger 尚未接入 sanitizer | Windows `tests/test_security.py` 30/30；runtime 联合 131/131；根测试 142/142 | sanitizer `F2`、logger 接入 `F1` | P01-W09/P01-FIX-03/P01-FIX-04 `VERIFIED`；W10 `NOT_STARTED` |
...
- P01-FIX-04 只修复 sanitizer digest 资源边界，未接入或修改 logger；开始 W10 前必须先将其状态改为 `IN_PROGRESS`。
