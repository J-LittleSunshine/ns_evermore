...
> 当前状态校准时间：`2026-07-17T01:10:53+08:00`
...
| 最近已验证工作包 | `P01-W12 补齐设计文档已明确的 RUNTIME_* 错误覆盖矩阵` |
...
| 当前本地测试基线 | `VERIFIED` | 最近一次本地验收：exceptions 24/24、W12 指定专项联合 155/155、P01/runtime 联合 174/174、根目录全量 185/185；`compileall`、runtime/backend 两套环境 `pip check`、96 项公共导出、71 项精确类型策略、18 域/64 码覆盖矩阵、冷启动导入、循环/内部路径扫描和 `git diff --check` 通过；历史命令见 acceptance log |
| 当前状态校准时间 | `VERIFIED` | `2026-07-17T01:10:53+08:00` |
...
| 公共异常与错误注册 | `src/ns_common/exceptions/` | 71 个异常按显式领域模块登记；冻结 `NsErrorDefinition`、稳定 severity/category、显式不可变注册表和 18 域/64 码 `RUNTIME_*` 覆盖矩阵；13 个 NACK reason 全部映射到细粒度 `RUNTIME_*` code；通用错误保守、精确叶子显式开启副作用策略，查询不做 MRO fallback；原 facade、构造、继承与四字段序列化兼容 | `tests/test_exceptions.py` 24/24；71 项完整策略矩阵及类/code/numeric_code 唯一；覆盖矩阵、NACK 映射、冷启动导入和依赖扫描通过 | `F2` | `VERIFIED`，P01-W11/P01-FIX-06/P01-W12 已完成 |
...
| `ERR-1` | NsErrorDefinition、稳定 severity/category、显式不可变错误注册表、18 域/64 码 `RUNTIME_*` 完整覆盖矩阵、全 `RUNTIME_*` NACK 映射、精确类型保守策略、code/numeric_code/类唯一性与原异常序列化兼容 | P01 | `VERIFIED` |
...
- P01-W12 已完成 18 域/64 码 `RUNTIME_*` 覆盖矩阵和 13 个 NACK reason 的细粒度映射；既有 33 个异常未重编号，原构造、继承和四字段序列化保持兼容。策略 metadata 只作用于精确异常类型且不是无条件执行指令；`safe_detail` 只是策略元数据，实际输出仍必须经过 sanitizer/logger/未来错误 Envelope 边界，见 [ADR-015](ns_runtime_architecture_decisions_0.0.2.md#adr-015)。
