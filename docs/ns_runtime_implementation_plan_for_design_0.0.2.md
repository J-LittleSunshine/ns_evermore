...
| 最近已验证工作包 | `P03-FIX-01 阶段出口严格类型与深度口径校准` |
...
| 当前本地测试基线 | `VERIFIED` | P03 专项 `Ran 49, OK`；runtime 环境排除按 DEP-1 不安装的 Django cache 后 P01+P02+P03 联合 `Ran 398, OK (skipped=1)`；backend 环境根目录全量 `Ran 409, OK (skipped=1)`。唯一跳过为 WSL 下 Windows 专用 event-loop policy；两套 `pip check`、两套全树 `compileall` 和 `git diff --check` 通过。 |
| 当前状态校准时间 | `VERIFIED` | `2026-07-20T21:36:57+08:00` |
...
验收记录：`P03-W01` 至 `P03-W11`、`P03-FIX-01` 与阶段出口见 [acceptance log](ns_runtime_acceptance_log_0.0.2.md#p03-w01)；完整协议边界登记为 `ENV-1`，长期决策见 [ADR-028](ns_runtime_architecture_decisions_0.0.2.md#adr-028)。
