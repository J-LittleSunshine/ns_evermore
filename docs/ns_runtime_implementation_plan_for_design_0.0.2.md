...
| 最近已验证工作包 | `P04-FIX-01 缺失 websocket 依赖时的错误归一化隔离` |
...
阶段出口验收见 [acceptance log](ns_runtime_acceptance_log_0.0.2.md#p04)，`P04-W01` 至 `P04-W10` 与 `P04-FIX-01` 已达到 `VERIFIED/F2`；游标只推进到 `P05-W01 NOT_STARTED`，未实施 P05。
...
| `TC-1` | 22 项有限闭集 transport adapter conformance；所有 adapter 复用公共 harness，并提供 TLS/plaintext policy、完整文本边界、binary/invalid UTF-8/size、每-session queue/backpressure、keepalive、close/error、并发/cancel、admission/drain/shutdown、禁用依赖隔离、安全诊断及 send success 不产生 runtime ACK 的专项证据 | P04 | `VERIFIED` |
