...
> 当前状态校准时间：`2026-07-20T19:48:39+08:00`
...
| 字段 | 当前值 |
|---|---|
| 当前阶段 | `P03 Envelope 协议层与类型注册表` |
| 当前工作包 | `P03-W11 FeatureDisabledProcessor` |
| 当前工作包状态 | `VERIFIED` |
| 当前阶段状态 | `IN_PROGRESS` |
| 最近已验证阶段 | `P02 Runtime 进程生命周期与事件循环` |
| 最近已验证工作包 | `P02-FIX-05 critical background task 生命周期联动` |
| 下一阶段 | `P04 Transport 抽象与 WebSocket/TCP Adapter` |
...
| `src/ns_runtime` 实际能力 | `VERIFIED` | 已建立独立 package、保持冷导入无副作用的唯一 `main.py`、六态一次性 `RuntimeService`、冻结显式 `RuntimeContext`、同步 startup preflight、只读初始角色/健康/能力快照、SIGINT/SIGTERM 共用的本地优雅关闭编排，以及 TaskSupervisor 承载的有界 event-loop monitor。`python -m ns_runtime.main diagnose` 复用唯一入口，只读检查显式配置、本地依赖/TLS/event-loop 选择与目录状态，输出脱敏 JSON 和稳定退出码，不准备目录、不安装 policy、不启动 service/monitor。transport、cluster_coordination、delivery 仍统一审计拒绝，active_master 配置值不授予协调权。尚未创建 listener、transport adapter/session、StateStore、HTTP client/exporter、协议或消息链路。 |
...
| 当前本地测试基线 | `VERIFIED` | P02-FIX-04/FIX-05 shutdown/monitor/service/main 专项 `Ran 49, OK`；runtime 环境非 Django cache 全量 `Ran 349, OK (skipped=1)`，backend 环境根目录全量 `Ran 360, OK (skipped=1)`；唯一跳过为 WSL 下 Windows 专用 event-loop policy 用例。两套 `pip check`、全树 `compileall` 和 `git diff --check` 通过，命令见 acceptance log。 |
| 当前状态校准时间 | `VERIFIED` | `2026-07-20T19:48:39+08:00` |
...
验收记录：`P03-W01`、`P03-W02` 见 [acceptance log](ns_runtime_acceptance_log_0.0.2.md#p03-w01)；分组与入站权威边界登记为 `ENV-1`，长期决策见 [ADR-028](ns_runtime_architecture_decisions_0.0.2.md#adr-028)。
...
| `ENV-1` | `ns_runtime.protocol` 的 transport-independent 分组 Envelope：固定 protocol/message/source/target/route/delivery/stream/auth_context/payload/callback/trace/extensions 顶层集合，protocol/message 必需，不适用分组省略；冻结类型拒绝顶层与核心分组未知字段、null/空占位和非 JSON 动态值，错误 detail 不回显输入值 | P03 | `IN_PROGRESS`（W01 分组模型已验证；inbound、限制、registry、error、canonical 与禁用 processor 待后续 W 完成） |
...
| P03 | Envelope 协议层与类型注册表 | `IN_PROGRESS` | F1 | P02 |
