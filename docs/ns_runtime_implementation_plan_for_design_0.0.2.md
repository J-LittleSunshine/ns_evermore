...
| `src/ns_common` 实际能力 | `VERIFIED` | 存在配置、路径、异步生命周期、UTC/单调/可控时钟、统一标识符、固定/指数/jitter 退避、不可变 retry 预算、日志、HTTP、普通 cache 与部分 runtime 错误类；缺少 P01 要求的 sanitizer、observability、testing 等公共能力 |
...
| 重试与退避基础设施 | `src/ns_common/retry.py` | 可插拔 BackoffStrategy；固定、指数封顶和确定性对称 jitter；默认 5 次的不可变 RetryBudget；显式 Clock 下同时计算 UTC next_retry_at 与 monotonic deadline；delivery retry 序号与 message 共享预算消耗解耦 | Windows `tests/test_retry.py` 14/14；WSL 14/14；runtime 联合 96/96；根测试回归 107/107 | `F2` | `VERIFIED`，P01-W08 已完成 |
