...
> 当前状态校准时间：`2026-07-17T18:58:08+08:00`
...
| 最近已验证工作包 | `P01-W17 建立 runtime 独立生产/测试依赖清单` |
...
| `src/ns_common` 实际能力 | `VERIFIED` | 存在配置、路径、异步生命周期、UTC/单调/可控时钟、统一标识符、固定/指数/jitter 退避、不可变 retry 预算、统一 sanitizer、日志、HTTP、普通 cache、完整 `RUNTIME_*` 错误覆盖矩阵、具备关闭故障隔离/35 项有限 metric definition/完整 record 字节门禁的 metrics/trace/diagnostic sink，以及实例隔离的临时目录/配置/可控 clock/内存 sink/保留端口/真实 Redis namespace 测试工厂；可选多进程日志依赖与 cache logger 均按实际使用延迟加载 |
...
| 当前本地测试基线 | `VERIFIED` | 最近一次本地验收：依赖清单门禁 7/7；P01/runtime 联合 `Ran 241, OK (skipped=1)`；backend 环境根目录全量 `Ran 252, OK (skipped=1)`，跳过项均为 WSL 下同一 Windows 专用用例；runtime/backend 生产、runtime 测试与压测清单均在仓库外全新环境完成安装和 `pip check`，Redis/Valkey 同步/异步真实 standalone 驱动隔离通过；`compileall`、冷启动依赖隔离、临时目录/进程残留、生产源码测试文件、仓库虚拟环境和 `git diff --check` 门禁通过，历史命令见 acceptance log |
| 当前状态校准时间 | `VERIFIED` | `2026-07-17T18:58:08+08:00` |
...
| 可观测与测试基础 | `src/ns_common/logger.py`、`src/ns_common/observability.py`、`src/ns_common/testing.py` | 已有日志、不可变脱敏 metrics/trace/diagnostic 记录、显式 sink 协议、关闭后原子拒绝与 dropped/rejected 分离、35 项有限 metric definition、完整公开 record 字节门禁，以及实例隔离的临时目录/配置/clock/sink/保留端口和同步/异步 Redis namespace 工厂；测试驱动和 asyncio-safe 压测工具与生产依赖分层；尚无 runtime 采集器、外部 exporter 或故障注入框架 | observability 23/23、testing 20/20（含真实 Redis）；依赖门禁 7/7；P01/runtime 联合 241 项；backend 根回归 252 项；真实 Redis/Valkey 驱动与隔离安装通过 | 公共观测与测试基础 `F2`；runtime 观测 `F0` | `VERIFIED`，P01-W15/P01-FIX-08/P01-W16/P01-W17 已完成，完整 runtime 观测与故障注入待 P02/P20 |
...
| `COM-015` | 测试资源工厂、临时配置和真实依赖管理 | `ns_common.testing` | `VERIFIED`（实例级临时目录/配置/clock/sink、绑定端口 reservation、同步/异步 Redis namespace、真实 Redis/Valkey 驱动、五层依赖隔离与 standalone 清理验证） | P01 |
...
| `P01-W16` | `VERIFIED` | 建立 `ns_common.testing` 测试工厂，统一临时目录、临时配置、可控 clock、内存 sink、随机端口和真实 Redis namespace | 测试之间无状态污染 |
...
| `TST-1` | `NsTestResourceFactory` 实例级临时目录/显式不可变配置/可控 clock/内存 sink；保持绑定的 OS 随机 TCP 端口；唯一 Redis namespace 的同步/异步 `SCAN` + 批量 `DELETE` 托管清理且禁止数据库级 flush | P01 | `VERIFIED` |
