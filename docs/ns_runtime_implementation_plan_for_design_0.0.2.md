...
| 当前本地测试基线 | `VERIFIED` | P07 专项 `Ran 14, OK`；P07 与 connection/session composition 联合 `Ran 60, OK`；runtime 边界全量 `Ran 679, OK (skipped=1)`；backend 全量 `Ran 690, OK (skipped=49)`。全树 `compileall`、两环境 `pip check`、冷导入、`git diff --check`、P08/可靠投递、第二 owner、远程插件、global runtime object 与敏感事件/审计字段扫描通过。 |
...
| Processor 与插件 | `src/ns_runtime/processor/`、`src/ns_runtime/connection/processors.py` | PC-1 normalized context 和显式依赖；安全、授权、限流接口、幂等接口、审计标记、路由接口、message、response 固定八阶段；实例 registry 按 type/stage/version/feature flag 冲突检测；existing lifecycle/disabled processor adapter；typed final audit、EventBus 与仅本地 allowlist plugin metadata/namespace 权限边界。P08 strong storage、真实 limiter/idempotency/routing、远程 plugin runtime 均不存在 | P07 专项 14/14；connection/session/composition 联合 60/60；runtime 679/679；backend 690/690 | `F3` | P07 `VERIFIED`；PC-1 冻结 |
