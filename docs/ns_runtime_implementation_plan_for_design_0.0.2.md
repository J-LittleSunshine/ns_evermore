...
> 当前状态校准时间：`2026-07-16T20:29:12+08:00`
...
| 最近已验证工作包 | `P01-FIX-03 修复 P01-W09 sanitizer 泄露和异常安全问题` |
...
| 当前工作区修改 | `VERIFIED` | 文档拆分开始前已有未提交的 P01-FIX-03 修改：本实施计划、`src/ns_common/security.py`、`tests/test_security.py`；本次新增 acceptance log 与 architecture decisions，并继续修改本实施计划；设计边界文档未修改 |
...
| 当前本地测试基线 | `VERIFIED` | 最近一次本地验收：sanitizer 22/22、runtime 联合 123/123、根目录全量 134/134；`compileall`、runtime/backend 两套环境 `pip check` 和 `git diff --check` 通过；历史命令见 acceptance log |
| 当前状态校准时间 | `VERIFIED` | `2026-07-16T20:29:12+08:00` |
...
| 日志与脱敏 | `src/ns_common/security.py`、`src/ns_common/logger.py` | sanitizer 已支持安全字段名/后缀/路径规则、Mapping key 脱敏与稳定冲突键、嵌套映射、dataclass、普通对象、异常 details、完整认证/Cookie 头、URL/自由文本、fail-closed、严格 JSON-safe、循环与深度门禁；peer/client/remote address 直接完全脱敏；logger 仍为 JSON/text/彩色 formatter、轮转与全局 map，尚未接入 sanitizer | Windows `tests/test_security.py` 22/22；runtime 联合 123/123；根测试 134/134 | sanitizer `F2`、logger 接入 `F1` | P01-W09/P01-FIX-03 `VERIFIED`；W10 `NOT_STARTED` |
...
| `SEC-1` | Sanitizer 字段/路径/对象规则、URL/文本脱敏、Mapping key、fail-closed 与严格 JSON-safe | P01 | `VERIFIED` |
...
| 当前工作区 | 保留 P01-FIX-03 的 `security.py`、`test_security.py` 和本计划未提交修改；本次文档拆分新增 acceptance log 与 architecture decisions；设计边界文档未修改 |

已知限制：

- sanitizer 仍未接入 `NsLogger` 的 extra/exception 输出；任意无字段名、无路径且无敏感标记的自由文本仍要求调用方提供结构化语义。验收历史见 [acceptance log / P01-FIX-03](ns_runtime_acceptance_log_0.0.2.md#p01-fix-03)，长期规则见 [ADR-009](ns_runtime_architecture_decisions_0.0.2.md#adr-009)。
...
- P01-W10 至 P01-W17 尚未完成；`src/ns_runtime`、强一致 StateStore、Envelope、transport、session、delivery、cluster 和 management 仍按后续阶段保持禁用或 F0。
- 本次结构拆分不改变任何工作包状态，也不开始功能开发。开始 W10 前必须先将其状态改为 `IN_PROGRESS`。
