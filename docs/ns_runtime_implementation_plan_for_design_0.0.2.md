...
| `src/ns_common` 实际能力 | `VERIFIED` | 存在配置、路径、异步生命周期、UTC/单调/可控时钟、日志、HTTP、普通 cache 与部分 runtime 错误类；缺少 P01 要求的 ID、retry、sanitizer、observability、testing 等公共能力 |
...
|---|---|---|---|---|---|
| 配置基础设施 | `src/ns_common/config.py` | backend/cache/log/runtime 总组及 17 个强类型 runtime 细分组的深度不可变快照、独立 metadata/生效模式、严格加载/序列化、兼容别名、固定来源优先级、backend 覆盖和已验证快照恢复已完成；runtime 集群静态启动角色固定为 singleton、sub_node、standby_master、active_master | `tests/test_config.py` 40/40；根测试回归 85/85 | `F2` | `VERIFIED`，P01-W01/W02/W03 与 P01-FIX-01 已完成 |
| 路径与生命周期 | `src/ns_common/paths.py`、`src/ns_common/async_runtime.py` | 仓库固定路径常量与目录创建；event loop selector 已支持 auto/asyncio/uvloop、Windows/Linux 策略、可选依赖失败和 restart_required 门禁；TaskSupervisor 已支持命名任务、异常收集、分组有序取消、全局关闭超时和未完成任务报告 | Windows `tests.test_async_runtime` 23/23；WSL 22 通过/1 Windows 专用跳过且真实 uvloop loop 已验证；根测试回归 85/85 | selector、TaskSupervisor `F2` | P01-W04/W05 `VERIFIED` |
| 时间基础设施 | `src/ns_common/time.py` | 显式 Clock 协议；SystemClock 提供 UTC wall clock、monotonic 和异步 sleep；ControlledClock 支持确定性 UTC/单调推进、并发 deadline 唤醒、取消清理和跨 loop 门禁 | Windows `tests/test_time.py` 11/11；WSL 11/11；根测试回归 85/85 | `F2` | `VERIFIED`，P01-W06 已完成 |
