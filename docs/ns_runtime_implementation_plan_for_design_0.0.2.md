...
> 当前状态校准时间：`2026-07-17T19:49:10+08:00`
...
| 当前工作包 | `P02-W01 建立 src/ns_runtime 独立组件和唯一进程入口` |
| 当前工作包状态 | `NOT_STARTED` |
| 当前阶段状态 | `NOT_STARTED` |
| 最近已验证阶段 | `P01 ns_common 公共基础设施加固`（2026-07-17） |
| 最近已验证工作包 | `P01-FIX-09 修复测试资源 Factory 的可重试关闭和并发关闭语义` |
...
| 当前本地测试基线 | `VERIFIED` | 最近一次本地验收：testing 专项 28/28、依赖清单门禁 7/7；P01/runtime 联合 `Ran 249, OK (skipped=1)`；backend 环境根目录全量 `Ran 260, OK (skipped=1)`，跳过项均为 WSL 下同一 Windows 专用用例；五份依赖清单内容未改变，既有四类仓库外全新环境安装证据保持有效，runtime/backend 持久环境 `pip check` 和真实 standalone Redis namespace 回归通过；`compileall`、facade/对象身份、冷启动依赖隔离、临时目录/测试进程残留、生产源码测试文件、仓库虚拟环境和 `git diff --check` 门禁通过，命令见 acceptance log |
| 当前状态校准时间 | `VERIFIED` | `2026-07-17T19:49:10+08:00` |
...
| 可观测与测试基础 | `src/ns_common/logger.py`、`src/ns_common/observability.py`、`src/ns_common/testing.py` | 已有日志、不可变脱敏 metrics/trace/diagnostic 记录、显式 sink 协议、关闭后原子拒绝与 dropped/rejected 分离、35 项有限 metric definition、完整公开 record 字节门禁，以及实例隔离、OPEN/CLOSING/CLOSED 状态、失败所有权保留和并发关闭屏障的临时目录/配置/clock/sink/保留端口与同步/异步 Redis namespace 工厂；测试驱动和 asyncio-safe 压测工具与生产依赖分层；尚无 runtime 采集器、外部 exporter 或故障注入框架 | observability 23/23、testing 28/28（含真实 Redis）；依赖门禁 7/7；P01/runtime 联合 249 项；backend 根回归 260 项；真实 Redis/Valkey 驱动与隔离安装通过 | 公共观测与测试基础 `F2`；runtime 观测 `F0` | `VERIFIED`，P01-W15/P01-FIX-08/P01-W16/P01-FIX-09/P01-W17 已完成，完整 runtime 观测与故障注入待 P02/P20 |
...
| `COM-015` | 测试资源工厂、临时配置和真实依赖管理 | `ns_common.testing` | `VERIFIED`（实例级目录/配置/clock/sink、绑定端口、同步/异步 namespace、OPEN/CLOSING/CLOSED 可重试并发关闭屏障、真实 Redis/Valkey 驱动与五层依赖隔离均已验证） | P01 |
...
| `P01-W16` | `VERIFIED` | 建立 `ns_common.testing` 测试工厂，统一临时目录、临时配置、可控 clock、内存 sink、随机端口和真实 Redis namespace | 测试之间无状态污染；FIX-09 已补齐可重试关闭与并发关闭屏障 |
| `P01-FIX-09` | `VERIFIED` | 修复测试资源 Factory 的可重试关闭和并发关闭语义；以 OPEN → CLOSING → CLOSED 显式状态保留未完成资源所有权 | 失败资源可重试；并发 close 形成关闭屏障；创建门禁、异常边界和 W16/W17/P01/backend 回归通过 |
...
| `TST-1` | `NsTestResourceFactory` 实例级临时目录/显式不可变配置/可控 clock/内存 sink；保持绑定的 OS 随机 TCP 端口；唯一 Redis namespace 的同步/异步 `SCAN` + 批量 `DELETE` 托管清理且禁止数据库级 flush；Factory 使用 OPEN → CLOSING → CLOSED 生命周期、可重试所有权和并发关闭屏障 | P01 | `VERIFIED` |
...
|---|---|
| 当前阻塞项 | 无 |
| 下一工作包 | `P02-W01 建立 src/ns_runtime 独立组件和唯一进程入口`（`NOT_STARTED`） |
...
- P01-W16/P01-FIX-09 已冻结实例隔离的 `ns_common.testing` 工厂、OPEN/CLOSING/CLOSED 可重试并发关闭屏障、绑定端口 reservation 和仅清理唯一前缀的 Redis namespace，见 [ADR-019](ns_runtime_architecture_decisions_0.0.2.md#adr-019)。Factory 不拥有 Redis client 或服务进程；同步 close 会等待当前底层清理调用返回。P01-W17 已建立 common/backend/runtime 生产、runtime 测试和 runtime 压测五层依赖清单，Redis/Valkey 驱动当前只属于测试层，pyperf/psutil 只属于压测层，QUIC/WebTransport 依赖继续留给 P21；多进程日志依赖和 cache logger 延迟到实际使用，见 [ADR-020](ns_runtime_architecture_decisions_0.0.2.md#adr-020)。当前真实依赖验收仍只证明 standalone namespace 与驱动隔离，不证明强一致 StateStore、Sentinel/Cluster、生产 Redis 或性能基线。`src/ns_runtime`、Envelope、transport、session、delivery、cluster 和 management 仍按后续阶段保持禁用或 F0。
