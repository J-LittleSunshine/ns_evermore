...
> 当前状态校准时间：`2026-07-20T09:07:26+08:00`
...
| 最近已验证工作包 | `P02-W03 建立显式 RuntimeContext` |
...
| 当前本地测试基线 | `VERIFIED` | 最近一次本地验收：P02-W03 RuntimeContext 专项 8/8，W01 入口加 W02/FIX-01 生命周期及 W03 context 联合 32/32；P01/runtime 加 P02 联合回归 `Ran 288, OK (skipped=1)`；backend 环境根目录全量 `Ran 299, OK (skipped=1)`，跳过项均为 WSL 下同一 Windows 专用用例；五份依赖清单内容未改变，runtime/backend 持久环境 `pip check` 通过；`compileall`、package/context 冷导入、无 ambient locator/import-time task/thread、根目录/外部目录入口、范围隔离、生产源码测试文件、仓库虚拟环境和 `git diff --check` 门禁通过，命令见 acceptance log |
| 当前状态校准时间 | `VERIFIED` | `2026-07-20T09:07:26+08:00` |
...
| `RTC-1` | `ns_runtime.context.RuntimeContext` 的冻结关键字构造与显式依赖身份：NsConfig、Clock、Logger、MetricsSink、TraceSink、TaskSupervisor 为必需依赖；有限冻结 `RuntimeDependencySlots` 只为 DiagnosticSnapshotSink 与 NsHttpClientOwner 提供可选类型化槽位；RuntimeService 必须构造注入同一 context；无默认依赖创建、全局 context、字符串 registry 或 ambient getter | P02 | `VERIFIED` |
...
- P02-W03 已冻结 `RTC-1`：context 只冻结依赖引用与类型边界，不冻结依赖对象内部状态，也不等于 composition root 或资源生命周期 owner；可选 diagnostic/HTTP 槽位只保留显式接线位置，不创建 client/exporter。`main.py` 仍未创建或运行 service，启动前校验、角色、信号驱动关闭编排和 loop 观测分别留给 P02-W04 至 W08，见 [ADR-022](ns_runtime_architecture_decisions_0.0.2.md#adr-022)。
