...
> 当前状态校准时间：`2026-07-17T17:33:12+08:00`
...
| 当前工作包 | `P01-W17 建立 runtime 独立生产/测试依赖清单` |
| 当前工作包状态 | `NOT_STARTED` |
| 当前阶段状态 | `IN_PROGRESS` |
| 最近已验证阶段 | `P00 本地仓库基线与实施账本`（2026-07-16） |
| 最近已验证工作包 | `P01-W16 建立 ns_common.testing 测试工厂` |
...
| `src/ns_common` 实际能力 | `VERIFIED` | 存在配置、路径、异步生命周期、UTC/单调/可控时钟、统一标识符、固定/指数/jitter 退避、不可变 retry 预算、统一 sanitizer、日志、HTTP、普通 cache、完整 `RUNTIME_*` 错误覆盖矩阵、具备关闭故障隔离/35 项有限 metric definition/完整 record 字节门禁的 metrics/trace/diagnostic sink，以及实例隔离的临时目录/配置/可控 clock/内存 sink/保留端口/真实 Redis namespace 测试工厂 |
...
| 当前本地测试基线 | `VERIFIED` | 最近一次本地验收：testing 专项 20/20（含真实 Redis standalone）；P01/runtime 联合 `Ran 233, OK (skipped=1)`；backend 环境根目录全量 `Ran 244, OK (skipped=1)`，跳过项均为 WSL 下同一 Windows 专用用例；`compileall`、runtime/backend 两套环境 `pip check`、204 项 common/14 项 testing facade 唯一导出与对象身份、冷启动与依赖扫描、临时目录/进程残留、生产源码测试文件、仓库虚拟环境和 `git diff --check` 门禁通过；历史命令见 acceptance log |
| 当前状态校准时间 | `VERIFIED` | `2026-07-17T17:33:12+08:00` |
...
| 可观测与测试基础 | `src/ns_common/logger.py`、`src/ns_common/observability.py`、`src/ns_common/testing.py` | 已有日志、不可变脱敏 metrics/trace/diagnostic 记录、显式 sink 协议、关闭后原子拒绝与 dropped/rejected 分离、35 项有限 metric definition、完整公开 record 字节门禁，以及实例隔离的临时目录/配置/clock/sink/保留端口和同步/异步 Redis namespace 工厂；尚无 runtime 采集器、外部 exporter 或故障注入框架 | observability 23/23、testing 20/20（含真实 Redis）；P01/runtime 联合 233 项；backend 根回归 244 项；静态/环境门禁通过 | 公共观测与测试基础 `F2`；runtime 观测 `F0` | `VERIFIED`，P01-W15/P01-FIX-08/P01-W16 已完成，完整 runtime 观测与故障注入待 P02/P20 |
...
| `COM-015` | 测试资源工厂、临时配置和真实依赖管理 | `ns_common.testing` | `VERIFIED`（实例级临时目录/配置/clock/sink、绑定端口 reservation、同步/异步 Redis namespace 与真实 standalone 清理验证） | P01 |
...
|---|---|
| 当前阻塞项 | 无 |
| 下一工作包 | `P01-W17 建立 runtime 独立生产/测试依赖清单`（`NOT_STARTED`） |
