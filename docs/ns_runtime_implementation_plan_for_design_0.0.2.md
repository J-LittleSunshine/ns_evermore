...
> 当前状态校准时间：`2026-07-20T18:29:15+08:00`
...
| 当前工作包 | `P02-W06 SIGINT/SIGTERM 优雅关闭编排` |
| 当前工作包状态 | `NOT_STARTED` |
| 当前阶段状态 | `IN_PROGRESS` |
| 最近已验证阶段 | `P01 ns_common 公共基础设施加固` |
| 最近已验证工作包 | `P02-W05 初始角色状态与未完成能力门禁` |
...
| `src/ns_runtime` 实际能力 | `VERIFIED` | 已建立独立 package、保持冷导入无副作用的唯一 `main.py`、六态一次性 `RuntimeService`、冻结显式 `RuntimeContext`、同步 startup preflight，以及只读初始角色/健康/能力快照。singleton、sub_node、standby_master、active_master 四个配置角色均可装载；transitioning/draining 与健康域保持分离，但尚无角色或健康迁移 API。transport、cluster_coordination、delivery 均由统一 `RUNTIME_FEATURE_DISABLED` 门禁审计后拒绝，active_master 配置值不授予选主、lease、fencing 或协调写权限。preflight 成功后 composition root 使用显式配置和显式 log 目录构造带 Sanitizer 的 `NsLogger`，失败前仍无 global config、目录、policy 或 service 副作用。尚未创建 listener、transport adapter/session、StateStore、HTTP client/exporter、信号关闭编排、loop lag collector、协议或消息链路。 |
...
| 当前本地测试基线 | `VERIFIED` | P02-W05 角色/能力门禁、service、main、logger、exceptions 专项 `Ran 77, OK`；runtime 环境非 Django cache 全量 `Ran 324, OK (skipped=1)`，backend 环境根目录全量 `Ran 335, OK (skipped=1)`；唯一跳过为 WSL 下 Windows 专用 event-loop policy 用例。显式 NsLogger 冷进程验证不初始化 global config、只写显式 log root 且 token/payload 零泄露；既有 dependency/security 冷进程零副作用回归保持通过。两套 `pip check`、`compileall` 和 `git diff --check` 通过，命令见 acceptance log。 |
| 当前状态校准时间 | `VERIFIED` | `2026-07-20T18:29:15+08:00` |
...
| runtime 进程与事件循环 | `src/ns_runtime/__init__.py`、`src/ns_runtime/main.py`、`src/ns_runtime/context.py`、`src/ns_runtime/service.py`、`src/ns_runtime/startup.py` | package/入口冷导入无启动副作用；`RuntimeServiceState` 六态一次性生命周期和显式 `RuntimeContext` 保持冻结；startup preflight 在 event-loop policy 与任何未来 listener 前完成严格环境、配置、生产 TLS/StateStore、transport feature gate、依赖和目录校验，返回冻结安全摘要；入口在 runtime 依赖齐全时使用配置 policy 运行一次无监听 service 生命周期，依赖缺失稳定拒绝 | startup 14/14，P02 联合 51/51；runtime 联合 307 项、backend 根回归 318 项，均仅 1 个 Windows 专用跳过 | 进程入口、composition root 与 startup preflight `F2`；service/context `F2`；角色、信号关闭与 loop 观测 `F0` | P02-W01 至 W04（含 FIX）`VERIFIED`；P02 `IN_PROGRESS` |
...
| `LOG-1` | NsLogger 单向注入/拥有 Sanitizer；JSON/text/color 在输出前处理消息、格式参数、extra、异常和安全 traceback 元数据，不直接序列化原始对象；调用方 extra 不得覆盖权威日志字段，冲突值只进入 `extra_fields`；可选显式配置快照和 log root 不读取 global `ns_config`，未提供时保持原兼容入口；可选多进程 handler 只在实际使用时加载 | P01/P02 | `VERIFIED` |
...
|---|---|
| 当前阻塞项 | 无 |
| 下一工作包 | `P02-W06 SIGINT/SIGTERM 优雅关闭顺序与资源清理编排`（`NOT_STARTED`） |
...
- P02-W02/P02-FIX-01 已冻结 `RuntimeService` 六态一次性生命周期、稳定非法迁移错误、stopped 后 stop 幂等、failed 后显式清理/重试、hook 原异常穿透不保存，以及首次 loop owner 原子绑定，见 [ADR-021](ns_runtime_architecture_decisions_0.0.2.md#adr-021)。RuntimeService 现已由 P02-W03 强制构造注入同一显式 context，但 start/stop hook 仍不创建、启动、flush 或关闭任何依赖；W06 不得推翻 stop 幂等与 failed 可清理语义。
