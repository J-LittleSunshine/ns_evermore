...
> 当前状态校准时间：`2026-07-20T18:56:04+08:00`
...
| 字段 | 当前值 |
|---|---|
| 当前阶段 | `P02 Runtime 进程生命周期与事件循环` |
| 当前工作包 | `P02-W08 本地进程诊断命令` |
| 当前工作包状态 | `IN_PROGRESS` |
| 当前阶段状态 | `IN_PROGRESS` |
| 最近已验证阶段 | `P01 ns_common 公共基础设施加固` |
| 最近已验证工作包 | `P02-W07 event loop lag 与 implementation 指标` |
| 下一阶段 | `P03 Envelope 协议层与类型注册表` |
...
| `src/ns_runtime` 实际能力 | `VERIFIED` | 已建立独立 package、保持冷导入无副作用的唯一 `main.py`、六态一次性 `RuntimeService`、冻结显式 `RuntimeContext`、同步 startup preflight、只读初始角色/健康/能力快照、SIGINT/SIGTERM 共用的本地优雅关闭编排，以及 TaskSupervisor 承载的有界 event-loop monitor。monitor 从 preflight 接收实际 asyncio/uvloop 实现，输出 lag/current/P95/P99、slow observation、pending/cancelled task、executor queue 与 probe/metric failure 的冻结内部 snapshot，并通过既有 OBS-1 写入 8 个低基数标准指标；普通观测失败不终止 service。transport、cluster_coordination、delivery 仍统一审计拒绝，active_master 配置值不授予协调权。尚未创建 listener、transport adapter/session、StateStore、HTTP client/exporter、本地诊断命令、协议或消息链路。 |
...
| 当前本地测试基线 | `VERIFIED` | P02-W07 monitor 专项 6/6，最终 lifecycle/main 联合 `Ran 45, OK`；runtime 环境非 Django cache 全量 `Ran 336, OK (skipped=1)`，backend 环境根目录全量 `Ran 347, OK (skipped=1)`；唯一跳过为 WSL 下 Windows 专用 event-loop policy 用例。runtime 隔离环境另以真实 uvloop loop 验证 8 个标准指标、实现标签和 shutdown 取消。两套 `pip check`、`compileall` 和 `git diff --check` 通过，命令见 acceptance log。 |
| 当前状态校准时间 | `VERIFIED` | `2026-07-20T18:56:04+08:00` |
...
| runtime 进程与事件循环 | `src/ns_runtime/__init__.py`、`src/ns_runtime/main.py`、`src/ns_runtime/context.py`、`src/ns_runtime/service.py`、`src/ns_runtime/startup.py`、`src/ns_runtime/shutdown.py`、`src/ns_runtime/event_loop_observability.py` | package/入口冷导入无启动副作用；startup preflight、RuntimeService、RuntimeShutdownCoordinator 保持同一显式 context；RuntimeEventLoopMonitor 使用 preflight 选择结果、现有配置阈值、TaskSupervisor 与 MetricsSink，维护 1024 项有界 lag 窗口和冻结 snapshot，写入 8 个既有低基数标准指标；普通 metric/clock/probe 失败 fail-soft 且未知值不伪装为零，shutdown 先取消 monitor 再关闭 sinks | monitor 6/6，最终 lifecycle/main 45/45；runtime 联合 336 项、backend 根回归 347 项，均仅 1 个 Windows 专用跳过；真实 uvloop 独立验证通过 | 进程入口、composition root、startup、service/context、角色、信号关闭与 loop 观测 `F2`；本地诊断 `F0` | P02-W01 至 W07（含 FIX）`VERIFIED`；P02 `IN_PROGRESS` |
...
| `P02-W08` | `IN_PROGRESS` | 建立本地进程诊断命令，只读取启动配置和本地状态，不开 HTTP 管理端口 | 可判断配置是否合法和进程依赖是否齐全 |
...
| `RSP-1` | `ns_runtime._bootstrap` 从权威模块重导出默认配置路径、唯一 `NsConfig` 类型、startup 错误与路径常量，不初始化 global `ns_config`；`load_config_snapshot()` 强制显式路径并与 `validate()`/`prepare()` 共用唯一安全错误归一化，普通配置错误保持 CFG-1 类型。`validate()` 不替换 policy，`prepare()` 只在环境、配置、安全、transport、依赖、TLS 和显式目录通过后安装 policy；当前只接纳 `websocket_tcp` 并检查 `websockets`，不创建 listener/adapter/StateStore/client/exporter。`ns_common(.config).ns_config` 仍在显式访问时返回同一真实 `NsConfig`，无参 load 语义不变；冷进程 `main()` 失败前不初始化该全局、不 ensure 目录，成功后才运行无监听 RuntimeService | P02 | `VERIFIED` |
...
| P02 | Runtime 进程生命周期与事件循环 | `IN_PROGRESS` | F1 | P01 |
...
|---|---|
| 当前阻塞项 | 无 |
| 下一工作包 | `P02-W08 本地进程诊断命令`（`IN_PROGRESS`） |
