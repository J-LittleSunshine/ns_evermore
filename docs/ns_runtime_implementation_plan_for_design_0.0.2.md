...
12. 每个 W 工作包完成边界 review 并提交到持续使用的同一 `codex` 分支后，立即推送该分支到远端；不得为每个工作包创建新分支。远端内容仍不得反向作为本地实现状态依据。
...
| 字段 | 当前值 |
|---|---|
| 当前阶段 | `P11 本地可靠投递调度与发送` |
| 当前工作包 | `P11-W01` |
| 当前工作包状态 | `BLOCKED`（等待独立授权） |
| 当前阶段状态 | `BLOCKED / F0` |
| 最近已验证阶段 | `P10 受理、Summary、去重与 Payload Reference`（`F3 / local only`） |
| 最近已验证工作包 | `P10-FIX-02 Redis StateStore trusted-boundary、lifecycle 与 recovery closure` |
| 下一阶段 | `P11 本地可靠投递调度与发送`（`P11-W01`，`BLOCKED`，等待独立授权） |
| 当前阻塞项 | P11-W01未获授权；task.dispatch production feature继续关闭，直到P12 ACK闭环完成 |
| 设计基线版本 | `0.0.2` |
| wire codec | `json.v1` |
...
| 生产状态存储目标 | P08 contract保持冻结；P10-FIX-02关闭Redis standalone provider的trusted-boundary、BaseException lifecycle、端口reservation与跨provider recovery。Redis standalone provider `VERIFIED`；Valkey driver compatibility `IMPLEMENTED`，Valkey server integration `UNVERIFIED`。普通cache/transient memory仍不能替代；Sentinel/Cluster、TLS材料、lease/fencing继续deferred。 |
...
| 基线字段 | 状态 | 本地结果 |
|---|---|---|
| 本地仓库根目录 | `VERIFIED` | 当前 WSL 路径 `/mnt/s/pythonproject/ns/ns_evermore`，对应 Windows 路径 `S:\PythonProject\ns\ns_evermore` |
| 当前分支状态 | `VERIFIED` | `codex/ns-runtime-implementation`；该分支持续承载后续阶段，不为每阶段新建分支；未读取远程内容或提交历史 |
...
| Python 版本与虚拟环境 | `VERIFIED` | 当前 Ubuntu 22.04.5 LTS / WSL2 使用 Python `3.10.12`；runtime 环境为 `/home/ns/.virtualenvs/ns_runtime`，backend/cache 回归环境为 `/home/ns/.virtualenvs/ns_backend`；仓库内无 `.venv`；Windows 规定环境路径仍为 `S:\PythonVenv` |
...
| `src/ns_common` 实际能力 | `VERIFIED` | P08 StateStore contract保持；Redis standalone provider具备Lua CAS/transaction/append、opaque revision、直接构造/replace可信边界、自定义secret返回复验、任意BaseException lifecycle恢复和runtime composition。Valkey driver compatibility已实现但无valkey-server集成证据。cache与StateStore严格分离，不含lease/fencing/cluster。 |
...
| `src/ns_runtime` 实际能力 | `VERIFIED` | P02-P09及DR-1保持冻结；composition root按typed state_store config创建provider，由既有RuntimeService启动路径open、既有shutdown owner close。delivery/processor仍只依赖StateStore且task.dispatch关闭，不含P11发送或P12 ACK能力。 |
...
| 当前本地测试基线 | `VERIFIED` | P10-FIX-02 provider专项23、P08/P10/provider 62、P03-P10联合417；Linux runtime标准asyncio与uvloop各 `Ran 791, OK (skipped=1)`；backend全量 `Ran 791, OK (skipped=50)`。全树compileall、两环境pip check、cold import、diff check及delivery/routing禁止项扫描通过；均为commit-recorded local evidence，无远程CI。 |
| 当前状态校准时间 | `VERIFIED` | `2026-07-22T22:33:17+08:00` |
...
|---|---|---|---|---|---|
| 配置基础设施 | `src/ns_common/config/` | backend/cache/log/runtime 总组及 17 个强类型 runtime 细分组的深度不可变快照、metadata/生效模式、严格加载/序列化、兼容别名、来源优先级、backend 覆盖和已验证快照恢复保持不变；facade 保留原公共入口与权威对象身份，global `ns_config` 改为显式访问时惰性初始化，无参 `NsConfig.load()` 目录语义不变 | `tests.test_config` 40/40、`tests.test_config_package` 9/9；本轮 facade/cache/logger/testing 兼容 67/67；runtime 联合 317、backend 全量 328 | `F2` | `VERIFIED`，P01 合同保持，P02-FIX-03 仅校准 bootstrap 时序 |
...
| 日志与脱敏 | `src/ns_common/security.py`、`src/ns_common/logger.py` | sanitizer 继续独占字段、路径、对象、fail-closed、严格 JSON-safe 与 digest 资源规则；`NsLogger` 单向拥有或接收显式 `Sanitizer`；JSON/text/color 的权威字段与 LogRecord 内建字段分离，冲突 extra 只进入 `extra_fields`，普通 extra 继续平铺；peer/client/remote address 继续完全脱敏；多进程 handler 及其 portalocker/Redis 可选集成只在实际使用时加载 | WSL logger 12/12、sanitizer 30/30；P01/runtime 249 项、backend 根回归 260 项；已安装测试驱动时冷导入仍不加载 Redis/Valkey | sanitizer、logger 接入均为 `F2` | P01-W09/P01-FIX-03/P01-FIX-04/P01-W10/P01-FIX-05/P01-W17 `VERIFIED` |
| HTTP 基础设施 | `src/ns_common/http_client.py` | 显式 factory/owner 与 legacy map 隔离；owner 具备可重试生命周期；HTTP 诊断仅使用安全 URL 和固定 body 摘要；同步 response sanitizer 接收隔离快照并二次脱敏、普通失败 fail-closed；bearer token 仅进入 Authorization header 且 URL/query 重复会在发送前拒绝 | WSL `tests.test_http_client` 14/14；P01/runtime `Ran 190, OK (skipped=1)`；backend 根回归 `Ran 201, OK (skipped=1)` | lifecycle 与 response security 均为 `F2` | P01-W13/P01-W14 `VERIFIED` |
...
| 强一致状态存储 | `src/ns_common/state_store/`、`src/ns_runtime/state_authority.py` | P08 contract与strong audit chain保持；Redis options精确复验backend/username/finite timeout/endpoint/namespace/source，自定义source结果只接纳None或非空exact str；open/close任意BaseException恢复NEW/OPEN后原对象穿透；真实Redis使用reserved port+PID ownership gate并验证3/501 target跨provider恢复与duplicate无增量 | provider专项23项、P08/P10/provider 62项、P03-P10 417项及双loop/backend全量见acceptance log | `F3`（standalone local evidence） | Redis standalone `VERIFIED`；Valkey driver compatibility `IMPLEMENTED`；Valkey server integration `UNVERIFIED` |
| 公共异常与错误注册 | `src/ns_common/exceptions/` | 85 个异常按显式领域模块登记；冻结 `NsErrorDefinition`、稳定 severity/category、显式不可变注册表和 20 域/78 码 `RUNTIME_*` 覆盖矩阵；P09新增唯一 `RUNTIME_ROUTE_REJECTED/200177`；既有设计场景与NACK映射不变，查询不做MRO fallback | `tests/test_exceptions.py` 完整策略矩阵及类/code/numeric_code唯一；覆盖矩阵、冷启动导入和依赖扫描通过 | `F2` | `VERIFIED`，ERR-1由P09追加精确policy reject错误 |
| backend IAM 合同 | `src/ns_backend/iam`、`src/ns_common/iam.py` | IAM-R1六类principal、完整introspection/access/bootstrap/invalidation及payload_ref validation；P10把result扩展为object/version/checksum/tenant/size typed integrity metadata并由runtime实时调用 | `tests.test_backend_runtime_iam`、`tests.test_runtime_iam_client`与backend全量；详见acceptance log | `F3` | P06保持`VERIFIED`；P10 payload_ref contract `VERIFIED` |
| runtime 进程与事件循环 | `src/ns_runtime/__init__.py`、`src/ns_runtime/main.py`、`src/ns_runtime/context.py`、`src/ns_runtime/service.py`、`src/ns_runtime/startup.py`、`src/ns_runtime/shutdown.py`、`src/ns_runtime/event_loop_observability.py`、`src/ns_runtime/diagnostics.py` | package/入口冷导入无启动副作用；startup preflight、RuntimeService、RuntimeShutdownCoordinator 保持同一显式 context；RuntimeEventLoopMonitor 使用 preflight 选择结果维护有界 snapshot 与既有指标，critical 非取消异常请求同一 shutdown 并先使 service FAILED，显式 stop 成功后按 RSL-1 进入 STOPPED，失败历史保留在 report；signal registration 在 asyncio/fallback 两条路径均精确恢复原 handler；diagnose 保持只读和脱敏 | shutdown/monitor/service/main 49/49；runtime 联合 349 项、backend 根回归 360 项，均仅 1 个 Windows 专用跳过 | P02 全部能力 `F2` | P02-W01 至 W08（含 FIX-04/FIX-05）与阶段出口重新 review 后 `VERIFIED` |
| Envelope 与类型注册 | `src/ns_runtime/protocol/` | ENV-1 grouped `json.v1`、raw/normalized authority、resource limits、schema/version/registry/extensions、canonical bytes、ERR-1 与单一 feature/direction/processor/routing metadata；TargetGroup冻结exact primary selector、strategy/rebind/count矩阵与capability AND集合；P05激活9项connection lifecycle contract，其余未实现能力继续disabled | P09 protocol/routing contract专项及P03-P08联合回归 | `F2` | `VERIFIED`，P09 Phase A追加严格target与RoutingRequirement合同 |
| Transport 与连接 | `src/ns_runtime/transport/` | TC-1 WebSocket/TCP adapter、TLS/plaintext policy、session queue/backpressure、deadline、terminal outcome、lifecycle owner 与 conformance harness | P04 22-case conformance及 P01-P05 联合回归 | `F2` | `VERIFIED`，P04 冻结 |
...
| 可靠投递 | `src/ns_runtime/delivery/` | DR-1逻辑未改写；独立provider A/B在同一Redis namespace验证all accepted与501-target root/shard Summary、prepared DeliveryRecord、revision/state version/payload摘要恢复，重复同message+fingerprint返回duplicate且key集合不变。delivery不导入driver、不注册task.dispatch、不发送 | P10-FIX-02真实恢复专项与P03-P10 417项通过；delivery/routing对基线零diff | `F3`（local only） | P10-W01至W14、P10-FIX-01及P10-FIX-02 `VERIFIED`；P11-W01继续`BLOCKED` |
...
| 可观测与测试基础 | `src/ns_common/logger.py`、`src/ns_common/observability.py`、`src/ns_common/testing.py` | 已有日志、不可变脱敏 metrics/trace/diagnostic 记录、显式 sink 协议、关闭后原子拒绝与 dropped/rejected 分离、35 项有限 metric definition，以及实例隔离、OPEN/CLOSING/CLOSED Factory 状态机、可重试端口/目录所有权、`NsReservedPort` 底层 socket 关闭屏障和同步/异步 Redis namespace 工厂 | observability 23/23、testing 35/35、依赖门禁 7/7、P01/runtime 256 项、backend 267 项；真实 socket 失败重试与直接 release/Factory close 并发专项通过 | 公共观测基础 `F2`；测试 Factory `F2`；runtime 观测 `F0` | `VERIFIED`，P01-W16/P01-FIX-09/P01-W17 与 P01 阶段出口均已验证 |
...
### 2.6 本地 Redis/Valkey StateStore 验收环境

P10 及后续可靠状态阶段允许使用当前 WSL Ubuntu 开发环境中的本地 standalone Redis 作为真实 StateStore provider 集成测试实例。

本地连接信息：

* 运行环境：当前 Codex 使用的 WSL Ubuntu
* Redis authentication mode：default user password authentication (requirepass)
* Host：`127.0.0.1`
* Port：`6379`
* Database：`0`
* Username：empty
...
1. runtime 必须通过 `runtime.state_store` 配置、P08 `StateStore` contract 和明确的 provider composition 使用 Redis。
2. admission service、delivery service、processor 和业务代码不得直接创建 Redis client。
3. 不得将现有 `ns_common.cache` Redis backend 用作 DeliveryRecord、MessageDeliverySummary、tenant dedup、state log 或其他强一致状态权威。
4. Redis client 和 StateStore provider 必须由现有 RuntimeService/RuntimeContext 生命周期 owner 创建和关闭，不得增加第二个 event loop、TaskSupervisor 或 shutdown owner。
5. 自动化测试必须为每次运行生成唯一 namespace，例如：
   `ns_runtime:test:<test-run-uuid>:`
6. 测试清理只能扫描并删除当前测试 namespace 下的 key。
7. 禁止执行：

   * `FLUSHDB`
   * `FLUSHALL`
   * 无 namespace 限制的 `KEYS *`
   * 删除其他测试运行或其他组件的 key
8. 本地 Redis 可使用 standalone 模式；Sentinel、Cluster、leader lease、fencing 和跨节点 owner 不属于当前 provider 修复范围。
9. Redis 连接成功、`PING` 成功或存在 Redis key 不能单独证明 P10 完成；必须通过真实 provider 验证 CAS、transaction、原子初始化、并发 dedup、冲突、超时和失败语义。
10. 文档中的本地连接信息只是 Codex 可使用的环境说明，不得被写成实施完成结果或测试通过证据。
11. 当前本地 Redis 使用 Redis default user + requirepass 模式。 Codex 不应使用 ACL username 认证方式连接。 如果未来切换 Redis ACL，需要单独更新该环境说明。

#### P10 验收要求

P10 Redis 修复完成后，acceptance log 必须分别记录：

* 实际使用的 Redis server 版本；
* 实际使用的 Python Redis driver 版本；
* 连接目标：`127.0.0.1:6379/0`；
* 实际认证模式，以及username为空或使用命名ACL user；
* 测试 namespace，密码必须在测试输出中脱敏；
* Redis provider lifecycle 测试结果；
* P08 StateStore contract conformance 测试结果；
* P10 Summary、DeliveryRecord、dedup 和 batched initialization 的真实 Redis 集成测试结果；
...
|---|---|---|---|---|
| `COM-001` | 不可变配置快照、配置来源、组版本元数据和强类型 runtime 细分配置 | `ns_common.config` | `VERIFIED`（P09追加三个严格正数的routing资源上限并完成round-trip） | P01/P09 |
| `COM-002` | event loop 选择、任务监督和优雅关闭基础 | `ns_common.async_runtime` | `VERIFIED`（selector 与 TaskSupervisor 公共原语已验证；进程信号和资源关闭编排属于 P02 runtime 集成） | P01 |
...
| `COM-008` | 显式 HTTP client factory 和生命周期管理 | `ns_common.http_client` | `VERIFIED` | P01 |
| `COM-009` | Metrics、trace 和 diagnostic sink 接口 | `ns_common.observability` | `VERIFIED`（关闭隔离、dropped/rejected 分离、35 项有限 metric definition、高基数别名防御和完整公开 record 字节门禁） | P01 |
| `COM-010` | 强一致 state store 抽象 | `ns_common.state_store` | `VERIFIED`（backend-neutral authority/namespace/capability/revision/CAS/transaction/lifecycle/failure contract；P10登记tenant-scoped delivery admission authority与p10 capability组合） | P08/P10 |
| `COM-011` | Redis/Valkey state store adapter 与 Lua runner | `ns_common.state_store` | Redis standalone `VERIFIED`；Valkey driver compatibility `IMPLEMENTED`；Valkey server integration `UNVERIFIED`。无Sentinel/Cluster/TLS材料/lease/fencing | P10-FIX-01/P10-FIX-02 |
| `COM-012` | SQLite WAL 开发 state store adapter | `ns_common.state_store` | `DEFERRED` | P08 后续 provider 阶段；P08 禁止实现 |
| `COM-013` | 通用 lease/fencing 原语 | `ns_common.state_store` | `DEFERRED` | P17/owner authority 阶段；P08 仅冻结 epoch assertion，不实现 lease/fencing |
| `COM-014` | 本地敏感凭证加密存储原语 | `ns_common.security.AesGcmSecretBox` | `VERIFIED`（AES-GCM seal/open、显式 AAD 与 nonce factory；runtime 私有缓存另负责 HMAC 签名复验、TTL、role scope 与本地撤销，无明文落盘接口） | P06 |
| `COM-015` | 测试资源工厂、临时配置和真实依赖管理 | `ns_common.testing` | `VERIFIED`（Factory 状态机、可重试所有权、并发 close 及 reservation 底层 socket 关闭屏障已验证） | P01 |
...
- Ubuntu/Linux：可使用 WSL；标准 asyncio 与 uvloop 双模式，Redis standalone集成与Valkey driver compatibility测试；只有实际`valkey-server`可用并通过时才记录Valkey server integration；新建 Python 虚拟环境统一位于 `/home/ns/.virtualenvs` 下。
...
| `P01-W11` | `VERIFIED` | 将 `ns_common.exceptions` 包化并建立显式、不可变错误注册表；每个现有错误登记 code、numeric_code、severity、category、retryable、disconnect_required、audit_required、safe_detail、action；保持原异常行为和序列化兼容 | 包结构、继承/序列化兼容、策略矩阵、错误码唯一性、显式注册、NACK 映射和依赖边界测试通过；不补齐 W12 错误 |
| `P01-FIX-06` | `VERIFIED` | 校准当前 33 个 definition 的策略 metadata：通用错误采用保守副作用策略，精确叶子错误显式开启 retry/disconnect/audit；查询保持精确类型且不做 MRO fallback | 33 项完整策略矩阵、当前调用事实、兼容契约、注册表结构和全部回归门禁通过 |
| `P01-W12` | `VERIFIED` | 完整登记设计文档已明确的 `RUNTIME_*` 错误，包括协议、IAM、tenant、target、route、payload_ref、ACK/NACK/Defer、lease、fencing、owner、processor、配置、transport 和集群错误 | 错误覆盖矩阵无缺项 |
| `P01-FIX-07` | `VERIFIED` | 校准 processor timeout 与 protocol parse 的保守默认策略，新增精确 `RUNTIME_PROTOCOL_VIOLATION` 并建立独立设计场景门禁 | W12 契约、策略矩阵、NACK 映射和独立场景门禁全部通过 |
| `P01-W13` | `VERIFIED` | 重构 HTTP client 创建方式，保留现有兼容函数，但新增显式 factory 和 owner 生命周期；runtime 只能接收显式实例 | factory/owner 隔离、并发/取消/失败关闭、兼容 getter 和 runtime 全局依赖边界验证通过 |
| `P01-W14` | `VERIFIED` | HTTP 错误只保留安全 body 摘要；调用方可提供 response sanitizer；IAM token 不进入 URL、日志和错误明细 | IAM 模拟错误响应不泄露敏感字段 |
| `P01-W15` | `VERIFIED` | 建立 MetricsSink、TraceSink、DiagnosticSnapshotSink 接口和内存测试实现；接口不得强制 HTTP exporter | runtime 后续模块可显式注入 sink；FIX-08 已完成加固并恢复验证状态 |
| `P01-FIX-08` | `VERIFIED` | 加固 sink 关闭后的 best-effort 故障隔离、metric 显式有限 attribute schema 与完整公开 record 字节限制 | 关闭竞态不破坏主链路；35 个标准 metric definition 完整冻结；三种公开 record 均满足严格 JSON 字节上限 |
| `P01-W16` | `VERIFIED` | 建立 `ns_common.testing` 测试工厂，统一临时目录、临时配置、可控 clock、内存 sink、随机端口和真实 Redis namespace | FIX-09 已补齐 Factory 与 reservation 两层可重试所有权及关闭屏障 |
| `P01-FIX-09` | `VERIFIED` | 修复测试资源 Factory 的可重试关闭和并发关闭语义；以 OPEN → CLOSING → CLOSED 显式状态保留未完成资源所有权 | 底层 socket 失败重试、直接 release/Factory close 屏障及全部回归通过 |
| `P01-W17` | `VERIFIED` | 建立 runtime 独立依赖清单和测试依赖清单；生产依赖与测试/压测依赖分离 | 安装 backend 不隐式安装 QUIC 实验依赖；runtime 安装可独立完成 |

**P01-FIX-09 本轮继续校准原因与结果：** Factory 状态机已建立后，静态复核发现 `NsReservedPort.release()` 在实际 `socket.close()` 前将 `_socket` 设为 `None`，底层关闭失败或并发直接释放时会提前表现为已释放。本轮已把真实 close 纳入 reservation 锁，并只在 close 成功后清空 socket；失败保留同一对象可重试，直接 release 与 Factory close 共享关闭屏障。
...
| `P01-FIX-06` | [acceptance log / P01-FIX-06](ns_runtime_acceptance_log_0.0.2.md#p01-fix-06) | [ADR-015](ns_runtime_architecture_decisions_0.0.2.md#adr-015) |
| `P01-W12` | [acceptance log / P01-W12](ns_runtime_acceptance_log_0.0.2.md#p01-w12) | [ADR-015](ns_runtime_architecture_decisions_0.0.2.md#adr-015) |
| `P01-FIX-07` | [acceptance log / P01-FIX-07](ns_runtime_acceptance_log_0.0.2.md#p01-fix-07) | [ADR-015](ns_runtime_architecture_decisions_0.0.2.md#adr-015) |
| `P01-W13` | [acceptance log / P01-W13](ns_runtime_acceptance_log_0.0.2.md#p01-w13) | [ADR-016](ns_runtime_architecture_decisions_0.0.2.md#adr-016) |
| `P01-W14` | [acceptance log / P01-W14](ns_runtime_acceptance_log_0.0.2.md#p01-w14) | [ADR-017](ns_runtime_architecture_decisions_0.0.2.md#adr-017) |
| `P01-W15` | [acceptance log / P01-W15](ns_runtime_acceptance_log_0.0.2.md#p01-w15) | [ADR-018](ns_runtime_architecture_decisions_0.0.2.md#adr-018) |
| `P01-FIX-08` | [acceptance log / P01-FIX-08](ns_runtime_acceptance_log_0.0.2.md#p01-fix-08) | [ADR-018](ns_runtime_architecture_decisions_0.0.2.md#adr-018) |
| `P01-W16` | [acceptance log / P01-W16](ns_runtime_acceptance_log_0.0.2.md#p01-w16) | [ADR-019](ns_runtime_architecture_decisions_0.0.2.md#adr-019) |
| `P01-FIX-09` | [acceptance log / P01-FIX-09](ns_runtime_acceptance_log_0.0.2.md#p01-fix-09) | [ADR-019](ns_runtime_architecture_decisions_0.0.2.md#adr-019) |
| `P01-W17` | [acceptance log / P01-W17](ns_runtime_acceptance_log_0.0.2.md#p01-w17) | [ADR-020](ns_runtime_architecture_decisions_0.0.2.md#adr-020) |
...
| ID | 状态 | 实施内容 | 完成判定 |
|---|---|---|---|
| `P02-W01` | `VERIFIED` | 建立 `src/ns_runtime` 独立组件和唯一进程入口 `main.py` | 可通过模块方式启动和退出 |
| `P02-W02` | `VERIFIED` | 建立 RuntimeService 生命周期：created、starting、running、stopping、stopped、failed | 非法迁移返回稳定错误 |
| `P02-FIX-01` | `VERIFIED` | 修复 RuntimeService 关闭幂等、失败后清理与首次 event-loop 绑定竞态 | stop 幂等、失败后可重试清理、首次 loop owner 原子绑定，并发与异常专项全部通过 |
| `P02-W03` | `VERIFIED` | 建立显式 RuntimeContext，持有配置快照、clock、logger、metrics、trace、task supervisor 和后续依赖占位 | 深层模块无全局服务定位器 |
| `P02-FIX-02` | `VERIFIED` | 修复 RuntimeContext 冷导入及构造期隐式全局配置副作用 | context 冷导入与构造期类型校验均不加载全局配置、logger 或 HTTP 模块，不读取或创建文件系统资源 |
| `P02-W04` | `VERIFIED` | 启动时执行环境、依赖、目录、event loop、transport 配置、state store 生产限制和 TLS 前置校验 | 配置错误在监听前失败 |
| `P02-FIX-03` | `VERIFIED` | 修复真实 main() 配置加载及冷进程 package bootstrap 绕过 RSP-1 | 显式配置加载与统一错误映射；冷进程 import 不初始化 global config，失败时无 ensure、目录、policy 或 service 副作用 |
| `P02-W05` | `VERIFIED` | 初始角色状态支持 singleton、sub_node、standby_master、active_master 配置值；实际协调能力保持 feature disabled | 四角色进入只读本地状态；transport/cluster_coordination/delivery 统一审计并返回 `RUNTIME_FEATURE_DISABLED` |
| `P02-W06` | `VERIFIED` | 实现 SIGINT/SIGTERM 优雅关闭顺序：停止接入、停止新任务、取消后台任务、关闭 sink/client、输出未完成任务摘要 | 重复关闭幂等；关闭超时可观测 |
...
- `P02-W01`：见 [acceptance log](ns_runtime_acceptance_log_0.0.2.md#p02-w01)；独立组件与唯一模块入口契约登记为 `RTE-1`。
- `P02-W02`：见 [acceptance log](ns_runtime_acceptance_log_0.0.2.md#p02-w02)；RuntimeService 六态生命周期契约登记为 `RSL-1`，最终失败清理与幂等边界由 P02-FIX-01 校准。
- `P02-FIX-01`：见 [acceptance log](ns_runtime_acceptance_log_0.0.2.md#p02-fix-01)；长期生命周期决策见 [ADR-021](ns_runtime_architecture_decisions_0.0.2.md#adr-021)。
- `P02-W03`：见 [acceptance log](ns_runtime_acceptance_log_0.0.2.md#p02-w03)；显式依赖上下文契约登记为 `RTC-1`，长期决策见 [ADR-022](ns_runtime_architecture_decisions_0.0.2.md#adr-022)。
- `P02-FIX-02`：见 [acceptance log](ns_runtime_acceptance_log_0.0.2.md#p02-fix-02)；RuntimeContext 冷导入与构造期类型验证隔离边界已校准进 `RTC-1` 与 [ADR-022](ns_runtime_architecture_decisions_0.0.2.md#adr-022)。
- `P02-W04`：见 [acceptance log](ns_runtime_acceptance_log_0.0.2.md#p02-w04)；启动 preflight 契约登记为 `RSP-1`，长期顺序与能力边界见 [ADR-023](ns_runtime_architecture_decisions_0.0.2.md#adr-023)。
- `P02-FIX-03`：见 [acceptance log](ns_runtime_acceptance_log_0.0.2.md#p02-fix-03)；真实冷进程 bootstrap、惰性 global config 兼容、显式配置加载、统一错误归一化与零目录副作用边界已校准进 `RSP-1` 和 [ADR-023](ns_runtime_architecture_decisions_0.0.2.md#adr-023)。
- `P02-W05`：见 [acceptance log](ns_runtime_acceptance_log_0.0.2.md#p02-w05)；初始角色与统一能力门禁契约登记为 `RRS-1`，显式安全日志接线和不授予协调权的长期边界见 [ADR-024](ns_runtime_architecture_decisions_0.0.2.md#adr-024)。
- `P02-W06`：见 [acceptance log](ns_runtime_acceptance_log_0.0.2.md#p02-w06)；本地进程关闭契约登记为 `RSD-1`，固定顺序、信号、失败隔离和未来 transport drain 边界见 [ADR-025](ns_runtime_architecture_decisions_0.0.2.md#adr-025)。
- `P02-FIX-04`：见 [acceptance log](ns_runtime_acceptance_log_0.0.2.md#p02-fix-04)；精确 signal handler 恢复语义已校准进 `RSD-1` 与 [ADR-025](ns_runtime_architecture_decisions_0.0.2.md#adr-025)。
- `P02-W07`：见 [acceptance log](ns_runtime_acceptance_log_0.0.2.md#p02-w07)；event-loop 内部观测契约登记为 `RLO-1`，采样、指标、失败语义和生命周期边界见 [ADR-026](ns_runtime_architecture_decisions_0.0.2.md#adr-026)。
- `P02-FIX-05`：见 [acceptance log](ns_runtime_acceptance_log_0.0.2.md#p02-fix-05)；critical monitor failure 联动已校准进 `RSL-1`/`RLO-1` 与 [ADR-021](ns_runtime_architecture_decisions_0.0.2.md#adr-021)/[ADR-026](ns_runtime_architecture_decisions_0.0.2.md#adr-026)。
- `P02-W08`：见 [acceptance log](ns_runtime_acceptance_log_0.0.2.md#p02-w08)；只读本地诊断契约登记为 `RDI-1`，唯一入口、输出脱敏、零启动副作用与后续管理边界见 [ADR-027](ns_runtime_architecture_decisions_0.0.2.md#adr-027)。
...
| ID | 状态 | 实施内容 | 完成判定 |
|---|---|---|---|
| `P03-W01` | `VERIFIED` | 定义核心分组 protocol、message、source、target、route、delivery、stream、auth_context、payload、callback、trace、extensions 的类型模型 | 所有分组有严格字段集合 |
| `P03-W02` | `VERIFIED` | 建立 inbound raw model 与 normalized model 分离；inbound 不允许 source/auth_context；normalized 由 runtime 注入 | 伪造测试全部拒绝 |
| `P03-W03` | `VERIFIED` | 建立 JSON 深度、总大小、字符串长度、数组长度和数字范围限制 | 资源消耗攻击被拒绝 |
| `P03-W04` | `VERIFIED` | 建立基础 schema + message.type schema 叠加校验 | 类型专属字段缺失时返回稳定错误 |
| `P03-W05` | `VERIFIED` | 建立协议版本模型、兼容矩阵、minor/patch 降级和 schema 选择 | processor 不需要判断协议版本 |
| `P03-W06` | `VERIFIED` | 建立内置 message type 全量注册表，覆盖 connection、task、delivery、stream、runtime.control、cluster.event、config、dead_letter、replay、cancel、hold、status、runtime.error | 设计要求的类型族无缺项 |
| `P03-W07` | `VERIFIED` | 每个注册项包含 schema、category、默认 reliability、权限声明、processor key、审计级别、feature flag 和响应类型 | 注册完整性测试通过 |
| `P03-W08` | `VERIFIED` | 建立 extension namespace 注册和 schema 校验；未注册 namespace 按策略拒绝或审计忽略 | 插件无法放宽核心字段 |
| `P03-W09` | `VERIFIED` | 建立标准错误 Envelope，安全 detail 只来自错误注册表和 sanitizer | 错误不携带原始异常或敏感值 |
| `P03-W10` | `VERIFIED` | 建立 canonical serialization 规则，用于 checksum、审计摘要和确定性测试 | 相同 normalized Envelope 序列化稳定 |
| `P03-W11` | `VERIFIED` | 为未实现 message type 建立统一 FeatureDisabledProcessor 占位行为；该行为只返回错误，不执行功能 | 不存在 stub success |
| `P03-FIX-01` | `VERIFIED` | 阶段出口校准必填整数/数组类型与 JSON 容器深度口径 | 直接构造与 wire 校验保持严格、稳定错误 |
| `P03-FIX-02` | `VERIFIED` | 收紧 StreamGroup 数组元素 required integer 语义与 extension namespace wire 语法顺序 | 非法输入稳定 fail-closed，unknown policy 仅处理合法未知 namespace |
...
| ID | 状态 | 实施内容 | 完成判定 |
|---|---|---|---|
| `P04-W01` | `VERIFIED` | 定义 TransportAdapter、TransportSession、TransportMessage、TransportClose、TransportError、TransportCapabilities 契约 | 上层不引用 WebSocket 库对象 |
| `P04-W02` | `VERIFIED` | 定义 capability 集合并为 websocket_tcp 声明 reliable_ordered_messages、native_keepalive、transport_flow_control 基础能力 | capability 由 adapter 权威声明 |
| `P04-W03` | `VERIFIED` | 实现 WebSocket TLS/TCP 监听、连接接受、文本消息接收、发送、ping/pong、关闭和异常映射 | conformance 正常路径通过 |
| `P04-W04` | `VERIFIED` | 强制 WebSocket 只接受文本帧；二进制帧返回协议错误或关闭 | 二进制负向测试通过 |
| `P04-W05` | `VERIFIED` | 建立单 transport session 读队列和写队列；限制消息大小和队列容量 | 队列满不无限等待 |
| `P04-W06` | `VERIFIED` | 建立 transport_connection_id、transport_session_id、transport_stream_id、peer 摘要和 path 初始模型 | 诊断字段与 logical connection 分离 |
| `P04-W07` | `VERIFIED` | 建立 transport error 到 `RUNTIME_TRANSPORT_*` 的映射 | 所有库异常被收敛 |
| `P04-W08` | `VERIFIED` | 建立 adapter registry；未启用 adapter 不加载依赖、不监听端口 | QUIC 依赖不会影响当前启动 |
| `P04-W09` | `VERIFIED` | 建立 transport conformance 测试套件，未来所有 adapter 必须复用 | WebSocket adapter 全部通过 |
| `P04-W10` | `VERIFIED` | 记录通用 transport 指标：连接数、握手耗时、收发字节、队列深度、发送失败、关闭原因、背压时长 | 指标标签不含高基数 ID |
| `P04-FIX-01` | `VERIFIED` | 修复 transport error mapper 在禁用/缺失 websockets 依赖环境下对普通异常的无条件第三方导入 | backend 目标回归与 fresh-process 依赖隔离通过 |
| `P04-FIX-02` | `VERIFIED` | 修复 transport terminal outcome exact classification、keepalive terminal close、单一 send deadline、close cancellation 可重试资源清理和 start failure composition-root cleanup | 竞态/取消测试与 P04 阶段出口全量回归通过 |
...
阶段出口验收见 [acceptance log](ns_runtime_acceptance_log_0.0.2.md#p04-fix-02)，`P04-W01` 至 `P04-W10`、`P04-FIX-01` 与 `P04-FIX-02` 已达到 `VERIFIED/F2`；游标只推进到 `P05-W01 NOT_STARTED`，未实施 P05。
...
| ID | 状态 | 实施内容 | 完成判定 |
|---|---|---|---|
| `P05-W01` | `VERIFIED` | 建立连接状态机 accepted、handshaking、authenticated、active、draining、closing、closed 和失败原因 | 非法迁移测试覆盖 |
| `P05-W02` | `VERIFIED` | 第一条有效应用消息强制为 connection.hello；握手超时后关闭 | 非 hello 消息无法进入 processor |
| `P05-W03` | `VERIFIED` | 解析 hello 中 token、component_type、协议版本、requested capabilities 和 resume 请求；token 只保留在握手受控内存 | token 不进入普通日志和 session snapshot |
| `P05-W04` | `VERIFIED` | 完成协议版本和 transport capability 协商并写入 SessionContext | 后续 codec 使用协商结果 |
| `P05-W05` | `VERIFIED` | 建立 logical connection、transport session、network path 三层映射 | connection_id 不等于 transport ID |
| `P05-W06` | `VERIFIED` | 建立 connection_id、identity、tenant、component_type、capability、session_id 本地索引 | 索引增删原子且无悬挂引用 |
| `P05-W07` | `VERIFIED` | connection.accepted 只返回规定的最小字段 | 不泄露 tenant、identity 和完整 capabilities |
| `P05-W08` | `VERIFIED` | 建立 transport 原生心跳和 envelope heartbeat；envelope heartbeat 经过轻量 processor，不创建 DeliveryRecord | 双层心跳行为可区分 |
| `P05-W09` | `VERIFIED` | 建立 connection.drain 单向状态；停止新目标分配，保留 ACK/control/health | drain 不可取消 |
| `P05-W10` | `VERIFIED` | 普通网络断开进入默认 30s grace；grace 内 connection 不作为 active target | 路由索引立即移除 active 属性 |
| `P05-W11` | `VERIFIED` | resume 必须重新 IAM 校验并匹配 identity、tenant、component_type；成功递增 connection_epoch | 旧 epoch 消息全部拒绝 |
| `P05-W12` | `VERIFIED` | kick、安全违规、协议严重错误和恶意重复确认设置不可恢复关闭标志 | resume 请求被拒绝并审计 |
| `P05-W13` | `VERIFIED` | 建立 connection.reauth、accepted/rejected 语义和 session 到期策略接口 | 续期失败不会无限信任旧权限 |
| `P05-W14` | `VERIFIED` | 会话快照异步输出；resume/kick/security close 强审计接口预留 | 普通 heartbeat 不进入强一致链路 |
| `P05-FIX-01` | `VERIFIED` | 校准 P05 logical ingress、canonical protocol activation、lightweight lifecycle processor、safe rejected handshake 与 runtime composition | P03 schema/processor conformance、真实 plaintext/TLS loopback、resume/reauth/fail-closed 回归全部通过 |
| `P05-FIX-02` | `VERIFIED` | harden logical composition parse cleanup、drain persistence、close ownership、total handshake deadline 与 IAM supervisor safety | failure/race/ControlledClock/real loopback 与全回归通过 |
| `P05-FIX-03` | `VERIFIED` | harden pre-index candidate cleanup ownership 与 draining external terminal convergence | candidate retry/cancel、cross-lifecycle first-reason-wins 与 stale-task 回归通过 |
| `P05-FIX-04` | `VERIFIED` | harden resume post-commit ownership handoff cancellation | coordinator commit 后同步 transfer，post-commit cancel/failure 由 logical owner fail-close 且 close failure 可 retry |
...
| ID | 状态 | 实施内容 | 完成判定 |
|---|---|---|---|
| `P06-B01` | `VERIFIED` | 扩展 principal 类型：frontend_user、backend_service、client、node、runtime_node、management | introspection contract 覆盖全部组件 |
| `P06-B02` | `VERIFIED` | introspection 返回 identity、tenant_id、component_type、capabilities、permission_snapshot_ref、permission_version、issued_at、expires_at、credential status | runtime 无需猜测 tenant 和能力 |
| `P06-B03` | `VERIFIED` | 校验客户端声明 component_type 和 requested capabilities；最终值由 IAM 裁决 | 客户端自报不能提权 |
| `P06-B04` | `VERIFIED` | 建立 runtime node credential 签发、刷新、撤销和 role scope | 节点凭证独立于用户 access token |
| `P06-B05` | `VERIFIED` | 建立 runtime bootstrap 合同，返回角色授权、候选 master、配置版本和策略版本 | runtime 启动可获取控制面信息 |
| `P06-B06` | `VERIFIED` | access_check 支持 runtime message.type、target、cross-tenant、management 和 task creation 上下文 | 权限决策可审计和解释 |
| `P06-B07` | `VERIFIED` | 建立权限版本失效事件或轮询版本合同 | 缓存模式可感知撤销 |
| `P06-B08` | `VERIFIED` | 建立 payload_ref validation 合同，覆盖对象、版本、checksum、tenant、owner、source、target、过期、撤销和 callback | P10 可直接接入 |

### runtime 工作包

| ID | 状态 | 实施内容 | 完成判定 |
|---|---|---|---|
| `P06-R01` | `VERIFIED` | 建立显式 IamClient，使用 P01 HTTP factory；所有请求携带 trace 和内部服务凭证 | 无全局 client |
| `P06-R02` | `VERIFIED` | 建立 handshake authentication 流程和安全超时 | IAM 不可用按配置拒绝或受限降级 |
| `P06-R03` | `VERIFIED` | 建立 PermissionSnapshot，普通 Envelope 只注入最小 auth_context 摘要 | 不保存原始 IAM 返回体 |
| `P06-R04` | `VERIFIED` | 严格消息鉴权每条调用 backend；缓存模式按 TTL、version 和失效事件刷新 | 两种模式行为一致可解释 |
| `P06-R05` | `VERIFIED` | tenant 硬校验在 processor 前执行；操作权限在通用鉴权 processor 执行 | 安全硬校验与业务权限分层 |
| `P06-R06` | `VERIFIED` | 建立本地凭证缓存加密、TTL、签名校验和 role scope 校验 | 明文凭证不落盘 |
| `P06-R07` | `VERIFIED` | 建立 backend 不可用降级矩阵，禁止高风险控制、跨 tenant、新配置和全局协调写入 | 降级能力受限且可审计 |
| `P06-R08` | `VERIFIED` | backend 恢复后重新验证凭证、角色、配置、lease、fencing 和 session snapshot | 不自动沿用过期授权 |
...
| ID | 状态 | 实施内容 | 完成判定 |
|---|---|---|---|
| `P07-W01` | `VERIFIED` | 定义 ProcessorContext，包含 normalized Envelope、session、trace、config/policy version、clock 和显式服务依赖 | context 不含隐藏全局访问 |
| `P07-W02` | `VERIFIED` | 定义通用阶段顺序：安全硬校验、权限、限流入口、幂等预检、审计标记、路由预处理、message processor、响应收尾 | 顺序固定并可测试 |
| `P07-W03` | `VERIFIED` | processor registry 按 message.type、阶段、协议版本、feature flag 注册 | 重复和冲突注册启动失败 |
| `P07-W04` | `VERIFIED` | processor 执行支持 timeout、取消、异常隔离、标准错误映射和最终审计 | 未捕获异常不泄露原始内容 |
| `P07-W05` | `VERIFIED` | 审计 sink 记录一次最终处理结果；包含安全摘要、processor、action、error、trace、config/policy version | 每条可执行消息恰好一个最终审计结果 |
| `P07-W06` | `VERIFIED` | 高风险控制审计接口标记为强一致，普通 processor 审计使用显式 sink；P08 已通过 authority service 接入 StateStore contract | 不把强审计误标为 best-effort |
| `P07-W07` | `VERIFIED` | 内部 EventBus 支持类型化事件、不可变 subscription handle、幂等退订、publish 快照、超时和异常隔离 | 订阅者失败或退订不破坏主链路和在途发布 |
| `P07-W08` | `VERIFIED` | 事件发布只携带安全摘要和对象 ID；禁止携带原始 token/payload | 事件泄露扫描为零 |
| `P07-W09` | `VERIFIED` | 插件发现只加载本地受信任插件；插件声明 namespace、schema、权限、timeout、状态 namespace 和 feature flag | 未授权插件无法加载 |
| `P07-W10` | `VERIFIED` | health、heartbeat、reauth、drain 和 feature disabled processor 完整接入 pipeline | 不存在直接 transport 回调处理业务 |
...
## P08 StateStore 抽象契约与 Authority Boundary
...
在 `ns_common.state_store` 建立严格失败、backend-neutral 的 StateStore contract，冻结 state authority、namespace、caller capability、revision、CAS、transaction scope、lifecycle 和 failure semantics。StateStore 是权威状态原语，不是 cache、database wrapper、ORM 或 Redis client。

P08 不实现任何具体存储 provider，不实现 Redis/Valkey/SQLite adapter、Lua、Sentinel、lease、fencing、leader election、cluster membership、reliable delivery 或 routing。P05 connection/session、P06 permission/credential 和 P07 processor state 保持原 owner；P08 当前只激活 strong audit authority，future authority 仅保留 contract，不创建领域记录。

### 工作包

| ID | 状态 | 实施内容 | 完成判定 |
|---|---|---|---|
| `P08-W01` | `VERIFIED` | State Authority and Capability Model：定义允许进入 StateStore 的 authority classification、caller capability 和 `StateStoreCapabilities` | local/external/transient state 不能被 Store scope 接纳；future authority 保持 reserved |
| `P08-W02` | `VERIFIED` | Core StateStore Contract：定义 StateNamespace、StateKey、StateDocument、StateRevision、StateRecord、StateAssertion、StateMutation、StateTransaction、StateStoreHealth 及 open/close/capabilities/read/CAS/transact/append/health | 无无条件 put 或 last-write-wins；所有操作严格失败 |
| `P08-W03` | `VERIFIED` | Namespace and Access Scope：类型化 tenant/system/runtime/plugin/audit namespace、精确 caller scope 和 domain/tenant 隔离 | 裸字符串 key、跨 tenant/domain/namespace 访问在 mutation 前拒绝 |
| `P08-W04` | `VERIFIED` | Version Revision CAS：区分 schema_version、state_version、opaque revision 和 epoch，支持 expected revision/state version/epoch | stale revision/schema/epoch 不执行 mutation；不实现 lease/fencing/epoch allocation |
| `P08-W05` | `VERIFIED` | Lifecycle and Composition：接入 RuntimeContext、RuntimeService 和既有 RuntimeShutdownCoordinator | 只有一个 owner/supervisor/loop/shutdown coordinator；admission 前 Store ready |
...
- open 前 read/write、closed access 和 lifecycle 幂等。
- capability mismatch、裸字符串 key、namespace/tenant/domain 越界。
- CAS success、stale revision conflict、schema mismatch 和 epoch mismatch。
- 多协程 concurrent CAS 只有一个成功；同 scope transaction 同成同败。
- LINEARIZABLE、AT_LEAST_REVISION、STALE_ALLOWED 和 minimum revision。
- unavailable、read timeout、unknown write timeout、indeterminate write 和 recovery generation。
- 多 RuntimeContext 隔离，无 global state、第二 lifecycle owner、TaskSupervisor 或 event loop。
- strong audit failure 阻断 pipeline success；Processor contract 不包含 StateStore。

### 阶段出口

- `COM-010` 达到 `VERIFIED`；`COM-011` 至 `COM-013` 明确保持 deferred，不能由 P08 结果推断已实现。
- `ADR-033`、`ADR-034`、`ADR-035` 冻结 authority、version/CAS 和 lifecycle/failure contract。
- cache 与 StateStore 完全分离；P05/P06/P07 状态 owner 不迁移。
- 所有后续可靠状态只能由窄 authority service 通过 StateStore contract 写入，Processor 不得直接访问。
- 禁止项扫描确认不存在具体 adapter、Lua、lease/fencing、cluster、delivery 或 routing 提前实现。
...
| ID | 状态 | 实施内容 | 完成判定 |
|---|---|---|---|
| `P09-W01` | `VERIFIED` | 实现 connection、identity、tenant、capability、component_type、runtime、broadcast target 严格校验与typed request | exact selector/strategy/rebind/count矩阵已验证 |
| `P09-W02` | `VERIFIED` | 从一次 `LocalConnectionIndex.snapshot()` 建立同一mutation sequence候选并过滤tenant/component/capability/eligibility/epoch | 不查询lifecycle service或transport；remote runtime稳定unavailable |
| `P09-W03` | `VERIFIED` | 建立single、all、broadcast、quorum、all_required、weighted_subset策略 | 多目标参数和证据显式；quorum不判断ACK |
| `P09-W04` | `VERIFIED` | capability默认single；tenant/broadcast只在显式策略下多目标 | 无隐式广播，all_required任一过滤整体失败 |
| `P09-W05` | `VERIFIED` | 建立fixed_connection、same_identity、same_capability、same_tenant、no_rebind_for_control | canonical identity reference执行，安全投影仅digest；无same_component |
| `P09-W06` | `VERIFIED` | 建立`runtime_fallback/fallback.v1`确定性scorer与canonical tie-break | fingerprint与selected targets可复现，不使用random/message/transport/P14信号 |
| `P09-W07` | `VERIFIED` | RoutingDecision显式区分ResolvedRoutingPlan与RoutingFailureReport并记录完整决策证据 | plan可解释且敏感执行字段不进入repr/普通观测 |
| `P09-W08` | `VERIFIED` | RoutingPlan深度不可变；显式previous context推导新plan version | Router无message→version隐藏map，历史plan不修改 |
| `P09-W09` | `VERIFIED` | 冻结RoutingConsistencyPolicy、RoutingPlanRecorder、StrongRoutingPlanAuthority接口 | **仅接口冻结**；Router不接收StateStore；strong-required在无provider时显式disabled/unavailable |
| `P09-W10` | `VERIFIED` | 稳定failure reason/public mapping/later-action纯数据，本地miss不读stale cache | policy/missing/tenant/unavailable精确映射；无retry/queue/timer/task |

### RP-1 post-submit contract review

- 状态：`VERIFIED`。复核开始时按`IN_PROGRESS`重新检查源码，不沿用既有VERIFIED摘要；最终证据追加到acceptance log。
- routing authority拆为`RequestedRoutingIntent`、`RoutingPolicyDecision`和trusted `RoutingRequest`；默认policy显式接受/拒绝/收紧sender建议，安全override只消费可信message contract/risk metadata。
- broadcast effective rebind固定为`fixed_connection`；重新broadcast读取完整新snapshot并创建新plan/version，不执行same-tenant局部替换。
- previous context补齐message ownership、decision/context fingerprint和selected bindings；跨message、非法plan ID、缺失/非法/篡改fingerprint在snapshot读取与plan ID生成前稳定拒绝，new fingerprint排除previous/current plan ID。
- PC-1 stage 2返回per-message `AuthorizationDecisionEvidence`，stage 3至5透明保留，stage 6验证target/tenant/message/permission binding；RoutingPlan IAM引用不再复用session permission snapshot。
- `StrategyParameters`、`ResolvedRoutingPlan`和`RoutingFailureReport`增加严格直接构造不变量与精确stable reason；ENV-1、P05 eligibility、stage-six、audit-before-send、lifecycle audit和W09 interface-only边界保持不变。

### P09-FIX-02 公共契约缺口修复

- 状态：`VERIFIED`。开始修复时P09按`IN_PROGRESS`处理；完成专项、既有边界、P03-P09联合、runtime双loop、backend全量和静态证据后恢复`VERIFIED/F3 (local only)`。
- Permission binding：authorization evidence分别绑定session/effective permission snapshot；允许同ref版本refresh，decision reference覆盖两组snapshot与底层decision结果。stage six在policy/Router前校验完整binding，mismatch稳定返回`AUTHORIZATION_EVIDENCE_MISMATCH`且index snapshot为零。
- Policy/scoring：rebind只能保持或收紧，禁止None到same-*及same-*横向扩张；broadcast固定、security-sensitive no-rebind，矛盾组合reject。affinity/static weight移入policy-owned `RoutingScoringDecision`，默认为空，不引入P14 health scoring。
- Plan/fingerprint：`ResolvedRoutingPlan`保存repr-hidden typed policy decision和authorization evidence，并独立复验展开字段与selected binding。fingerprint绑定IAM decision、authorized target、effective permission和scorer input，但继续排除plan ID、created-at和message reference。
- Failure hint：闭集校准为local/master-query/remote-runtime/authority-recovery；P09不执行master query。P10 Summary/DeliveryRecord、remote/master routing、strong provider、retry/ACK均未实施，`P10-W01`继续`BLOCKED`。

### P09-FIX-03 RP-1 public type trusted-boundary closure

- 状态：`VERIFIED`。修复期间P09按`IN_PROGRESS`处理；专项、既有边界、P03-P09联合、runtime双loop、backend全量和静态证据全部通过后恢复`VERIFIED/F3 (local only)`。
- IAM evidence：`message_binding_reference`只验证当前消息，`semantic_decision_reference`只表达可复现IAM语义并进入fingerprint。公共类型自身拒绝ref不一致、空值和伪造binding；同ref版本refresh合法。真实两Envelope仅message ID不同得到不同message binding、相同semantic reference和fingerprint；跨message evidence复用在policy与Router前拒绝。
- Policy/scorer：stage six从可信message contract构造并验证不可变`RoutingPolicyInvocation`；message type/risk/config/policy或contract security任一不一致都在Router前拒绝。security override为类型化闭集，安全合同只能reject或no-rebind；scorer身份严格固定为`runtime_fallback/fallback.v1`。
- Plan/fingerprint：Router和plan调用同一canonical fingerprint函数，plan公开构造独立验证candidate/binding、filtered集合、strategy cardinality、policy/IAM/scoring展开值并重算fingerprint。candidate、selected顺序、index、previous fingerprint或任一authority语义变化而沿用旧fingerprint均失败。
- 范围：只关闭P09 RP-1公共类型可信边界；未实施P10、P14 health scoring、remote/master routing、strong authority provider、Delivery/Summary/ACK/retry。`P10-W01`保持`BLOCKED`。

### P09-FIX-04 RP-1 selected authority and scorer closure

- 状态：`VERIFIED`。修复期间P09按`IN_PROGRESS`处理；专项、边界、P03-P09联合、runtime双loop、backend全量和静态证据通过后恢复`VERIFIED/F3 (local only)`。
- Authorization：stage-two成功值只接受类型化`AuthorizationDecisionOutcome.ALLOW`；Evidence、AuthorizationProcessor和stage six逐层fail-closed，deny-like自定义依赖无法进入stage three，reason字符串不提供授权能力。
...
- 验收证据见 [P09 acceptance log](ns_runtime_acceptance_log_0.0.2.md#p09-routingplan-与本地路由验收证据)，长期决策见 [ADR-036](ns_runtime_architecture_decisions_0.0.2.md#adr-036)。仅本地routing达到F3；strong RoutingPlan persistence、master query与remote routing均未实现。
...
**阶段状态：`BLOCKED`**
**目标完成度：`F3`**  
**前置阶段：P10 `VERIFIED/F3 (local only)`；P11-W01继续`BLOCKED`**
...
| ID | 状态 | 实施内容 | 完成判定 |
|---|---|---|---|
| `P11-W01` | `BLOCKED` | prepared 激活器按批次、priority、tenant 和水位执行 prepared -> queued | 等待独立授权；不一次性激活大 fanout |
...
| `LOG-1` | NsLogger 单向注入/拥有 Sanitizer；JSON/text/color 在输出前处理消息、格式参数、extra、异常和安全 traceback 元数据，不直接序列化原始对象；调用方 extra 不得覆盖权威日志字段，冲突值只进入 `extra_fields`；可选显式配置快照和 log root 不读取 global `ns_config`，未提供时保持原兼容入口；实例 `close()` 幂等 flush/close 自有 handlers 并清除初始化所有权；可选多进程 handler 只在实际使用时加载 | P01/P02 | `VERIFIED` |
| `ERR-1` | NsErrorDefinition、稳定 severity/category、显式不可变错误注册表、20 域/78 码 `RUNTIME_*` 完整覆盖矩阵、20 个独立人工冻结设计场景（含 `RUNTIME_FEATURE_DISABLED`）、全 `RUNTIME_*` NACK 精确映射、精确类型保守策略、code/numeric_code/类唯一性与原异常序列化兼容；P08增加11个StateStore严格失败错误，P09增加`RUNTIME_ROUTE_REJECTED/200177` | P01/P02/P08/P09 | `VERIFIED` |
| `HTTP-1` | `NsHttpClientFactory` 独立 caller-owned 创建、`NsHttpClientOwner` open/closing/closed 生命周期与 legacy map 隔离；HTTP error 固定安全摘要、安全 URL、隔离快照 `NsHttpResponseSanitizer`、普通失败 fail-closed、bearer token header-only 与发送前 URL/query guard | P01 | `VERIFIED` |
| `OBS-1` | UTC、不可变、脱敏且严格 JSON-safe 的 metric/trace/diagnostic 记录；同步本地 `record()` 与异步 `flush()/aclose()` sink 协议；35 个标准 metric definition、显式有限 attribute schema、高基数别名防御、完整公开 record 字节门禁和有界线程安全内存实现 | P01 | `VERIFIED` |
| `TST-1` | `NsTestResourceFactory` 实例级临时目录/显式不可变配置/可控 clock/内存 sink；保持绑定的 OS 随机 TCP 端口；唯一 Redis namespace 的同步/异步 `SCAN` + 批量 `DELETE` 托管清理且禁止数据库级 flush；Factory 使用 OPEN → CLOSING → CLOSED 生命周期、可重试所有权和并发关闭屏障；`NsReservedPort` 只在底层 socket close 成功后转移所有权，直接 release 与 Factory close 共享 reservation 锁 | P01 | `VERIFIED` |
| `DEP-1` | common、backend、runtime 生产、runtime 测试、runtime 压测五层无环依赖清单；生产不引入测试/压测/QUIC 包，backend 不引入 runtime 包，uvloop 使用非 Windows marker | P01 | `VERIFIED` |
| `RTE-1` | `src/ns_runtime` 独立组件边界；`python -m ns_runtime.main` 是唯一模块进程入口，package 导入不得产生启动或 event loop policy 副作用 | P02 | `VERIFIED` |
| `RSL-1` | `ns_runtime.service.RuntimeService` 的 created/starting/running/stopping/stopped/failed 六态一次性生命周期；start 只允许从 created 开始且禁止 restart；stopped 后同 owner loop 的 stop 幂等；failed 禁止 start 但允许 stop 清理或重试，清理成功进入 stopped，stop 失败/取消后回 failed；critical monitor 非取消异常请求同一 shutdown coordinator 并从 RUNNING 进入 FAILED，显式 stop 成功后进入 STOPPED，reason/failed_tasks 保留历史；同 loop 操作串行，首次 loop owner 由同步锁原子绑定且跨 loop 在 async lock 前拒绝；hook 任意 `BaseException` 在状态更新后原对象穿透且不保存内容；非法迁移与 loop mismatch 使用稳定 `NsStateError` | P02 | `VERIFIED` |
| `RTC-1` | `ns_runtime.context.RuntimeContext` 的冻结关键字构造与显式依赖身份：NsConfig、Clock、Logger、MetricsSink、TraceSink、TaskSupervisor 为必需依赖；有限冻结 `RuntimeDependencySlots` 为 DiagnosticSnapshotSink、NsHttpClientOwner与StateStore提供可选类型化槽位；RuntimeService必须构造注入同一context；无默认依赖创建、全局context、字符串registry或ambient getter；有限规范类型边界使用真实类对象判断且不提供依赖实例locator | P02/P08 | `VERIFIED` |
| `RSP-1` | `ns_runtime._bootstrap` 从权威模块重导出默认配置路径、唯一 `NsConfig` 类型、startup 错误与路径常量，不初始化 global `ns_config`；`load_config_snapshot()` 强制显式路径并与 `inspect()`/`validate()`/`prepare()` 共用唯一安全错误归一化，普通配置错误保持 CFG-1 类型。`inspect()` 只读返回目录四态且不替换 policy，`validate()` 不替换 policy但保持既有目录准备语义，`prepare()` 只在环境、配置、安全、transport、依赖、TLS 和显式目录通过后安装 policy；当前只接纳 `websocket_tcp` 并检查 `websockets`，preflight 自身不创建 listener/adapter/StateStore/client/exporter。`ns_common(.config).ns_config` 仍在显式访问时返回同一真实 `NsConfig`，无参 load 语义不变；冷进程 `main()` 失败前不初始化该全局、不 ensure 目录，成功后才由 P04 composition root 构造并运行 TransportRuntimeService | P02/P04 | `VERIFIED` |
| `RRS-1` | `RuntimeService.role` 返回冻结 `RuntimeRoleSnapshot`；配置初始角色仅允许 singleton/sub_node/standby_master/active_master，transitioning/draining 为角色过渡域，healthy/degraded/isolated/unavailable 为独立健康域。P02 不暴露角色/健康迁移 API；transport、cluster_coordination、delivery capability 均固定 disabled，`require_capability()` 先记录固定安全审计字段再抛 `RUNTIME_FEATURE_DISABLED`。active_master 配置值不授予选主、leader lease、fencing 或协调写权限。production composition root 只在 preflight 成功后使用显式配置与 log root 构造带 Sanitizer 的 NsLogger | P02 | `VERIFIED` |
| `RSD-1` | `RuntimeShutdownCoordinator` 与 `RuntimeService` 必须共享同一 `RuntimeContext` 身份；shutdown固定按stop transport admission、stop logical admission/read loops、drain/close logical connections、drain/close transport、cancel supervisor tasks、close可选StateStore、flush/close sinks、close clients、写摘要和close logger执行。StateStore复用既有coordinator且在sink前关闭，不新增signal/shutdown/loop/supervisor owner；普通资源失败记录后继续，进程级异常穿透，重复关闭返回同一冻结报告 | P02/P04/P05/P08 | `VERIFIED` |
| `RLO-1` | `RuntimeEventLoopMonitor` 必须与 RuntimeService 共享同一 context，并使用 RSP-1 已选定的 asyncio/uvloop 实现；1 秒单调 deadline 采样不追赶旧 deadline，lag 历史最多 1024 项并以 nearest-rank 计算 P95/P99。冻结 snapshot 覆盖 implementation、lag、slow-threshold observation、pending/cancelled task、executor queue、probe/metric failure；8 个 OBS-1 标准指标仅使用既有有限 attributes。普通 clock/sink/probe 失败不终止 service，未知 count 为 None 且不写失真指标；monitor 正常取消不触发 failure，非取消异常请求 RSD-1 并使 service 先 FAILED，显式清理成功后 STOPPED，report 保留原因与 failed task；monitor 仍由唯一 TaskSupervisor 拥有并在 sink 关闭前取消，不创建 exporter、线程、端口或强一致记录 | P02 | `VERIFIED` |
| `RDI-1` | `python -m ns_runtime.main diagnose` 复用 RTE-1 唯一入口和 RSP-1 配置/安全/依赖/TLS/event-loop 选择语义；只读目录状态仅输出固定 role 与 accessible/access_denied/missing/not_directory，不输出路径。成功报告为冻结本地事实，配置或依赖错误只输出稳定 code/numeric_code 与 allowlist details，未知普通异常收敛为通用错误。诊断不准备目录、不安装 policy、不启动 RuntimeService/monitor/task/logger file/client/sink exporter、不开 listener/HTTP 管理端口、不访问远端或强一致状态 | P02 | `VERIFIED` |
| `ENV-1` | `ns_runtime.protocol` transport-independent `json.v1`：固定12分组与strict raw/normalized authority boundary、资源上限、exact schema、50项/13族不可变registry及feature/direction/processor/routing metadata、canonical bytes与SHA-256。TargetGroup精确冻结七种primary selector、六种strategy、五种rebind和对应count/epoch矩阵，capabilities为非空无重复AND集合；P05激活9项connection lifecycle，其余未实现能力统一feature-disabled | P03/P05/P09 | `VERIFIED` |
| `TC-1` | 22 项有限闭集 transport adapter conformance；所有 adapter 复用公共 harness，并提供 TLS/plaintext policy、完整文本边界、binary/invalid UTF-8/size、每-session queue/backpressure、exact receive terminal outcome、terminal keepalive failure、单一 send deadline、close cancellation 可重试所有权、FAILED start composition-root cleanup、并发/cancel、admission/drain/shutdown、禁用依赖隔离、安全诊断及 send success 不产生 runtime ACK 的专项证据 | P04 | `VERIFIED` |
| `SC-1` | frozen/slots/kw-only SessionContext保持既有logical/session/IAM边界且不含敏感/transport/task依赖；P05 logical owner在同一LocalConnectionIndex mutation原子发布state、active target与ELIGIBLE/RECONNECT_GRACE/DRAINING/AUTHORITY_SUSPENDED/SESSION_EXPIRY_SUSPENDED，resume仍epoch+1，reauth原子替换authority/index；Router只读取一次冻结snapshot，不反推lifecycle原因 | P05/P09 | `VERIFIED` |
| `IAM-R1` | `ns_common.iam` 冻结六类 principal、introspection/access/bootstrap/invalidation/payload_ref 类型；backend 负责最终 component/capability/tenant/role/version 裁决，并从 target/message.type 重算安全标志。runtime 只通过显式 P01 HTTP owner/client 携带 trace 和内部服务凭证；生产强制 IAM HTTPS、非占位两端凭证和 fail-closed。最小 PermissionSnapshot 下 strict 每条实时检查，cache 受 TTL/version/invalidation 约束且 tenant hard check 先于 permission；高风险、跨 tenant、新配置和全局协调写始终绕过缓存，backend unavailable 时拒绝。节点凭证缓存仅保存 AES-GCM ciphertext并复验签名/TTL/role；恢复必须重新验证 credential/role/config/lease/fencing/session snapshot，重验期间新失联使本代结果作废。本合同不实现 P07 pipeline、P08 StateStore/lease/fencing 或 P10 payload storage | P06 | `VERIFIED` |
| `PC-1` | normalized ProcessorContext、固定八阶段pipeline、实例registry、supervised timeout/cancellation、typed final audit/EventBus与local trusted plugin boundary；RoutingRequirement与RoutingPreparationResult冻结，stage six解析plan或typed short-circuit，feature-disabled不调用Router；response finalizer纯构造，final audit之后才由composition emitter发送，strong audit失败response为空且send为零；Processor contract不含StateStore/transport | P07/P08/P09 | `VERIFIED` |
| `SS-1` | backend-neutral StateStore authority、capability、typed namespace/key、scoped access与strong audit authority boundary；P08只激活strong audit，P05/P06/P07 owner不迁移，无global registry或具体provider | P08 | `VERIFIED` |
...
| P01 | ns_common 公共基础设施加固 | `VERIFIED` | F2 | P00 |
| P02 | Runtime 进程生命周期与事件循环 | `VERIFIED` | F2 | P01 |
| P03 | Envelope 协议层与类型注册表 | `VERIFIED` | F2 | P02 |
| P04 | Transport 抽象与 WebSocket/TCP | `VERIFIED` | F2 | P03 |
| P05 | 连接、会话、握手、心跳、Resume | `VERIFIED` | F3 | P04 |
| P06 | IAM、安全上下文与 backend 合同 | `VERIFIED` | F3 | P05 |
...
| P11 | 本地可靠投递调度与发送 | `BLOCKED` | F0 | P10 |
...
|---|---|
| 当前阻塞项 | 无 |
| 下一工作包 | `P11-W01` 保持 `BLOCKED`，等待独立授权；task.dispatch production feature直到P12 ACK闭环仍关闭 |
...
- P01-W12 与 P01-FIX-07 的错误矩阵由 P02-W05 以追加方式扩展为 19 域/66 码、20 个独立人工冻结设计场景；新增 `RUNTIME_FEATURE_DISABLED` 为 audit-required 的统一功能禁用错误，既有错误均未重编号或改变行为。13 个 NACK reason 映射保持不变；策略 metadata 只作用于精确异常类型且不是无条件执行指令，实际输出仍必须经过 sanitizer/logger/未来错误 Envelope 边界，见 [ADR-015](ns_runtime_architecture_decisions_0.0.2.md#adr-015) 和 [ADR-024](ns_runtime_architecture_decisions_0.0.2.md#adr-024)。
- P01-W13/P01-W14 已完成显式 HTTP factory/owner 生命周期、legacy getter 兼容边界、安全 body 摘要、response sanitizer 与 bearer token URL/日志/错误隔离，见 [ADR-016](ns_runtime_architecture_decisions_0.0.2.md#adr-016) 和 [ADR-017](ns_runtime_architecture_decisions_0.0.2.md#adr-017)。成功响应原始正文仍由业务调用方消费且不得直接进入诊断输出；无标签自由文本仍要求 response sanitizer 提供结构化语义。
- P01-W15/P01-FIX-08 已冻结不绑定 exporter 的 metrics/trace/diagnostic sink、关闭后的 best-effort 拒绝、`dropped_count`/`rejected_count` 分离、脱敏不可变记录、35 项显式有限 metric definition、高基数别名防御和完整公开 record 字节门禁，见 [ADR-018](ns_runtime_architecture_decisions_0.0.2.md#adr-018)。该公共基础不代表 runtime 指标采集、HTTP/OTLP exporter 或 P20 故障注入已经可用。
- P01-W16/P01-FIX-09 已完成 `ns_common.testing` 校准：Factory 的 OPEN/CLOSING/CLOSED 状态机、Factory close 屏障和可重试资源所有权均已验证；reservation 只在真实 socket close 成功后转移自身所有权，直接 release 与 Factory close 共享关闭屏障。Factory 不拥有 Redis client 或服务进程。P01-W17 的五层依赖清单保持 `VERIFIED`；Redis/Valkey 驱动仍只属于测试层，pyperf/psutil 只属于压测层，QUIC/WebTransport 依赖继续留给 P21。P02-W01 已建立 `src/ns_runtime` 与唯一模块入口；Envelope、transport、session、delivery、cluster 和 management 仍按后续阶段保持禁用或 F0。
- P02-W02/P02-FIX-01 已冻结 `RuntimeService` 六态一次性生命周期、稳定非法迁移错误、stopped 后 stop 幂等、failed 后显式清理/重试、hook 原异常穿透不保存，以及首次 loop owner 原子绑定，见 [ADR-021](ns_runtime_architecture_decisions_0.0.2.md#adr-021)。P02-W06 在 stop hook 成功后调用同一 context 的 coordinator；构造与 start 仍不创建或启动隐式依赖，stop 幂等与 failed 可清理语义保持不变。
- P02-W03/P02-FIX-02 已冻结 `RTC-1`：context 只冻结依赖引用与类型边界，不冻结依赖对象内部状态，也不等于 composition root 或资源生命周期 owner；可选 diagnostic/HTTP 槽位只保留显式接线位置，不创建 client/exporter。context 冷导入、非法 core 依赖和非法 optional HTTP owner 构造均不通过类型引用加载 `ns_common` 全局配置、HTTP/logger 链或访问文件系统；有限边界使用已加载的真实公共类对象而非类名或实例 locator，见 [ADR-022](ns_runtime_architecture_decisions_0.0.2.md#adr-022)。
- P02-W04/P02-FIX-03 已冻结并校准 `RSP-1`：`ns_runtime._bootstrap` 重用权威配置/错误对象且不触发 global config；`main.py` 显式解析配置路径并复用唯一安全错误归一化，冷进程 dependency/security 失败时不初始化 global、调用 ensure、创建目录、安装 policy 或加载 service。P02-W05 在 preflight 成功后才以显式配置与显式 log root 构造带 Sanitizer 的 `NsLogger`，不恢复 global config 副作用。该 P02 冻结当时没有 adapter/listener；现已由 P04 在同一 RSP-1 之后接入，但 preflight 本身仍不证明 listener、StateStore、IAM、角色协调或生产证书可用，完整 TLS 材料仍归 P20，见 [ADR-023](ns_runtime_architecture_decisions_0.0.2.md#adr-023)、[ADR-024](ns_runtime_architecture_decisions_0.0.2.md#adr-024) 与 [ADR-029](ns_runtime_architecture_decisions_0.0.2.md#adr-029)。
- P02-W05 已冻结 `RRS-1`：四个配置初始角色只形成只读本地状态，transitioning/draining 与健康附加状态保持独立建模但没有提前开放迁移；transport、cluster_coordination、delivery 查询统一审计并返回 `RUNTIME_FEATURE_DISABLED`。即使配置为 active_master，也不持有 leader lease/fencing，不执行全局协调写入；角色切换、强一致审计、transport 与 delivery 分别留给 P17/P08、P04 和 P10 以后阶段，见 [ADR-024](ns_runtime_architecture_decisions_0.0.2.md#adr-024)。
- P02-W06 冻结的 `RSD-1` 已由 P04-W10 通过单一类型化 `TransportLifecycleOwner` 兼容扩展：首次 shutdown reason 同步关闭 listener admission，随后 drain session/I/O、close adapter/listener，再继续既有 TaskSupervisor/sink/client/logger 相位；没有第二 coordinator、signal owner、supervisor 或 event loop。该 transport drain 不等于 P05 logical connection draining、通知重连、delivery 转移或管理通道，见 [ADR-025](ns_runtime_architecture_decisions_0.0.2.md#adr-025) 与 [ADR-029](ns_runtime_architecture_decisions_0.0.2.md#adr-029)。
- P02-W07 已冻结 `RLO-1`：monitor 只提供进程内冻结 snapshot 与 best-effort 标准指标，不是 exporter、管理查询或健康权威；slow callback total 当前定义为调度 lag 达到配置 slow-callback threshold 的运行观测计数，不解析 asyncio 日志。pending task 使用当前 loop、cancelled task 使用同一 TaskSupervisor，executor queue 无公开 API 时只做受控本地探针；失败明确为未知并累计，不伪装为零。W08 可只读该 snapshot，但不得让诊断命令启动第二个 monitor 或开放 HTTP 端口，见 [ADR-026](ns_runtime_architecture_decisions_0.0.2.md#adr-026)。
- P02-W08 已冻结 `RDI-1`：diagnose 只是唯一 main 模块入口下的本地只读启动检查，不是 P16 管理查询、P20 diagnostic snapshot exporter 或健康权威。它不读取运行中 monitor、不启动第二个 service/monitor、不准备目录或 policy，也不开放 socket/HTTP 端点；输出只含有限本地能力事实和脱敏稳定错误，见 [ADR-027](ns_runtime_architecture_decisions_0.0.2.md#adr-027)。
- P02-FIX-04 已校准 `RSD-1`：RuntimeSignalRegistration 在 asyncio/fallback 两条路径安装前保存 SIGINT/SIGTERM 的原 handler 对象，close 精确恢复且幂等；该修复不增加 signal owner 或 transport drain，见 [ADR-025](ns_runtime_architecture_decisions_0.0.2.md#adr-025)。
- P02-FIX-05 已校准 `RSL-1`/`RLO-1`：critical monitor 非取消异常请求同一 RSD-1 shutdown 并先使 service FAILED；后续显式 stop 成功后进入 STOPPED，reason 与 failed monitor task 由 shutdown report 保留，重复 stop 幂等。正常取消仍 STOPPED，普通 probe/metric/clock/sink 失败仍 fail-soft。没有第二 TaskSupervisor 或 shutdown task，见 [ADR-021](ns_runtime_architecture_decisions_0.0.2.md#adr-021) 与 [ADR-026](ns_runtime_architecture_decisions_0.0.2.md#adr-026)。
- P03 当时冻结 `ENV-1` 时，除标准 runtime.error 外所有 message family 确实保持 feature disabled；该历史 acceptance 事实不变。P05-W01至W14实现了 connection lifecycle 领域能力，`P05-FIX-01` 激活对应 9 项 canonical connection contracts。P07 已把当前已认证 ingress 统一接入 PC-1，并让未实现 task/delivery/stream/control/cluster/config/dead-letter/replay/cancel/hold/status 通过同一 pipeline 返回 feature disabled；P08/P10/P12/P17 前仍无 StateStore、DeliveryRecord、ACK 状态提交、可靠投递或集群权威，见 [ADR-028](ns_runtime_architecture_decisions_0.0.2.md#adr-028)、[ADR-030](ns_runtime_architecture_decisions_0.0.2.md#adr-030) 与 [ADR-032](ns_runtime_architecture_decisions_0.0.2.md#adr-032)。
- P07 的 `STRONG_REQUIRED` 是强一致需求标记而不是 durable 成功声明；当前 logging/test sink 普通失败会被记录为失败，高风险执行不会被标成成功，但真正不可丢控制审计必须等待 P08 authority store。EventBus 没有状态修改 API，subscriber timeout/exception 只影响 publish report。plugin 只接受本地显式 allowlist metadata 与 message-processor registration，不下载、不提供 runtime/sandbox，也不能注册通用前置阶段绕过 IAM/audit。
- `P05-FIX-02` 只校准 FIX-01 composition 暴露的 lifecycle regression：semantic hello parse 失败经同一 receiver 清理 credential 并关闭 candidate transport；drain 保持 DRAINING/read loop/control ingress 到 deadline 或显式 completion；transport close 失败或取消保留 CLOSING index/owner 并可 retry；receive、parse、IAM、协商共享单一 absolute monotonic handshake budget；IAM/rejected 普通失败在 supervised operation 内转为清洁 typed outcome，不把 credential traceback 留入 TaskSupervisor。该校准未改变 9 enabled/40 disabled、P03 exact schema、P05 lightweight processor、production fail-closed IAM、SC-1 或任何 P06+ 边界。
- `P05-FIX-03` 将尚未进入正式index的handshake/resume candidate纳入`ConnectionLifecycleManager`单 owner cleanup collection；close普通失败或取消只保留CLOSING candidate，由retry或既有manager drain继续，不把candidate伪装为active index。DRAINING期间reauth、heartbeat、expiry、protocol/shutdown终态先同步同一DrainService first reason、取消deadline，再由真实close结果决定completion signal与owner回收；后到DRAIN_TIMEOUT/SHUTDOWN不得覆盖首因。该内部所有权不含token、peer或transport ID日志/指标，也未增加global registry、supervisor或shutdown owner。
- `P05-FIX-04` 在 resume coordinator 已完成 mapping/index/accepted/ACTIVE/grace commit 后、任何下一个可取消 await 前，同步把新 context、transport 与新 grace 交给既有 logical owner，设置 post-commit activation marker并删除 candidate record。后续旧 expiry stop、heartbeat/expiry/read activation或shutdown发生失败/取消时只走 logical fail-close；CancelledError原对象穿透，transport close普通失败保留CLOSING index/owner且只允许`retry_cleanup()`继续，不重新建立candidate ownership。
- P09只把单进程本地路由提升到`VERIFIED/F3`。P09-FIX-03已把message binding与semantic IAM decision分离，以真实不同message ID测试证明semantic fingerprint独立性，并由可信policy invocation、类型化security/scorer合同及plan独立重算关闭公共构造绕过面。`RoutingConsistencyPolicy`、`RoutingPlanRecorder`与`StrongRoutingPlanAuthority`已冻结，但当前production composition没有strong RoutingPlan provider，strong-required请求稳定unavailable；ordinary recorder只接收`SafeRoutingProjection`。remote runtime/master query、stale topology、DeliveryRecord/attempt、ACK/NACK/Defer、retry/queue/timer、P14 health scoring、lease/fencing与cluster仍未实现，见[ADR-036](ns_runtime_architecture_decisions_0.0.2.md#adr-036)。
