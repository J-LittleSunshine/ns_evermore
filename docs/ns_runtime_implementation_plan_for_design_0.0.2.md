...
> 当前状态校准时间：`2026-07-20T21:36:57+08:00`
...
| 字段 | 当前值 |
|---|---|
| 当前阶段 | `P05 连接、会话、握手、心跳与 Resume` |
| 当前工作包 | `P05-FIX-04` |
| 当前工作包状态 | `VERIFIED` |
| 当前阶段状态 | `VERIFIED / F3` |
| 最近已验证阶段 | `P05 连接、会话、握手、心跳与 Resume` |
| 最近已验证工作包 | `P05-FIX-04 resume post-commit ownership handoff cancellation hardening` |
| 下一阶段 | `P06 IAM、安全上下文与 backend 合同`（暂停，`NOT_STARTED`） |
| 当前阻塞项 | 无；当前 WSL 验证使用 `/home/ns/.virtualenvs/ns_runtime` 与 `/home/ns/.virtualenvs/ns_backend` 隔离环境 |
| 设计基线版本 | `0.0.2` |
| wire codec | `json.v1` |
| 当前正式 transport | `websocket_tcp` adapter 为 `VERIFIED/F2`；P05 composition 已由 `P05-FIX-01` 闭环、`P05-FIX-02` 加固 indexed lifecycle、`P05-FIX-03` 补齐 pre-index candidate cleanup 与 draining terminal convergence、`P05-FIX-04` 补齐 resume post-commit ownership handoff cancellation，P06 前 ordinary production connection 继续 fail-closed |
...
| `src/ns_runtime` 实际能力 | `VERIFIED` | P02 生命周期、P03 `json.v1` Envelope、P04 WebSocket/TCP transport 与 P05 logical connection/session 已实现并冻结：canonical connection exact schema/feature、hello-first、测试 IAM、协议/capability、三层绑定、本地索引、accepted、lightweight lifecycle processor、safe rejected、双 heartbeat、drain、grace/resume/epoch、reauth/expiry、不可恢复关闭、安全 snapshot、显式 audit handoff 与真实 logical accept/read composition。P06 IAM client、P07 generic processor/durable audit、P08 StateStore、DeliveryRecord、路由/投递和 cluster/management 执行链路仍未开始；ordinary production connection 继续 fail-closed。 |
...
| 当前本地测试基线 | `VERIFIED` | P03 54/54、P04 59/59、P05 179/179；P03+P04+P05 联合 `Ran 292, OK`；runtime 环境按 DEP-1 排除未安装 Django 的 `test_cache` 后 P01-P05 全量 `Ran 642, OK (skipped=1)`；backend 环境根目录全量 `Ran 653, OK (skipped=46)`。两套 `pip check`、全树 `compileall`、`git diff --check`、TLS ResourceWarning/tracemalloc 专项与 P05 安全/依赖边界扫描通过。 |
| 当前状态校准时间 | `VERIFIED` | `2026-07-21T20:56:01+08:00` |
...
| Session、握手与恢复 | `src/ns_runtime/connection/` | SC-1 logical state/session、canonical hello-first、测试 IAM、协议/capability、binding/index、accepted/rejected、heartbeat、drain、grace/resume/epoch、reauth/expiry、non-resumable close、安全 snapshot、typed audit handoff、lightweight lifecycle processor 与单一 logical accept/read owner；production IAM保持fail-closed；single handshake budget、IAM outcome sanitization、pre-index candidate cleanup ownership、persistent drain 与 external terminal first-reason convergence 已加固 | P05 connection 179/179；真实 plaintext/TLS/resume/reauth/drain交叉loopback；runtime 642；backend 653 | `F3` | `VERIFIED`，P05-W01至W14、P05-FIX-01至FIX-03 |
...
|---|---|
| 当前阻塞项 | 无 |
| 下一工作包 | `P06-B01/P06-R01` 保持暂停且为 `NOT_STARTED`；恢复需新的显式指令 |
