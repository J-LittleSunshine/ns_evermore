...
> 当前状态校准时间：`2026-07-20T13:49:45+08:00`
...
| 当前工作包 | `P02-W04 启动时执行环境、依赖、目录、event loop、transport 配置、state store 生产限制和 TLS 前置校验` |
| 当前工作包状态 | `NOT_STARTED` |
| 当前阶段状态 | `IN_PROGRESS` |
| 最近已验证阶段 | `P01 ns_common 公共基础设施加固` |
| 最近已验证工作包 | `P02-FIX-02 修复 RuntimeContext 冷导入及构造期隐式全局配置副作用` |
...
| `src/ns_runtime` 实际能力 | `VERIFIED` | 已建立独立 package、无导入副作用的唯一 `main.py` 进程入口，以及 created/starting/running/stopping/stopped/failed 六态一次性 `RuntimeService`；start 仅允许从 created 开始且禁止 restart，stopped 后同 owner loop 的 stop 幂等，failed 禁止 start 但允许 stop 清理或重试，stop 失败/取消后回到 failed；同 loop lifecycle 操作串行，loop owner 由同步锁原子绑定且跨 loop 在 async lock 前稳定拒绝，hook 的任意 `BaseException` 原对象穿透且不保存内容；新增冻结、关键字构造的显式 `RuntimeContext`，持有不可变 `NsConfig` 引用、clock、logger、metrics、traces、TaskSupervisor 和有限类型化后续槽位，RuntimeService 构造时必须接收并保持该 context 身份；无模块级 context、字符串 registry 或 ambient getter。context 的具体类型引用使用 postponed annotations/`TYPE_CHECKING`，冷导入与合法/非法构造期类型验证均不 import `ns_common`、全局配置、logger 或 HTTP 模块，也不读取或创建文件系统资源；有限验证边界从已加载规范模块取得真实类对象执行 `isinstance`，同名伪造类不会通过，稳定 `NsValidationError` 不依赖 package facade 初始化。main 尚未接入 service，启动校验、资源关闭编排、协议、transport、session、processor、路由、delivery、stream、cluster、management 与真实 observability collector 仍为 F0 |
...
| 当前本地测试基线 | `VERIFIED` | 最近一次本地验收：P02-W03/FIX-02 RuntimeContext 专项 12/12，W01 入口加 W02/FIX-01 生命周期及 W03/FIX-02 context 联合 36/36，exceptions 公共契约 26/26；P01/runtime 加 P02 联合回归 `Ran 292, OK (skipped=1)`；backend 环境根目录全量 `Ran 303, OK (skipped=1)`，跳过项均为 WSL 下同一 Windows 专用用例；五份依赖清单内容未改变，runtime/backend 持久环境 `pip check` 通过；`compileall`、独立解释器 context 冷导入及非法 config/HTTP owner 构造期文件系统/配置/logger/HTTP/task/thread/event-loop 副作用专项、异常 facade/注册表/依赖边界、范围隔离和 `git diff --check` 门禁通过，命令见 acceptance log |
| 当前状态校准时间 | `VERIFIED` | `2026-07-20T13:49:45+08:00` |
...
| runtime 进程与事件循环 | `src/ns_runtime/__init__.py`、`src/ns_runtime/main.py`、`src/ns_runtime/service.py` | 独立 package 与唯一模块进程入口保持无副作用；`RuntimeServiceState` 冻结六态，start 只允许从 created 开始且禁止 restart，stopped 后 stop 幂等，failed 禁止 start 但允许 stop 执行或重试清理；同 loop start/stop 串行，首次 loop owner 由同步锁原子绑定，跨 loop 在 async lock 前稳定拒绝；hook 任意 `BaseException` 在状态进入 failed 后原对象穿透且不保存内容；main 集成、运行上下文、启动校验和信号关闭尚未实现 | `tests.test_runtime_main` 3/3、`tests.test_runtime_service` 21/21；P01/runtime 加 P02 联合回归 280 项；backend 根回归 291 项，均只有 1 个既有 Windows 专用跳过 | 进程入口 `F1`；service 生命周期 `F2`；loop 集成与上下文 `F0` | P02-W01/W02/FIX-01 `VERIFIED`；P02 `IN_PROGRESS` |
...
|---|---|
| 当前阻塞项 | 无 |
| 下一工作包 | `P02-W04 启动时执行环境、依赖、目录、event loop、transport 配置、state store 生产限制和 TLS 前置校验`（`NOT_STARTED`） |
