...
| 当前工作包 | `P01-W05 建立统一 TaskSupervisor` |
| 当前阶段状态 | `IN_PROGRESS` |
...
|---|---|---|---|---|---|
| 配置基础设施 | `src/ns_common/config.py` | backend/cache/log/runtime 总组及 17 个强类型 runtime 细分组的深度不可变快照、独立 metadata/生效模式、严格加载/序列化、兼容别名、固定来源优先级、backend 覆盖和已验证快照恢复已完成 | `tests/test_config.py` 35/35；根测试回归 60/60 | `F2` | `VERIFIED`，P01-W01/W02/W03 已完成 |
| 路径与生命周期 | `src/ns_common/paths.py`、`src/ns_common/async_runtime.py` | 仓库固定路径常量与目录创建；event loop selector 已支持 auto/asyncio/uvloop、Windows/Linux 策略、可选依赖失败和 restart_required 门禁；TaskSupervisor 与优雅关闭待 W05 | `tests/test_async_runtime.py` 14/14；根测试回归 60/60 | selector `F2`；TaskSupervisor `F0` | P01-W04 `VERIFIED`，进入 P01-W05 |
...
| `COM-002` | event loop 选择、任务监督和优雅关闭基础 | `ns_common.async_runtime` | `IN_PROGRESS`（selector 已验证，TaskSupervisor 待 W05） | P01-P02 |
...
- 隔离边界：未安装 uvloop、未变更依赖清单、未创建 `src/ns_runtime`、未启动 transport 或外部服务；Linux uvloop 分支通过注入的兼容 policy factory 验证，真实 Linux/uvloop workload 与性能比较保留至后续阶段。
