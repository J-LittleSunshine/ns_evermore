...
| 最近已验证工作包 | `P05-FIX-01 logical ingress/processor/protocol activation and handshake rejection alignment` |
...
| 设计基线版本 | `0.0.2` |
| wire codec | `json.v1` |
| 当前正式 transport | `websocket_tcp` adapter 为 `VERIFIED/F2`；P05 logical ingress/processor/protocol activation/composition 已由 `P05-FIX-01` 闭环，P06 前 ordinary production connection 继续 fail-closed |
...
| 当前本地测试基线 | `VERIFIED` | P03 54/54、P04 59/59、P05 163/163；P03+P04+P05 联合 `Ran 276, OK`；runtime 环境按 DEP-1 排除未安装 Django 的 `test_cache` 后 P01-P05 全量 `Ran 626, OK (skipped=1)`；backend 环境根目录全量 `Ran 637, OK (skipped=39)`。两套 `pip check`、全树 `compileall`、`git diff --check` 与 P05 安全/依赖边界扫描通过。 |
| 当前状态校准时间 | `VERIFIED` | `2026-07-21T18:59:32+08:00` |
...
|---|---|
| 当前阻塞项 | 无 |
| 下一工作包 | 无进行中工作包；`P06-B01/P06-R01` 已暂停并保持 `NOT_STARTED` |
