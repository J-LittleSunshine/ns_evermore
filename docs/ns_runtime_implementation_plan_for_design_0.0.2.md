...
> 当前状态校准时间：`2026-07-17T21:44:08+08:00`
...
| 当前本地测试基线 | `VERIFIED` | 最近一次本地验收：testing 专项 35/35、依赖清单门禁 7/7；P01/runtime 联合 `Ran 256, OK (skipped=1)`；backend 环境根目录全量 `Ran 267, OK (skipped=1)`，跳过项均为 WSL 下同一 Windows 专用用例；五份依赖清单内容未改变，既有四类仓库外全新环境安装证据保持有效，runtime/backend 持久环境 `pip check` 和真实 standalone Redis namespace 回归通过；`compileall`、facade/对象身份、冷启动依赖隔离、临时目录/测试进程残留、生产源码测试文件、仓库虚拟环境和 `git diff --check` 门禁通过，命令见 acceptance log |
| 当前状态校准时间 | `VERIFIED` | `2026-07-17T21:28:56+08:00` |
