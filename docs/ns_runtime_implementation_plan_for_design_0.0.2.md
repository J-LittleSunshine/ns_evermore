...
| 最近已验证工作包 | `P09 RP-1 post-submit contract review` |
| 下一阶段 | `P10 受理、Summary、去重与 Payload Reference`（`P10-W01`，`BLOCKED`，等待独立授权） |
| 当前阻塞项 | P10-W01按本轮范围保持阻塞；P09复核使用`/home/ns/.virtualenvs/ns_runtime_p09_rp1_review`与`/home/ns/.virtualenvs/ns_backend_p09_rp1_review`隔离环境完成 |
| 设计基线版本 | `0.0.2` |
| wire codec | `json.v1` |
...
| RoutingPlan 与调度 | `src/ns_runtime/routing/` | RP-1单进程本地routing：`RequestedRoutingIntent -> RoutingPolicyDecision -> trusted RoutingRequest`显式authority、per-message `AuthorizationDecisionEvidence`、一次index snapshot、target expansion、静态/动态过滤、确定性fallback.v1、fixed broadcast rebroadcast、previous-chain integrity，以及deeply immutable plan/failure。ordinary recorder仅接收安全投影；strong authority仅冻结接口且默认unavailable | policy/authorization/broadcast/previous-chain/type-invariant专项、5001 fanout、P03-P09联合、Linux双loop runtime与backend全量及禁止边界扫描；详见acceptance log | `F3`（local only） | P09-W01至W10及post-submit contract review `VERIFIED`；remote/master routing与strong persistence未启用 |
