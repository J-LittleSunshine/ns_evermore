...
| 当前工作包 | `P01-W11 扩展 ns_common.exceptions 为错误注册表` |
| 当前工作包状态 | `NOT_STARTED` |
| 当前阶段状态 | `IN_PROGRESS` |
| 最近已验证阶段 | `P00 本地仓库基线与实施账本`（2026-07-16） |
| 最近已验证工作包 | `P01-REF-01 将 ns_common.config 从单模块拆分为包` |
...
| 当前本地测试基线 | `VERIFIED` | 最近一次本地验收：配置专项与结构测试 48/48、logger 9/9、P01/runtime 联合 148/148、根目录全量 159/159；`compileall`、runtime/backend 两套环境 `pip check`、公共导出/循环导入/生产源码内部路径扫描、backend settings 导入和 `git diff --check` 通过；历史命令见 acceptance log |
| 当前状态校准时间 | `VERIFIED` | `2026-07-16T22:15:11+08:00` |
...
| 日志与脱敏 | `src/ns_common/security.py`、`src/ns_common/logger.py` | sanitizer 继续独占字段、路径、对象、fail-closed、严格 JSON-safe 与 digest 资源规则；`NsLogger` 单向拥有或接收显式 `Sanitizer`，JSON/text/color formatter 在格式化前处理消息、参数、extra/类 Envelope、异常和安全 traceback 元数据，不直接字符串化或序列化原始对象；peer/client/remote address 继续完全脱敏 | Windows logger 9/9、sanitizer 30/30；runtime 联合 140/140；根测试 151/151 | sanitizer、logger 接入均为 `F2` | P01-W09/P01-FIX-03/P01-FIX-04/P01-W10 `VERIFIED` |
...
| `P01-W10` | `VERIFIED` | 把 sanitizer 接入 `NsLogger` 的 extra 和 exception 输出路径；禁止 formatter 直接序列化原始 Envelope | 泄露扫描为零；普通日志行为保持兼容 |
...
| `LOG-1` | NsLogger 单向注入/拥有 Sanitizer；JSON/text/color 在输出前处理消息、格式参数、extra、异常和安全 traceback 元数据，不直接序列化原始对象 | P01 | `VERIFIED` |
...
|---|---|
| 当前阻塞项 | 无 |
| 下一工作包 | `P01-W11 扩展 ns_common.exceptions 为错误注册表` |
...
- P01-REF-01 已完成纯结构拆分，`CFG-1` 契约内容、默认值、校验语义、错误类型和稳定 details 保持不变；P01-W11 仍为 `NOT_STARTED`。
- P01-W11 至 P01-W17 尚未完成；`src/ns_runtime`、强一致 StateStore、Envelope、transport、session、delivery、cluster 和 management 仍按后续阶段保持禁用或 F0。
