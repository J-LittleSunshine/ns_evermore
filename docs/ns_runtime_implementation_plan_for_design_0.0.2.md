...
> 当前状态校准时间：`2026-07-20T15:05:49+08:00`
...
| 最近已验证工作包 | `P02-W04 启动前环境、依赖、目录、event loop、transport、StateStore 与 TLS preflight` |
...
| `src/ns_runtime` 实际能力 | `VERIFIED` | 已建立独立 package、保持冷导入无副作用的唯一 `main.py` 进程入口、created/starting/running/stopping/stopped/failed 六态一次性 `RuntimeService`，以及冻结、显式注入的 `RuntimeContext`。新增同步 `RuntimeStartupPreflight`：严格解析 local/dev/test/prod，复用不可变配置校验并把 TLS/生产 StateStore 违规归一为稳定 startup security 错误；只允许当前 `websocket_tcp` 配置基线，未来 HTTP/3/WebTransport/QUIC 误启用稳定 feature gate；检查 `websockets`、服务端 TLS context 能力、data/etc/log/tmp 与 SQLite parent 可访问性；生产仅接受 Redis/Valkey 配置；所有检查通过后才选择或安装 event-loop policy。冻结结果只公开环境、loop 选择、配置化 adapter、StateStore backend、已检查依赖与目录角色，不公开 URL、真实路径或底层异常。main 现以函数内延迟 import 加载配置、构造显式 context、执行 preflight，再通过所选 policy 启动并停止无监听 RuntimeService；缺少 runtime 生产依赖时启动失败。尚未创建 listener、transport adapter、StateStore、HTTP client/exporter、角色状态、信号关闭编排、loop lag collector、协议或消息链路。 |
...
| 当前本地测试基线 | `VERIFIED` | 最近一次本地验收：P02-W04 startup preflight 专项 14/14，W01 入口、W02/FIX-01 生命周期、W03/FIX-02 context 与 W04 startup 联合 51/51；runtime 环境 P01/P02 非 Django cache 联合回归 `Ran 307, OK (skipped=1)`，backend 环境根目录全量 `Ran 318, OK (skipped=1)`；唯一跳过为 WSL 下 Windows 专用 event-loop policy 用例。backend 环境未混装 runtime 生产依赖，入口测试确认缺少 `websockets` 时稳定 fail-closed；runtime 环境入口成功并使用真实 Linux auto/uvloop policy。五份依赖清单内容未改变，两套持久环境 `pip check`、`compileall`、入口/context 冷导入、配置/依赖/目录/TLS 安全错误、禁止 listener/StateStore/client/exporter 范围扫描和 `git diff --check` 门禁通过，命令见 acceptance log。 |
| 当前状态校准时间 | `VERIFIED` | `2026-07-20T15:05:49+08:00` |
...
| `RSP-1` | `ns_runtime.startup.RuntimeStartupPreflight` 的同步启动边界：`validate()` 不替换 policy，`prepare()` 只在严格环境、完整配置、生产 TLS/Redis-Valkey 限制、transport feature gate、固定 Python 依赖、服务端 TLS 能力和显式目录全部通过后安装 event-loop policy；结果冻结且不公开 URL、真实路径或底层异常；当前只接纳 `websocket_tcp` 配置并检查 `websockets`，不创建 listener/adapter/StateStore/client/exporter；`ns_runtime.main` 保持冷导入无副作用并在 runtime 依赖齐全时按此顺序运行无监听 RuntimeService | P02 | `VERIFIED` |
...
- P02-W04 已冻结 `RSP-1`：`main.py` 现在通过函数内延迟 import 构造最小显式 context，完成同步 preflight 后才安装 policy 并运行一次无监听 service 生命周期；入口模块冷导入仍不加载 `ns_common`、startup/context/service、uvloop 或 websockets。preflight 只证明启动配置、固定依赖、目录与本机 TLS 能力，不证明 WebSocket listener、Redis/Valkey StateStore、IAM、角色协调或生产证书可用；`websocket_tcp` 只是当前可接纳配置基线，P04 前没有 adapter/listener，证书、私钥、CA、TLS 版本与 cipher 验证仍归 P20。backend 环境按 `DEP-1` 不安装 runtime 生产依赖，入口在该环境稳定返回缺失 `websockets`，而 runtime 环境完成真实 auto/uvloop 无监听启动，见 [ADR-023](ns_runtime_architecture_decisions_0.0.2.md#adr-023)。
