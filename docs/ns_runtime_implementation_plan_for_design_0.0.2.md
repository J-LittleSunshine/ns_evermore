...
| 最近已验证工作包 | `P01-FIX-06 校准错误注册表中的通用异常策略语义` |
...
| 当前本地测试基线 | `VERIFIED` | 最近一次本地验收：exceptions 23/23、W11 指定专项联合 154/154、P01/runtime 联合 173/173、根目录全量 184/184；`compileall`、runtime/backend 两套环境 `pip check`、公共导出、精确类型查询、冷启动导入、循环/内部路径扫描和 `git diff --check` 通过；历史命令见 acceptance log |
| 当前状态校准时间 | `VERIFIED` | `2026-07-17T00:29:45+08:00` |
...
| 公共异常与错误注册 | `src/ns_common/exceptions/` | 33 个现有异常按 base/common/protocol/payload_ref/delivery/cluster 分组；冻结 `NsErrorDefinition`、稳定 severity/category、显式不可变注册表和 NACK 映射完整性验证；通用错误保守、精确叶子显式开启副作用策略，查询不做 MRO fallback；原 facade、构造、继承与四字段序列化兼容 | `tests/test_exceptions.py` 23/23；33 项完整策略矩阵及类/code/numeric_code 唯一；调用事实、冷启动导入和依赖扫描通过 | `F2` | `VERIFIED`，P01-W11/P01-FIX-06 已完成；完整错误覆盖矩阵待 W12 |
...
| `ERR-1` | NsErrorDefinition、稳定 severity/category、显式不可变错误注册表、精确类型保守策略、code/numeric_code/类唯一性与原异常序列化兼容 | P01 | `VERIFIED` |
...
- P01-FIX-06 已完成通用异常的保守副作用策略校准；P01-W11 的包结构、异常兼容和注册表机制保持不变，未补齐、重编号或改写 P01-W12 的错误覆盖矩阵。策略 metadata 只作用于精确异常类型且不是无条件执行指令；`safe_detail` 只是策略元数据，实际输出仍必须经过 sanitizer/logger/未来错误 Envelope 边界，见 [ADR-015](ns_runtime_architecture_decisions_0.0.2.md#adr-015)。
