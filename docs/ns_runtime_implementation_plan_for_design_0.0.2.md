...
| 当前本地测试基线 | `VERIFIED` | P07 blocker 修复专项 `Ran 18, OK`；P07 与 connection/session 相关联合 `Ran 89, OK`；runtime 边界全量 `Ran 683, OK (skipped=1)`；backend 全量 `Ran 694, OK (skipped=49)`。全树 `compileall`、两环境 `pip check`、冷导入、`git diff --check`、P08/可靠投递、第二 owner、transport bypass、global processor state 与敏感事件/审计字段扫描通过。 |
...
| Processor 与插件 | `src/ns_runtime/processor/`、`src/ns_runtime/connection/processors.py` | PC-1 normalized context 和显式依赖；安全、授权、限流接口、幂等接口、审计标记、路由接口、message、response 显式固定八阶段；`MessageProcessor` binding 只能由 `MessageProcessorStageProcessor` execution boundary 进入标准 stage map；实例 registry 按 type/stage/version/feature flag 冲突检测；existing lifecycle/disabled processor adapter；typed final audit、EventBus 与仅本地 allowlist plugin metadata/namespace 权限边界。P08 strong storage、真实 limiter/idempotency/routing、远程 plugin runtime 均不存在 | P07 blocker 专项 18/18；connection/session 相关联合 89/89；runtime 683/683；backend 694/694 | `F3` | P07 review blocker 已关闭并重新 `VERIFIED`；PC-1 冻结 |
...
| `P07-W07` | `VERIFIED` | 内部 EventBus 支持类型化事件、订阅、超时和异常隔离 | 订阅者失败不影响主链路 |
