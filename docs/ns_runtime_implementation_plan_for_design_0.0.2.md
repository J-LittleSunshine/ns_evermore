...
| 字段 | 当前值 |
|---|---|
| 当前阶段 | `P06 IAM、安全上下文与 backend 合同` |
| 当前工作包 | `P06-B01` |
| 当前工作包状态 | `NOT_STARTED` |
| 当前阶段状态 | `NOT_STARTED` |
| 最近已验证阶段 | `P05 连接、会话、握手、心跳与 Resume` |
| 最近已验证工作包 | `P05-W14 async safe session snapshot and audit aggregation` |
| 下一阶段 | `P07 Processor、插件、事件与审计` |
| 当前阻塞项 | 无；当前 WSL 验证使用 `/home/ns/.virtualenvs/ns_runtime` 与 `/home/ns/.virtualenvs/ns_backend` 隔离环境 |
| 设计基线版本 | `0.0.2` |
| wire codec | `json.v1` |
| 当前正式 transport | `websocket_tcp` adapter 为 `VERIFIED/F2`；显式测试 IAM 下 P05 logical lifecycle 为 `VERIFIED/F3`，P06 前 ordinary production connection 继续 fail-closed |
...
| `src/ns_runtime` 实际能力 | `VERIFIED` | P02 生命周期、P03 `json.v1` Envelope、P04 WebSocket/TCP transport 与 P05 logical connection/session 已实现并冻结：hello-first、测试 IAM、协议/capability、三层绑定、本地索引、accepted、双 heartbeat、drain、grace/resume/epoch、reauth/expiry、不可恢复关闭、安全 snapshot 与显式 audit handoff。P06 IAM client、P07 processor/durable audit、P08 StateStore、DeliveryRecord、路由/投递和 cluster/management 执行链路仍未开始；ordinary production connection 继续 fail-closed。 |
...
| 当前本地测试基线 | `VERIFIED` | P05 connection 专项 `Ran 151, OK`；runtime 环境按 DEP-1 排除未安装 Django 的 `test_cache` 后 P01-P05 联合 `Ran 611, OK (skipped=1)`；backend 环境根目录全量 `Ran 622, OK (skipped=32)`。两套 `pip check`、全树 `compileall`、`git diff --check` 与 P05 安全/依赖边界扫描通过。 |
| 当前状态校准时间 | `VERIFIED` | `2026-07-21T17:50:28+08:00` |
...
| Envelope 与类型注册 | `src/ns_runtime/protocol/` | ENV-1 grouped `json.v1`、raw/normalized authority、resource limits、schema/version/registry/extensions、canonical bytes、ERR-1 与 feature-disabled dispatch metadata | P03 52项专项及 P01-P05 联合回归 | `F2` | `VERIFIED`，P03 冻结 |
...
| Session、握手与恢复 | `src/ns_runtime/connection/` | SC-1 logical state/session、hello-first、测试 IAM、协议/capability、binding/index、accepted、heartbeat、drain、grace/resume/epoch、reauth/expiry、non-resumable close、安全 snapshot 与 typed audit handoff；production IAM保持fail-closed | P05 connection 151/151；runtime 611；backend 622 | `F3` | `VERIFIED`，P05-W01至W14 |
| Processor 与插件 | 无通用实现 | 无 processor pipeline、plugin或event bus；P05只有显式typed lifecycle audit handoff，不是P07 processor final audit或P08 durable storage | 路径扫描与普通heartbeat零audit测试 | `F0` | `NOT_STARTED`，目标 P07 |
...
| `RSD-1` | `RuntimeShutdownCoordinator` 与 `RuntimeService` 必须共享同一 `RuntimeContext` 身份；首次 SIGINT/SIGTERM/显式/critical task 原因胜出并同步关闭本地与类型化 transport admission gate，shutdown 固定按 stop transport admission、drain sessions/I/O、close adapters/listeners、cancel supervisor tasks、flush/close sinks、close 显式 HTTP owner、写安全有界摘要、close owned logger 的顺序执行。P04 只通过单一 `TransportLifecycleOwner` 兼容扩展既有 coordinator，不新增 signal/shutdown/loop/supervisor owner。signal registration 精确恢复原 handler 且幂等；普通资源失败只记录 phase/resource/error type 并继续，进程级异常穿透；重复关闭返回同一冻结报告。连接迁移、Envelope 和管理旁路仍不属于本合同 | P02/P04 | `VERIFIED` |
...
| `ENV-1` | `ns_runtime.protocol` transport-independent `json.v1`：固定 12 分组与 strict raw/normalized authority boundary；1 MiB/32 depth/65536 string/4096 container/100000 node/signed-64/finite-number limits；base + exact type schema；major-strict explicit compatibility/schema matrix；50 项/13 族不可变 registry 及完整 dispatch metadata；direct extension namespace registry；ERR-1 + SEC-1 标准 runtime.error；sorted compact UTF-8 canonical bytes 与 SHA-256；49 项 disabled 类型统一安全审计并只返回 `RUNTIME_FEATURE_DISABLED`，无 callback/stub success | P03 | `VERIFIED` |
...
| `SC-1` | frozen/slots/kw-only SessionContext：logical connection/session/epoch、identity/tenant/component、negotiated protocol/schema/codec/capability、permission ref/digest/version、IAM mode/issued/expiry/resume eligibility、established/created time；不含token、permission mapping、IAM raw、transport/path/peer、task/sink/client/callback | P05 | `VERIFIED` |
...
|---|---|
| 当前阻塞项 | 无 |
| 下一工作包 | `P06-B01 runtime IAM principal contract`（`NOT_STARTED`） |
...
- P03 已冻结 `ENV-1`，但注册不等于业务可用：除标准 runtime.error 构造外，connection/task/delivery/stream/control/cluster/config/dead-letter/replay/cancel/hold/status 全部继续 feature disabled。P04 已提供 transport adapter/listener 与完整 text message 边界，但 P05/P06 前仍无 logical session/IAM 权威建立，P07/P08 前无通用 pipeline/强审计，P08/P10/P12/P17 前无 StateStore、DeliveryRecord、ACK 状态提交或集群权威；不得把 transport send、canonical bytes、registry metadata 或 best-effort audit log 解释为这些能力已实现，见 [ADR-028](ns_runtime_architecture_decisions_0.0.2.md#adr-028) 与 [ADR-029](ns_runtime_architecture_decisions_0.0.2.md#adr-029)。
