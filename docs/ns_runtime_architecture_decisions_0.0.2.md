...
- ADR 编号：`ADR-039`
- 状态：`ACCEPTED`（P11 local reliable scheduling/write boundary）
...
- 决策：queued -> sending、DeliveryAttempt create以及root/shard Summary queued/sending计数必须在同一StateStore transaction提交；transport调用前authority必须已经是sending且attempt为writing。进入sending时记录ack_deadline，write timeout必须小于owner lease TTL。write成功只把attempt转write_succeeded并把DeliveryRecord转ack_waiting；不存在sent或sent_success状态，ack_waiting持续占inflight。write error、write timeout和shutdown interruption只产生bounded typed failure并原子转write_failed；retry_scheduled只保留为公共enum的拒绝构造placeholder，不存在production transition、worker或budget，dead_lettered不实现。
