...
- ADR 编号：`ADR-053`
- 状态：`ACCEPTED`（authority 稳定性 barrier、test-only 诊断与 service cleanup runner）
- 背景：全仓高负载执行中，短至0.6秒的test session certificate、固定0.42秒sleep、固定0.25秒worker delay与3秒broker channel timeout发生竞争，偶发把测试调度抖动表现为`attestor_unavailable`或`broker_unavailable`；单项复跑不能区分certificate边界、child退出、提前cleanup、IPC timeout或attestor timeout。composition曾通过一次性`asyncio.run()`执行service；cleanup连续无进展后虽然保存了绑定原loop的lease，但`asyncio.run()`随即关闭该loop，使outer同步cleanup拿到必然`service_cleanup_loop_mismatch`的伪可重试owner。
...
- 决策（service owner）：StateStore成功open且service即将尝试start时，composition把transport、task supervisor、logger和共享broker-backed StateStore/repository graph一次性移交给service。同步runner一次性创建并显式持有原event loop，在loop内创建close-only lease，并保存运行异常；稳定`cleanup_pending_no_progress`异常持有该仍可执行runner及`cleanup_pending`、`service_stopped`和owner-active事实。outer composition只调用runner的同步`close()`，runner再以原loop执行`lease.close()`；错误loop直接调用lease继续fail closed。只有cleanup完成且service为`STOPPED`，或进入明确不可恢复终止状态后，runner才能关闭loop。成功phase不重复，service已接管的transport/task/logger/StateStore/broker graph不回到outer owner，也不创建第二loop、RuntimeService或资源owner。
