...
- 阶段冻结：P03-W01 至 W11 与 P03-FIX-01 已在 WSL/runtime/backend 联合回归后达到 `VERIFIED/F2`。`ENV-1` 只冻结协议入口、类型/registry/错误/serialization 与 disabled 行为；P04-W01 保持 `NOT_STARTED`，不得由本 ADR 推断 transport、session、IAM、StateStore、delivery 或 cluster 已开始。
