...
- 后果：并发重复认证最多一个进入 authenticated；active 上的 drain/close 按锁顺序线性化，close 可从 draining 继续完成且无法恢复 active。handshake total deadline 已覆盖协商，协商失败不会进入 active。SC-1 的 SessionContext 和三层 identity mapping 已增量冻结，但 index/lifecycle 部分仍由 W06-W14 完成。
- 阶段状态：`P05-W01` 至 `P05-W05 VERIFIED`；SC-1 为 `IN_PROGRESS`，索引、accepted、heartbeat、grace/resume、reauth 与 snapshot/audit 尚未完成，P06 真实 IAM backend/client 保持 `NOT_STARTED`。
