...
- 决策：配置分别承载backend、禁止userinfo的endpoint、username、password source reference、namespace和operation timeout。password只允许从env/file/显式none source解析，endpoint/config/options/provider repr均不显示secret；provider不记录底层异常文本、连接参数或credential，也没有audit/log输出。Redis/Valkey driver属于runtime生产依赖而非cache或test-only authority。
...
- 决策：runtime composition root创建provider但不打开；既有单一service运行路径在start前open，既有RuntimeShutdownCoordinator在sink/client/logger前close。provider不创建TaskSupervisor、event loop、thread、retry worker或第二shutdown owner。测试清理只SCAN并UNLINK本次随机namespace，生产provider不暴露清库API。
- 后果：P10不再受“仓库没有Redis/Valkey production StateStore provider”限制；standalone Redis/Valkey上的DR-1 admission authority可以配置和注入。该结论不包含Sentinel、Cluster、failover、replica read、lease/fencing、leader election、TLS材料验收、P11 send、P12 ACK/retry或任何cache authority。
- 关联阶段/工作包：`P08`、`P10-FIX-01`、`P10-W01`至`P10-W14`、`P11`、`P12`、`P17`、`P20`、`P22`。
