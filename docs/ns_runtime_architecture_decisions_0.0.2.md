...
- 决策：公共观测边界由 `MetricsSink`、`TraceSink` 和 `DiagnosticSnapshotSink` 三个可显式注入的结构化协议组成。`record()` 是同步、本地、不得执行外部 I/O 的接收边界并返回是否接受记录；实现需要远程输出时必须先进入实现自有缓冲，再通过异步 `flush()`/`aclose()` 管理资源。记录对象在进入 sink 前形成 timezone-aware UTC、深度不可变、经公共 `Sanitizer` 处理且可用 `allow_nan=False` 严格编码的快照；普通 sanitizer 失败转为稳定 fail-closed 状态，`KeyboardInterrupt`、`SystemExit` 等进程级异常保持穿透。metric 只接受有限标量标签，禁止 connection/session/transport/path/message/summary/delivery/stream/plan/operation/trace/span/request/correlation 和原始 tenant ID 等高基数键；受控 tenant 维度使用 `tenant_scope`。设计文档列出的 8 个 event-loop、10 个通用 transport 和 17 个 QUIC/WebTransport 指标名在公共层预留。内存实现线程安全、有界，容量满时丢弃最旧记录并累计本地 dropped count；其记录快照、清理和幂等关闭只服务测试与本地诊断，不构成持久化或投递成功证明。
- 后果：P02 composition root 必须创建并显式注入 sink，在停止新任务后调用其 `flush()`/`aclose()`；深层 runtime 模块不得获取全局 sink。P20 可增加 OTLP、Prometheus、backend 推送或其他 adapter，但不得修改公共记录安全边界、在同步 `record()` 中发起网络 I/O，或把 exporter 成功解释为强一致状态提交。trace 和按需 diagnostic snapshot 可以携带高基数标识，但仍受脱敏与大小边界约束；普通 metric 标签不得绕过高基数门禁。未启用 adapter 时标准指标可以不注册或为空，不能因此宣称对应 transport 已实现。
- 关联阶段/工作包：`P01-W15`、`P02`、`P04`、`P05`、`P07`、`P14`、`P20`、`P21`。
