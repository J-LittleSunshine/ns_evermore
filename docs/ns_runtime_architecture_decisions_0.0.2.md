...
- ADR 编号：`ADR-032`
- 状态：`ACCEPTED`（P07 阶段已冻结）
- 背景：P05 lightweight lifecycle registry 只能调度 heartbeat/drain/reauth，P03 feature-disabled processor 仍由 connection read loop 旁路调用；P06 已提供 IAM-R1 message authorization，但尚无统一 timeout、final audit、EventBus 或 plugin boundary。P07 必须把这些能力收敛到单一 ingress，同时禁止提前实现 P08 StateStore、可靠投递和 RoutingPlan。
- 决策：冻结 `PC-1`。`ProcessorContext` 精确包含 runtime authority 已注入的 normalized Envelope、SC-1 session、typed trace reference、config/policy version、Clock 和有限显式 `ProcessorDependencies`；依赖包含 authorization、rate-limit interface、idempotency interface、routing-preparation interface、response finalizer、error mapper、IAM principal type、AuditSink、EventBus 与既有 TaskSupervisor。context、registry、EventBus 和 AuditSink 都是 composition-owned 实例，不允许 global dict、global sink、global bus、global config/IAM/cache 或 service locator。
- 决策：pipeline 顺序显式固定为 security validation、authorization、rate-limit entry、idempotency precheck、audit marker、routing preparation、message processor、response finalize。`MessageProcessor` 是消息特定 binding contract，必须由标准 `MessageProcessorStageProcessor` execution boundary 包装后进入 stage map；标准构造器必须返回完整八阶段，不能依赖 composition 另行补注册。`ProcessorRegistry` 只通过显式 registration 按 message.type、stage、同 major 有界 protocol version 和 feature flag/state 解析；duplicate exact registration、重叠 version、同时匹配多个 feature flag 和缺失 flag resolution 均 fail-closed。当前 rate-limit、idempotency 与 routing 只冻结无状态接口，不建立 limiter store、idempotency store、target selection 或 RoutingPlan。
- 决策：每次 pipeline 执行由 composition 传入全局唯一 execution sequence，并通过 RSD-1 同一个 TaskSupervisor 创建一个 supervised execution task；不得创建第二 supervisor、event loop 或 shutdown owner。timeout 取消该 task并映射 `RUNTIME_PROCESSOR_TIMEOUT`；调用方 cancellation 原样穿透；已登记 runtime error 保持精确类型，未知普通异常只映射不含原异常文本/details/cause 的 `RUNTIME_PROCESSOR_FAILED`。任何失败立即截断后续 stage，severe protocol violation 仍由既有 lifecycle owner发送安全错误后关闭，普通 processor failure被隔离在当前消息。
- 决策：每条进入 PC-1 的可执行消息无论 success、reject、timeout、unknown failure 或 cancellation，都只尝试一次 typed final audit。record 只包含 `ProcessorSafeSummary`、processor、闭集 action、稳定 error code、typed trace、config/policy version、required consistency 与 UTC time；不接收 Envelope、payload、token、credential、permission mapping 或 IAM response。普通 sink failure不覆盖业务结果；`STRONG_REQUIRED` sink failure不得返回 pipeline success。该字段只是强一致要求，不声明 logging/test sink durable，真正不可丢控制审计仍必须由 P08 authority store实现。
- 决策：进程内 `EventBus` 只接受 typed event，event 精确只含 object id、safe summary 和 trace reference。`subscribe` 返回不可变、bus-instance scoped `SubscriptionHandle`；`unsubscribe` 返回闭集 `REMOVED/NOT_FOUND`，重复、缺失或其他 bus handle 均安全。publish 启动时冻结 subscriber snapshot，退订不取消已经启动的 subscriber，只影响后续 publish。subscriber task复用同一 TaskSupervisor；单 subscriber timeout/exception只形成 publish report，不传播为主链路失败，也不提供状态修改或 authority API。事件丢失不得影响任何当前状态；后续 worker 被唤醒后仍必须重新读取 P08 authority。
- 决策：plugin 只建立显式本地可信 allowlist。metadata 冻结 dotted namespace、exact extension schema、permission tuple、positive timeout、精确 `plugin.<namespace>` state namespace 和 feature flag；duplicate namespace、invalid schema、permission/namespace/flag 未授权均拒绝，registration batch 先完整校验再原子发布。plugin registration 只能进入 message processor stage，不能注册 security/authorization/audit/routing 前置 stage；本阶段不下载代码、不提供 plugin runtime、sandbox、远程 discovery 或私有状态 backend。
- 决策：P03 inbound codec/schema 仍先拒绝 source/auth_context forging，随后 connection composition 通过 `RuntimeAuthority` 注入 source/auth_context并形成 normalized Envelope。P05 lifecycle processor 与 P03 feature-disabled processor都作为 PC-1 message stage adapter；heartbeat、health、reauth、drain、task/management 等 disabled message 不再由 transport callback直接执行业务。`HandshakeIamAuthority` 追加 IAM-R1 `principal_type` 并在 detached copy中保留，用于显式构造 P07 PermissionSnapshot；logical owner冻结初始 principal type，resume/reauth出现类型漂移时 fail-closed；SC-1 frozen field set保持不变。
- 后果：当前已认证 ingress 具备固定、可测试、可取消、可超时、可审计的统一 processor 链；health 与其余未启用能力仍返回稳定 `RUNTIME_FEATURE_DISABLED`，不会产生 delivery、ACK 状态或伪成功。EventBus 与 local plugin不能成为旁路权威。P08 必须消费 `STRONG_REQUIRED` 而不是把普通 logging sink误作 durable audit，并继续保持单 TaskSupervisor/loop/shutdown owner。
- 复审修订：首次冻结后发现 `build_standard_stage_processors()` 只返回七个通用阶段，production composition 通过额外 registration 补入 message handler，而手工八阶段测试掩盖了标准 contract 缺口。现已以强制 message binding 参数、固定 stage wrapper、完整标准 map、missing processor 稳定失败与 plugin wrapper 门禁关闭该 blocker。
...
- ADR 编号：`ADR-036`
- 状态：`ACCEPTED`（P09 RP-1 local routing contract）
- 背景：P09需要在不引入可靠投递、远端拓扑或具体StateStore provider的前提下，把ENV-1 target、P05 lifecycle eligibility和PC-1第六阶段收敛为可复现、可解释的本地决策。如果Router跨多个query拼接状态、从transport或expiry反推eligibility、在processor或finalizer中发送响应，或者用普通日志伪装strong persistence，就会破坏既有owner、audit-before-send与P08 authority边界。
- 决策：冻结`RP-1`。`TargetGroup`恰有一个primary selector，支持connection、identity、tenant、capability、component_type、runtime与tenant-scoped broadcast；strategy闭集为single、all、broadcast、quorum、all_required、weighted_subset，rebind闭集为fixed_connection、same_identity、same_capability、same_tenant、no_rebind_for_control。capabilities为非空无重复AND集合；strategy、count、epoch与selector的无效组合在ENV-1拒绝。sender wire值先形成`RequestedRoutingIntent`，再由显式`RoutingPolicy`生成accepted/rejected/tightened `RoutingPolicyDecision`，最后才允许构造trusted `RoutingRequest`；Router不得接收未经decision的sender intent。默认普通本地策略可以显式接受，control、management、config、cluster、security-audit或management-capability合同必须依赖可信`MessageTypeContract`/risk metadata收紧为`no_rebind_for_control`，不得依赖wire category字符串或不存在的security category。
- 决策：broadcast wire继续禁止rebind字段，policy decision的effective rebind固定为`fixed_connection`，每个selected binding均为fixed binding，禁止用`same_tenant`表达broadcast。携带previous context的重新broadcast是一次完整新决策：重新读取唯一完整index snapshot、生成新plan ID/version并重建全部bindings；不得把失效旧binding按tenant局部替换。
- 决策：Router每次决策恰好读取一次`LocalConnectionIndexSnapshot`，处理顺序固定为primary expansion、IAM/effective tenant boundary、intended universe、static constraints、dynamic eligibility、scoring、selection。P05 owner在同一index mutation原子发布state、active target与ELIGIBLE、RECONNECT_GRACE、DRAINING、AUTHORITY_SUSPENDED、SESSION_EXPIRY_SUSPENDED；Router不查询GraceService、DrainService、transport或expires_at。all_required任一成员被过滤即整体unavailable；quorum只选择fanout并记录required count，不解释ACK。
- 决策：fallback scorer固定为`runtime_fallback/fallback.v1`，只使用selector exactness、trusted affinity、runtime policy static weight、capability surplus与canonical binding tie-break；不使用random、调用顺序、sender message ID/weight、transport identity或P14 health/latency/pressure。decision fingerprint覆盖normalized requested/effective policy、唯一snapshot mutation、policy/scorer版本与决策证据；rebind时包含previous decision fingerprint，但排除previous/current plan ID、创建时间和message reference，因此语义相同而随机plan ID不同的previous decision产生相同新fingerprint和selected targets。
- 决策：`RoutingDecision`显式区分deeply immutable `ResolvedRoutingPlan`与`RoutingFailureReport`，失败不能伪装为空plan。plan分别保存requested/effective strategy、rebind和parameters证据，以及原target、候选/过滤/评分、selected binding、message IAM decision、config/policy/scorer、index mutation、local/stale和previous decision context。`PreviousRoutingPlanContext`包含plan ID/version、message reference、decision fingerprint、context-integrity fingerprint与selected bindings；Router在读取snapshot或生成plan ID前验证message ownership、plan ID/version及fingerprint格式/完整性，跨message链或篡改稳定拒绝。version只从已验证previous context推导，Router不保存message→version map。敏感执行值不进入repr；日志、指标和普通审计只能消费`SafeRoutingProjection`。
- 决策：PC-1 authorization stage返回immutable `AuthorizationDecisionEvidence`，rate-limit、idempotency和audit-marker原样传递，第六阶段在policy/Router之前验证message、target、principal/effective tenant和permission snapshot binding。若backend IAM不提供decision ID，runtime用access-check输入/allow结果、permission版本、normalized target reference和当前message reference生成类型化decision reference；RoutingPlan的IAM reference/version必须来自该message decision，禁止以session permission snapshot冒充。cross-tenant只能由明确allow decision产生，不允许从tenant不等式直接推导，也不使用global/context mutation或共享last-decision。
- 决策（P09-FIX-02 permission binding校准）：`AuthorizationDecisionEvidence`分别冻结authorization开始时的session permission snapshot ref/version与最终access check使用的effective snapshot ref/version；当前IAM refresh只允许同一snapshot ref内版本更新。外层decision reference绑定message、target、tenant/cross-tenant、两组snapshot和底层decision-result reference。PC-1第六阶段在policy与Router之前校验session snapshot、同ref refresh和完整binding；任一篡改返回`AUTHORIZATION_EVIDENCE_MISMATCH`且不读取index snapshot。
- 决策（P09-FIX-02 policy/plan authority校准）：scorer affinity与static weight只能由`RoutingPolicyDecision`中的immutable `RoutingScoringDecision`提供，`RoutingRequest`不再提供独立注入面；默认policy输出空输入并冻结input version/reference，仍不使用P14 health信号。policy direct construction禁止None rebind扩张为same-*、same-*互相扩张、broadcast非fixed或security-sensitive非no-rebind；二者冲突的security-sensitive broadcast明确reject。`ResolvedRoutingPlan`保存repr-hidden typed policy decision与authorization evidence，并独立复验strategy、parameters、rebind、security evidence、展开证据、selected binding及IAM/scorer authority。
- 决策（P09-FIX-02 fingerprint/failure校准）：decision fingerprint新增IAM decision ref/version、authorized target ref、effective permission snapshot ref/version及scorer input ref/version；继续排除current/previous plan ID、created-at与message reference，并保留previous decision fingerprint。`ResolutionHint`最终闭集固定为`local`、`master_query_required`、`remote_runtime_required`、`authority_recovery_required`；P09只产生local、remote-runtime与authority-recovery，master-query仅冻结枚举值。
- 决策（P09-FIX-03 IAM trusted-boundary校准，取代FIX-02的单一decision reference语义）：`AuthorizationDecisionEvidence`分别持有`message_binding_reference`与`semantic_decision_reference`。前者绑定当前message reference/type、semantic reference、principal/effective tenant、cross-tenant、target和effective permission ref/version，供PC-1第六阶段在policy与Router前防止跨message复用；后者只绑定access-check语义、allow classification/reason、message type、tenant/cross-tenant、target、effective permission ref/version和schema version，明确不含message/reference/time/random。公共类型构造本身拒绝session/effective permission ref不同、空ref/version及任一binding无效；同ref的v1到v2 refresh保持合法。两份真实Envelope仅message ID不同的集成测试证明message binding不同、semantic reference及最终fingerprint相同；把第一份evidence复用于第二份message则在policy调用和index snapshot前拒绝。
- 决策（P09-FIX-03 policy/scorer trusted-boundary校准）：不可变`RoutingPolicyInvocation`由可信`MessageTypeContract`构造，绑定message type/category/audit/security、contract ref/version/required capabilities、config/policy version以及requested intent/risk metadata reference；policy decision必须携带完全相同的invocation及reference，stage six重建并逐项比较。security override改为类型化闭集，安全合同只能明确reject或收紧为no-rebind，不能靠伪造`security_sensitive=false`或手写evidence字符串降级。scorer身份是只接受`runtime_fallback/fallback.v1`的`RoutingScorerIdentity`，不得用任意source/version冒充。
- 决策（P09-FIX-03 plan/fingerprint trusted-boundary校准）：Router与`ResolvedRoutingPlan`共享唯一公开canonical fingerprint函数；plan构造时独立重算并拒绝不一致。fingerprint包含target、policy invocation/裁决、semantic IAM/effective permission、typed scorer/input、index sequence、完整candidate/selected语义、previous decision fingerprint和stale标志，继续排除message/plan/time ID。plan还独立验证candidate与selected binding精确对应、filtered集合完整且无额外项，以及single/quorum/weighted-subset/all-required等strategy cardinality；即使调用方同步伪造expanded字段，也不能绕过公共类型不变量。
- 决策（P09-FIX-04 ALLOW与message/target authority校准）：stage-two成功合同新增唯一枚举`AuthorizationDecisionOutcome.ALLOW`；`AuthorizationDecisionEvidence`公共构造、`AuthorizationProcessor`和PC-1第六阶段均再次拒绝非ALLOW，production只在真实IAM decision为allowed时构造该证据，普通`decision_reason`不决定允许/拒绝。Plan必须证明policy invocation的完整`RequestedRoutingIntent`及normalized target与`original_target`相等，并要求policy message type、IAM message type、plan message reference、IAM authorized target reference和plan canonical target形成同一链；同步重算fingerprint不能替代这些语义不变量。
- 决策（P09-FIX-04 candidate/selection authority校准）：Router和Plan共用纯`validate_candidate_against_routing_target`与`derive_candidate_filter_reason`，CandidateEvidence冻结P05 snapshot中的connection state、active-target eligibility、typed routing eligibility和previous-rebind结果。Plan对全部candidate验证primary intended universe、effective tenant、target capability AND/component/runtime/epoch约束、dynamic filter reason及filtered score=None；selected binding必须与通过全部约束的candidate逐字段相等。Router和Plan还共用`compute_fallback_candidate_score`与`select_candidates_from_evidence`，Plan按fallback.v1重新计算每个通过candidate的score并以canonical升序重算single第1名、quorum/subset前N及all/broadcast/all-required全集和顺序；数量正确但错选、逆序或同步修改score/candidate/binding/fingerprint仍拒绝。
- 决策：PC-1继续只有八阶段。第六阶段消费`RoutingPreparationResult`的NO_ROUTING_REQUIRED、RESOLVED、REJECTED或UNAVAILABLE；后两者立即截断后续stage，feature-disabled contract不调用Router。response finalizer只构造/验证响应，final audit之后才由composition-owned emitter发送成功execution result；strong final audit失败必须清空响应且send为零。resume、reauth rejection和security/kick close的strong lifecycle audit为必需fail-closed依赖；无provider时返回typed unavailable，安全撤权/关闭仍执行并保留enforcement outcome；heartbeat不进入strong audit。
- 决策：P09-W09只冻结`RoutingConsistencyPolicy`、`RoutingPlanRecorder`和`StrongRoutingPlanAuthority`接口。Router不接收StateStore；ordinary plan可停留在返回对象或以safe projection进入进程内recorder。当前不激活`FUTURE_AUTHORITY`或`StateAuthorityKind.ROUTING_PLAN`，production默认strong authority unavailable，因此strong-required routing不能返回成功；这不代表strong RoutingPlan persistence、Redis/Valkey/SQLite provider或durability已经实现。
- 决策：资源上限固定由配置提供，默认candidate 10000、selected 10000、evidence 20000，必须为正且完整round-trip；超限unavailable而不截断，并支持5001 selected bindings。`RoutingFailureReport`必须携带outcome、精确stable reason、original-target safe reference、config/policy version、index mutation、later action、resolution hint和occurred-at；至少细分strategy/rebind reject、三种limit、no candidate、capability、target missing、stale epoch、draining、grace、两种authority suspension、strong-plan authority和remote runtime。strategy/rebind/previous-chain reject映射`RUNTIME_ROUTE_REJECTED/200177`，权威connection missing映射`RUNTIME_TARGET_NOT_FOUND`，tenant/IAM evidence使用既有精确错误，其余映射`RUNTIME_ROUTE_UNAVAILABLE`；公共error可合并，内部reason证据不得丢失。later action只是纯数据，不创建retry、queue、timer、task、worker或subscription。
- 后果：P09达到`VERIFIED/F3`仅指单进程本地routing。Router无transport写入、DeliveryRecord/DeliveryAttempt、ACK/NACK/Defer、delivery quorum、stale topology、master query、remote forwarding、lease/fencing、cluster membership、第二TaskSupervisor/event loop/shutdown owner或global routing registry。P10只能消费`ResolvedRoutingPlan`并独立冻结受理与delivery authority；P14以后才能引入健康/公平信号，P17/P18以后才能定义集群和跨节点路由。
- 关联阶段/工作包：P09 Phase A、`P09-W01`至`P09-W10`、`P09-FIX-02`、`P09-FIX-03`、`P09-FIX-04`、`P10`、`P14`、`P17`、`P18`。

## ADR-037

- ADR 编号：`ADR-037`
- 状态：`ACCEPTED`（P10 DR-1 admission contract frozen）
- 背景：P10必须在不启用发送worker、ACK/retry或集群能力的前提下，把PC-1第六阶段的本地路由结果转换为可查询、可去重、强一致的受理事实。若P10从wire/dict重建RoutingPlan、再次路由、用cache或进程内dict冒充authority，或把response/transport成功解释为delivery成功，就会破坏RP-1、PC-1和P08边界。
- 决策：冻结`DR-1`。P10唯一入口token是由`RoutingPreparationResult.RESOLVED`签发的typed `StageSixAdmissionInput`；其公共直接构造与`dataclasses.replace()`均拒绝，admission只持有原`ResolvedRoutingPlan`对象，不调用Router、不修改或补选selected binding。Envelope message_id必须复算为plan message reference，tenant与authorization message binding必须连续。
- 决策：sender priority、reliability、expires_at、ack timeout和target strategy均为request。不可变trusted config与显式`AdmissionPolicy`输出typed decision，service再次核对request fingerprint、config/policy version、RP-1 effective strategy、上限、TTL和expiry窗口；自定义policy无法通过返回同步伪造对象绕过过期/短窗口、大小、深度或策略版本。
- 决策：inline payload只在内存中形成bounded canonical bytes，按JSON/bytes类型、深度、policy/application/transport三重size限制校验，并保存size、SHA-256 digest/checksum、schema及evidence fingerprint；完整业务payload不进入StateStore、audit、metric或state dump。P10不自动把inline转换为payload_ref。
- 决策：payload_ref通过typed `IamPayloadRefClient`逐selected target实时调用`ns_backend` IAM endpoint，不缓存授权；valid结果必须回显object id、version、checksum、tenant、size和有效期。invalid、unauthorized、tenant mismatch直接形成target rejection且不建DeliveryRecord；timeout/unavailable/异常由可信policy闭集映射reject/wait/dead_letter admission disposition，P10不创建DeadLetterRecord或replay。
- 决策：tenant + raw message_id + RP-1 selected target fingerprint形成dedup key，登记包含schema、TTL、summary、in_progress/acked/dead/expired/cancelled闭集及自校验fingerprint。`StateStoreDeliveryAdmissionStore`在同一tenant namespace先以absent assertions原子创建dedup、initializing root/shard Summary及首批prepared DeliveryRecord；后续批次和终态以provider revision/state version CAS推进。首事务冲突后只做linearizable dedup read。普通cache、模块dict和transient memory不实现该production接口；无provider使用typed unavailable，write outcome未知保持indeterminate且不自动retry。
- 决策：`MessageDeliverySummary`、`DeliveryRecord`、payload/dedup evidence、policy/result/response与atomic initialization均为frozen typed public contract，构造时校验enum、schema/state version、count、fingerprint、plan/tenant/message/summary/shard/delivery/binding跨对象链。prepared只表示已强一致创建但不可发送，P10 active/inflight恒为零；唯一cancel迁移只接受initializing graph，把已创建prepared批量转`cancelled_initializing`并把未创建target记`not_initialized`，不能扩展为一般cancel状态机。
- 决策：fanout `<=5000`只有一个shard；`>5000`按1000 bucket分片。初始化固定每批最多500条：首批与dedup/initializing Summary原子建图，后续批次在同一事务内创建该批DeliveryRecord并CAS更新root/shard进度，最后单独CAS到accepted/partial终态；后续批次或终态发生确定性CAS冲突时，单次事务把已创建prepared转`cancelled_initializing`、未创建target记`not_initialized`并把dedup转cancelled，禁止留下可发送的半初始化图。all accepted、partial rejected与all rejected分别形成accepted/partial/failed root/shard Summary；all rejected仍创建dedup与failed Summary但零DeliveryRecord。rejection evidence只保存低基数typed reason。
- 决策：accepted response exact字段为message_id、summary_id、accepted_at、status_query_hint、trace，不返回delivery_id数组；rejected/duplicate也使用稳定typed response。response emitter没有Store依赖且只在commit结果后调用；发送异常只向observer记录bounded outcome并返回false，不回滚Summary/DeliveryRecord/dedup，也不改变delivery语义。
- 后果：P10达到`VERIFIED/F3 (local only)`；P10-FIX-01/P10-FIX-02随后按ADR-038补齐并重新验收Redis standalone provider，但不改变DR-1。Valkey仅实现driver compatibility，server integration仍UNVERIFIED。task.dispatch production feature继续disabled，直到P11发送与P12 ACK闭环获得独立授权并完成；P10不包含prepared->queued、claim/send、DeliveryAttempt、ACK/NACK/Defer、retry/timeout、dead letter/replay、一般cancel/hold、lease/fencing、master query/remote forwarding、cluster、P14 health/fair scheduling或第二lifecycle owner。
- 关联阶段/工作包：`P10-W01`至`P10-W14`、`P11`、`P12`、`P13`、`P14`、`P17`、`P18`。

## ADR-038

- ADR 编号：`ADR-038`
- 状态：`ACCEPTED`（P10-FIX-02 trusted-boundary、lifecycle与recovery closure后重新验收）
- 背景：P08只冻结backend-neutral StateStore contract，P10 DR-1只通过该contract建立delivery authority；若provider被放入delivery、复用soft-failure cache client、在Python逐key检查后再写入，或把连接成功解释为强一致成功，都会破坏既有authority与atomicity边界。
- 决策：`RedisValkeyStateStore`是`StateStore`的唯一Redis协议production provider，位于`ns_common.state_store`。Redis和Valkey driver按backend动态加载；普通导入不加载driver或创建client。composition factory只消费typed runtime.state_store config，delivery/admission/processor继续只依赖`StateStore`。
- 决策：配置分别承载backend、禁止userinfo的endpoint、username、password source reference、namespace和operation timeout。公开`RedisStateStoreOptions`不信任config/factory：直接构造与`dataclasses.replace()`均重新验证exact backend string、exact且最长128字符并禁止NUL/CR/LF的username（空串表示default user）、finite positive exact int/float timeout、credential-free endpoint、namespace和source类型。password只允许从env/file/显式none source解析；自定义source的`resolve()`结果必须再次证明为None或非空exact str，bytes/object/coroutine/str subclass均拒绝且不做`str()`转换。endpoint/config/options/provider repr、typed异常、log和audit均不显示secret/source repr/endpoint/username或driver文本。
- 决策：物理key固定为provider namespace加typed StateKey canonical digest，revision由同一provider namespace的单调counter签发并作为opaque `StateRevision`返回。read只访问primary standalone client；minimum revision按同一counter比较。append log与record key空间分离。
- 决策：CAS与multi-key transaction使用固定Lua script。script先对全部mutation执行absent/revision/state_version/epoch/schema/next-version检查，任一失败在首个写入前返回typed conflict/version mismatch；全部通过后才生成revision并create/replace/delete。append以独立Lua同时校验tail assertion、分配revision、追加entry并更新tail metadata。timeout后的写入保持indeterminate且不自动retry。
- 决策：`StateStore.open()/close()`对hook抛出的KeyboardInterrupt、SystemExit或其他非Exception BaseException，必须先分别恢复NEW/OPEN再原对象穿透；CancelledError、typed error与timeout/Exception映射不变，不保存异常对象。provider的timeout子Task在内部只拦截非取消BaseException并把同一对象交回父Task抛出，防止KeyboardInterrupt/SystemExit先逃逸event loop而绕过StateStore状态恢复；异常对象不写入provider字段。open失败的best-effort client cleanup不得遮蔽原异常；close失败保留client以允许重试。
- 决策：runtime composition root创建provider但不打开；既有单一service运行路径在start前open，既有RuntimeShutdownCoordinator在sink/client/logger前close。provider不创建TaskSupervisor、event loop、thread、retry worker或第二shutdown owner。真实Redis测试必须通过`NsTestResourceFactory.reserve_tcp_port()`持有socket到`Popen`前最后时刻，启动后以Redis server INFO process_id核对Popen PID；清理只SCAN/UNLINK本次随机namespace，禁止FLUSHDB/FLUSHALL/KEYS。恢复证据必须关闭provider A并从全新provider/clock/plan/request/service/scope/key B读取authority记录，不能复用A的内存store。
- 决策：服务证据按实现与实际server拆分。Redis standalone provider为`VERIFIED`；valkey-py连接Redis server只证明Valkey driver compatibility `IMPLEMENTED`，机器无`valkey-server`时Valkey server integration固定`UNVERIFIED`，不得由协议兼容探针升级。
- 后果：P10不再受“仓库没有Redis standalone production StateStore provider”限制；Redis standalone上的DR-1 admission authority可以配置、注入并跨provider重建恢复。该结论不包含Valkey server integration、Sentinel、Cluster、failover、replica read、lease/fencing、leader election、TLS材料验收、P11 send、P12 ACK/retry或任何cache authority。
- 关联阶段/工作包：`P08`、`P10-FIX-01`、`P10-FIX-02`、`P10-W01`至`P10-W14`、`P11`、`P12`、`P17`、`P20`、`P22`。

## ADR-039

- ADR 编号：`ADR-039`
- 状态：`SUPERSEDED IN PART`（P11 local reliable scheduling/write boundary；transport write uncertainty 由 ADR-045 取代）
- 背景：P11必须只消费P10已经受理完成的typed prepared DeliveryRecord，在单runtime和本地target范围内形成prepared -> queued -> sending -> ack_waiting，同时不能重新调用Router、改写RP-1 RoutingPlan、把connection写队列当作权威queued队列，或把transport write success误认为delivery success。P12尚未授权，因此ACK/NACK/Defer、ACK timeout和retry都不能提前进入该状态机。
- 决策：P10 DR-1、tenant dedup、prepared初始化与P08 authority map保持冻结。P11只对MessageDeliverySummary/DeliveryRecord做向后兼容的typed附加状态字段，P10构造默认值和prepared不变量不变。StateStore追加backend-neutral typed paginated scan；Redis/Valkey provider的同一Lua mutation在写record时原子维护namespace + object-type索引。激活器每轮从StateStore读取authority，按priority、batch、tenant/target/global queued水位选择，并把config version、policy version、reason、candidate count和activated_at写入DeliveryActivationEvidence。global水位由单runtime coordinator在同一activation lock内对已知tenant逐一读取authority计数后裁决，不由普通cache或connection write queue提供。
- 决策：queued claim使用provider revision/state-version CAS。DeliveryOwner只含当前local runtime、worker、opaque claim token、15s默认lease、renew failure与risk窗口；renew默认5s，连续失败超过2次进入at_risk。该owner lease是P11本地写保护，不是P17 leader lease，不含fencing token、不允许remote owner或伪造transfer。旧token、过期lease和at_risk owner不能开始新write；已经处于sending的write只允许在默认4s保护窗口内提交完成结果。
- 决策：SendWorker每次从StateStore重新读取DeliveryRecord并核对queued status、owner/lease、activation与admission config/policy version、expires_at，再通过P05 ConnectionLifecycleManager核对active eligibility、connection/session/epoch/tenant/identity。payload validator/resolver必须回显并匹配DeliveryRecord中的tenant、object/version/checksum、evidence fingerprint、message、target与content digest；自定义dependency返回mapping/object/subclass等非精确typed结果一律fail closed。P11不从wire/dict重建RoutingPlan、不调用Router、不重新选择target，也不绕过IAM/payload authority boundary。
- 决策（历史，已由 ADR-045 部分取代）：queued -> sending、DeliveryAttempt create以及root/shard Summary queued/sending计数必须在同一StateStore transaction提交；transport调用前authority必须已经是sending且attempt为writing。进入sending时记录ack_deadline，write timeout必须小于owner lease TTL。write成功只把attempt转write_succeeded并把DeliveryRecord转ack_waiting；不存在sent或sent_success状态，ack_waiting持续占inflight。关于 write error、write timeout 和 shutdown interruption 的旧 `write_failed` 规则不再有效，必须使用 ADR-045 的 `NOT_STARTED / UNCERTAIN / SUCCEEDED` 与 `WRITE_UNCERTAIN` 规则；retry_scheduled只保留为公共enum的拒绝构造placeholder，不存在production transition、worker或budget，dead_lettered不实现。
- 决策：prepared不占active/inflight，queued只占tenant/global/target queue名额，sending占active/write slot，ack_waiting占inflight。每次状态迁移同步更新root/shard Summary；resource accounting可从authoritative DeliveryRecord重算并与root Summary比对，不一致typed unavailable。LocalDeliveryDispatchCoordinator只提交有界run-once任务给P01唯一TaskSupervisor，不创建event loop、第二supervisor或shutdown owner；遇到disconnect/payload invalid/owner risk立即停止本轮，避免重复claim放大风险。
- 决策：task.dispatch仅开放显式processor injection和默认false的local experimental配置入口；builtin protocol production contract继续feature disabled，主composition不默认注册。发送方得到accepted只说明P10受理提交成功；transport write完成只说明进入ack_waiting。P12完成合法AckRecord + acked原子闭环并重新验收前，不得声明production reliable delivery可用。
- 后果：P11达到`VERIFIED/F3 (local only)`只表示本地authority scheduling与write到ack_waiting通过本地测试和真实Redis原子claim验证，没有远程CI。未实现ACK/NACK/Defer、ACK timeout scanner、retry worker/budget、DeadLetterRecord/replay、一般cancel/hold、P17 fencing/leader lease、master query、remote forwarding、cluster、P14 health/fair scheduling或production task.dispatch enable。
- 关联阶段/工作包：`P08`、`P09`、`P10`、`P11-W01`至`P11-W11`、`P12`、`P13`、`P14`、`P17`、`P18`、`P22`。

## ADR-040

- ADR 编号：`ADR-040`
- 状态：`ACCEPTED`（P10-FIX-03 + P11-FIX-01 authority closure）
- 背景：P10/P11首次验收后复核发现，普通fanout仍被强制建root+shard、fanout与批量参数分散硬编码，可靠inline请求fingerprint不绑定canonical内容且正文没有durable authority，payload_ref验证结果也未绑定具体请求和target。P11激活/资源统计仍依赖record scan与应用层排序，global调度依赖进程known-tenant集合，claim/renew只有revision CAS和调用者布尔结果，transport接收resolver提供的raw wire text。这些实现不能支撑重启恢复、完整Envelope authority或高并发原子投影。
- 决策：`DR-1`继续表示受理合同家族，但持久schema升级为`dr-2`；legacy `dr-1`反序列化只返回明确`migration_required`，不得静默猜测或混读。普通fanout只建一个Summary，超过`AdmissionPolicyConfig`冻结到每条decision的threshold才建立root+shard；threshold、bucket、initialization batch和activation batch由同一typed policy/config拥有，不再由model/service散落常量决定。Summary状态闭集使用`initializing/pending/partial_acked/all_acked/partial_failed/failed/cancelled`，P10/P11未收到ACK前只使用其中已授权状态。
- 决策：inline payload的request fingerprint绑定canonical bytes、digest、size和policy limits；可靠inline正文以独立`payload_body` authority持久化，DeliveryRecord、Summary、dedup、transition log和audit只保存body_ref与摘要，不复制正文。payload_ref验证请求与返回值同时绑定request fingerprint和target fingerprint，自定义dependency返回旧类型、mapping、subclass或错target evidence均fail closed。dependency outcome闭集为`reject/wait_required/dead_letter_required/dependency_unavailable`；这里只表达受理裁决，不创建P13 dead-letter record。
- 决策：StateStore新增provider-neutral ordered-index mutation/read与transition-log append合同；record mutation、prepared/ready/claimed/lease/sending/ack/failed及target/global投影和transition log必须由一个`StateTransaction`提交或全部回滚。Redis standalone provider以固定Lua先验证所有record assertion，再执行Hash/ZSet/log append；tenant index key使用tenant+bucket hash tag。该Lua只声明standalone原子性，不声明Redis Cluster跨slot、leader或failover能力，Cluster仍归P17以后。
- 决策：激活只分页读取权威prepared index，并同时遵守runtime安全上限与DeliveryRecord冻结的config/policy snapshot；不扫描全部DeliveryRecord、不依赖进程known-tenant集合、不在应用层全量排序。claim写入单调per-delivery fencing；本地expired queued owner只能由同runtime以更高fencing恢复，expired sending转`write_uncertain`且禁止重写，ack_waiting只重建本地owner而不重发。该fencing只隔离单delivery旧worker，不是P17 leader/cluster fencing，也不授权跨runtime ownership transfer。
- 决策：LeaseRenewWorker在既有P01唯一TaskSupervisor内运行真实Clock驱动续约任务，续约结果只来自StateStore authority事务。SendWorker重读DeliveryRecord后校验owner/fencing、target/session/epoch/identity、payload evidence和expiry，以record冻结的config/policy版本重建完整typed Envelope并重新canonicalize；resolver只能返回typed payload material，不能提供wire text。transport write后若authority提交冲突，状态转`write_uncertain`，不得伪造ack_waiting或重发；write成功仍只进入ack_waiting。
- 后果：P10-FIX-03与P11-FIX-01关闭后，P10/P11恢复`VERIFIED/F3 (local only)`。结论只覆盖Redis standalone与本地单runtime写到ack_waiting；没有远程CI，也不包含P12 ACK/NACK/Defer/timeout/retry、P13 DLQ/replay/cancel/hold、P14 fairness/health、P17 leader/cluster fencing、remote/master/cluster或production task.dispatch enable。
- 关联阶段/工作包：`P08`、`P09`、`P10-FIX-03`、`P11-FIX-01`、`P11-W01`至`P11-W11`、`P12`、`P13`、`P14`、`P17`、`P22`。

## ADR-041

- ADR 编号：`ADR-041`
- 状态：`ACCEPTED`（P10-FIX-04 + P11-FIX-02 authority、Cluster-key布局与恢复语义闭合）
- 背景：P10/P11联合复核发现，出站Envelope曾用target binding和IAM decision字段重造source/auth_context并硬编码protocol；Redis transaction的revision/record/scan/ordered/log物理key没有统一tenant bucket slot；owner清空会丢失fencing历史；lease与ACK deadline未完全分离；transport成功后的indeterminate StateStore结果缺少三分支reconcile；prepared首页的无效或水位阻塞成员可能造成饥饿；waiting与write failure、inline重复遍历、payload验证旧结果重放也没有形成独立authority边界。上述问题必须在不解冻RP-1、P08 owner、P12 ACK或P17 cluster coordination的前提下闭合。
- 决策：admission从可信ProcessorContext精确保留原始`ProtocolGroup`、`MessageGroup`、`SourceGroup`、`AuthContextGroup`及必要`TraceGroup`，持久化只保存协议字段和identity/capability/permission摘要，不保存raw identity、credential或token。DeliveryRecord保存authority fingerprint，公共直接构造、反序列化和`dataclasses.replace()`均重新计算；发送时原样复用source/auth/message/trace，不能以target runtime冒充source，也不能以IAM decision字段伪造permission snapshot。P11读取当前P05 target的协商protocol/schema和实时access decision reference；当前没有schema translation，因此只接受exact protocol compatibility，禁止硬编码1.0或静默降级。
- 决策：delivery authority使用由typed policy冻结的正整数`authority_bucket_count`和`sha256(message_id)`稳定派生的`authority_bucket_id`。同一StateTransaction的revision、record、object scan index、ordered index和transition log全部带精确`{tenant_id:bucket_id}`Redis hash tag；不再维护跨slot system global ZSet，tenant/global水位由各tenant bucket权威投影有界聚合。旧无tag物理record不会被静默混读；新布局查无记录但旧key存在时返回`legacy_physical_key_migration_required`。该决策和key-slot测试只证明transaction Cluster-slot兼容布局，不实现Redis Cluster部署、membership、failover、leader或P17 fencing。
- 决策：DeliveryRecord独立持久化`last_fencing`与`owner_epoch`；首次claim、release后reclaim、expired queued reclaim和ACK_WAITING owner恢复都严格递增，owner清空不清零历史。DeliveryClaim、DeliveryOwner和DeliveryAttempt同时绑定fencing与epoch，即使复用同worker/token，旧claim也不能复活。该token只隔离单delivery本地worker，不是P17集群fencing，不允许ownership transfer。
- 决策：`delivery.lease`索引score始终等于`owner.lease_expires_at`，`delivery.ack`只保存`ack_deadline`。ACK_WAITING owner恢复只续`now + lease_ttl`，不得延长到ACK deadline。transport write成功后的StateStore结果只有indeterminate exception进入reconcile：若authority已经是ACK_WAITING且attempt为当前write_succeeded则接受已提交结果；若仍是SENDING则以CAS转WRITE_UNCERTAIN；任何其他状态返回typed `completion_reconcile_conflict`。确定性StateStore error不得伪装为indeterminate或盲转状态。
- 决策：ordered index读取增加typed`start_after` cursor；prepared activation以配置化`activation_scan_budget`有界分页，删除stale member并越过target watermark候选继续扫描，不能因第一页永久饥饿。TARGET_WAITING使用独立waiting index与waiting count，不进入write_failed；EXPIRED、PAYLOAD_REJECTED、WRITE_FAILED、WRITE_UNCERTAIN分别拥有独立索引和Summary/resource计数。P12保留的PARTIAL_ACKED/ALL_ACKED/PARTIAL_FAILED在当前公共构造中一律拒绝。
- 决策：inline payload在AdmissionRequest边界只构造一次bounded typed descriptor，包含media、canonical bytes或typed rejection、size、digest、observed depth和双重limit；policy与service只消费descriptor，不重新遍历原始对象。cycle、超深、NaN/Inf、非法类型和超限稳定形成rejected/failed Summary。P11 `PayloadValidationResult`精确绑定当前request fingerprint、DeliveryRecord target fingerprint及本次解析出的实时target access decision reference；mapping/object/subclass、跨target或旧decision replay均在transport前fail closed。
- 后果：P10-FIX-04与P11-FIX-02关闭后，P10/P11恢复`VERIFIED/F3 (local only)`。结论只覆盖本地单runtime、Redis standalone authority以及prepared -> queued -> sending -> ack_waiting/typed failure；没有远程CI。P12继续BLOCKED；未实现ACK/NACK/Defer、ACK timeout/retry、DLQ/replay、一般cancel/hold、P14 health/fairness、P17 leader/cluster coordination、remote/master routing或production task.dispatch enable。
- 关联阶段/工作包：`P08`、`P09`、`P10-FIX-04`、`P11-FIX-02`、`P11-W01`至`P11-W11`、`P12`、`P13`、`P14`、`P17`、`P22`。

## ADR-042

- ADR 编号：`ADR-042`
- 状态：`ACCEPTED`（P10-FIX-05 + P11-FIX-03 authority layout、runtime-global watermark 与写后对账闭合）
- 背景：P10/P11联合复核发现，tenant集合只存在于进程内时，scheduler重启后无法重建真正runtime-global queued水位；固定8桶缺少持久layout version/generation，配置改为16桶可能静默形成双布局；单bucket优先扫描会让后桶prepared或expired lease长期饥饿；发送仍可能沿用入站MessageGroup的请求priority/reliability/expiry；transport已写后任意StateStore冲突未统一进入权威对账；payload_ref validation也需要把对象请求与当前target permission snapshot、实际access decision同时绑定。这些缺口必须在不改变P08 authority owner、不解冻RP-1/P12/P17且不创建跨slot事务的前提下闭合。
- 决策：authority layout固定持久`layout_version`、正整数`layout_generation`和`bucket_count`，物理partition为`layout-{generation}-bucket-{bucket}`；runtime配置的布局变更语义固定为`restart_required`。同一runtime使用provider-neutral StateStore registry持久化唯一layout和tenant有序索引；registry使用独立合成tenant namespace，只用于恢复枚举，不与任一tenant delivery事务原子联结。admission与scheduler每次进入authority前校验registry；8到16或generation不匹配均typed fail-closed。Redis查无当前generation record时还检查上一代tagged key并返回显式migration-required，不静默混读。
- 决策：runtime-global queued水位只从registry枚举全部tenant，再按冻结layout有界聚合所有bucket权威索引；调用方传入的旧计数必须与该聚合精确相等。单runtime scheduler用同一activation coordinator串行执行激活；prepared activation在全局scan budget内轮转全部bucket，先给每桶有限首页预算，再按轮次推进cursor。claim/recovery以每tenant轮转bucket cursor处理expired lease与ready记录，避免持续有ready的低号桶永久遮蔽其他桶。上述轮转只保证P11本地有界进展，不实现P14 health scoring或通用公平调度。
- 决策：出站Envelope的message identity/type/category/created_at保留入站权威值，但priority、reliability和expires_at必须从DeliveryRecord冻结policy重建；禁止重新调用Router、修改RoutingPlan或重选target。payload_ref validation结果必须精确绑定对象/version/checksum/request fingerprint、target identity、当前permission snapshot和本次实际IAM access decision reference；dependency返回旧结果、跨对象、跨target或旧snapshot均在transport前fail-closed。
- 决策：transport write返回成功后，ack_waiting提交携带发送前SENDING的expected state_version；任何StateStore error或中间lease renew造成的版本冲突都统一重读authority并执行三分支对账：已是匹配attempt的ACK_WAITING则接受；仍是SENDING则CAS进入WRITE_UNCERTAIN；其他状态typed conflict。若对账读取本身不可用，则返回`WRITE_OUTCOME_UNKNOWN`且保留最后已知SENDING，不盲写、不重发、不伪造成功。transport write成功仍不等于delivery success。
- 后果：P10-FIX-05与P11-FIX-03关闭后，P10/P11恢复`VERIFIED/F3 (local only)`。结论只覆盖本地单runtime、Redis standalone authority以及prepared -> queued -> sending -> ack_waiting/typed failure/uncertain保护；没有远程CI。P12继续BLOCKED；未实现ACK/NACK/Defer、ACK timeout/retry、DLQ/replay、一般cancel/hold、P14 health/fair scheduling、P17 leader/cluster coordination、remote/master routing或production task.dispatch enable。
- 关联阶段/工作包：`P08`、`P09`、`P10-FIX-05`、`P11-FIX-03`、`P11-W01`至`P11-W11`、`P12`、`P13`、`P14`、`P17`、`P22`。

## ADR-043

- ADR 编号：`ADR-043`
- 状态：`ACCEPTED`（P11-FIX-04 持久调度进展、写后对账与 payload IAM evidence 闭合）
- 背景：P11-FIX-03 后的 bucket 轮转和 ordered-index 分页进展仍保存在 scheduler 实例内；prepared、ready 或 lease 首页若持续包含缺失record、错误状态、旧score或不可由当前local runtime恢复的owner，重启或换provider后可能重复扫描同一前缀。transport已经接受bytes后，completion还可能因owner lease已过期、authority已经提交ACK_WAITING或更高fencing替换而返回typed state error；只处理普通StateStore error会泄漏异常或错误扩大写入。payload_ref发送前也需要一个由production IAM client实时生成、可完整复算且绑定当前target permission snapshot/version的精确typed evidence。
- 决策：activation、ready claim、expired lease recovery和bucket rotation的进展统一持久化为同tenant/bucket StateStore authority cursor；cursor记录使用revision/state-version CAS，包含精确member/score或next bucket，不依赖进程cache。每次调用受全局`activation_scan_budget`约束，cursor失效时从权威索引安全重锚；provider A关闭后provider B必须能继续而不是从固定前缀重新开始。该进展保证只属于P11本地有界扫描，不是P14公平调度、P17 lease或ownership transfer。
- 决策：ordered-index投影与DeliveryRecord不一致时，只对可证明的record缺失、malformed、state-version/identifier不匹配、错误状态、owner缺失、foreign runtime owner或旧lease score执行同scope原子remove/score repair；高风险项进入scheduler quarantine索引。每次repair追加只含index、原因、member摘要、replacement/quarantine布尔值和时间的安全transition log，不保存raw member、payload、identity或异常文本。provider不可用、migration-required等非投影分歧错误必须原样fail closed，禁止误删有效投影。
- 决策：transport成功后的completion异常不再要求当前owner lease仍有效。reconcile必须先读取当前DeliveryRecord和当前DeliveryAttempt，并以delivery/attempt id、attempt number、原claim token、fencing和owner epoch精确绑定：匹配的ACK_WAITING + WRITE_SUCCEEDED即使lease随后过期也接受已提交结果；匹配的SENDING + WRITING即使owner过期或已被更高fencing替换也只CAS到WRITE_UNCERTAIN并清除owner；其余状态或attempt不匹配返回typed conflict。reconcile自身不可用或冲突返回WRITE_OUTCOME_UNKNOWN，不伪造ack_waiting、不重写、不重发。
- 决策：production payload reference validator只接受显式注入的production `IamClient`与`Clock`，每次发送实时调用payload_ref integrity validation和target access check。`PayloadAccessDecisionEvidence`精确绑定object/version/checksum、P10 request fingerprint、DeliveryRecord target fingerprint、当前connection/session/epoch/identity、permission snapshot reference/fingerprint/version、IAM decision reference、decision time和expiry；公共构造与`dataclasses.replace()`均复算全部fingerprint。denied、revoked、expired、旧permission version、refresh-required、错object/target或非精确typed dependency结果一律在transport前fail closed。
- 后果：P11-FIX-04关闭后P11恢复`VERIFIED/F3 (local only)`。该结论只覆盖单runtime、本地target、Redis standalone authority和`prepared -> queued -> sending -> ack_waiting`及typed waiting/failure/uncertain保护；没有远程CI，transport success仍不是delivery success。P12 ACK/NACK/Defer/timeout/retry、P13 DLQ/replay/cancel/hold、P14 health/fair scheduling、P17 leader/cluster fencing、remote/master/cluster和production `task.dispatch` enable继续未实现。
- 关联阶段/工作包：`P05`、`P06`、`P08`、`P09`、`P10`、`P11-FIX-04`、`P11-W01`至`P11-W11`、`P12`、`P13`、`P14`、`P17`、`P22`。

## ADR-044

- ADR 编号：`ADR-044`
- 状态：`ACCEPTED`（P11-FIX-05 authority precondition、renew lifecycle与对象级IAM closure）
- 背景：P11-FIX-04的ordered-index repair先读record/index、后提交index/log事务，若release、renew或create在两者之间提交，repair可能删除新ready member、覆盖新lease score或为已创建record追加错误quarantine log。scheduler cursor的旧逻辑ID又只含cursor name，依赖provider scope形成物理隔离；renew task没有显式stop authority；payload access evidence可由公共构造按本地摘要公式伪造，且runtime把不可逆identity digest当作IAM principal传入两个分离请求。这些问题均会削弱P08 authority和P11发送前fail-closed边界。
- 决策：`StateTransaction`增加provider-neutral只读前置断言，record支持absent/present及可选revision/state_version，ordered-index member支持absent/present及可选exact score。断言不是mutation，不单独构成写事务；provider必须在任何record/index/log写入及revision分配前一次性验证全部断言。Redis standalone固定Lua与deterministic contract model遵守同一顺序；任一失败返回typed conflict且record、index、repair log均零落地。
- 决策：prepared/ready/lease repair必须把已观察record revision/state_version和该页member score放入同一事务。missing/malformed路径在提交前再次linearizable读取：若已成为有效record则放弃repair；若仍缺失或仍malformed则分别绑定absent或精确record断言。断言竞争只允许重新读取，不写repair log；renew胜出时旧score repair不得覆盖新lease score，release/create胜出时不得删除新投影。
- 决策：scheduler cursor schema升级为v2，typed StateKey逻辑摘要显式包含authority layout generation、bucket id、operation和index identity；payload同时回显并校验这些字段。旧name-only cursor只用于migration detection；发现即返回`migration_reset_required` version mismatch，不与v2静默混读或自动猜测。
- 决策：`LeaseRenewWorker.schedule()`返回每claim typed `LeaseRenewalHandle`，由既有P01 TaskSupervisor继续拥有task，handle只提供显式stop/join authority。PRECHECK_FAILED、OWNER_RISK、WRITE_FAILED和WRITE_OUTCOME_UNKNOWN后coordinator停止并等待该renew task；unknown的SENDING不再续租，允许本地lease到期后由既有authority recovery转WRITE_UNCERTAIN。owner释放、terminal、lease过期、claim/fencing失配均为renew loop正常退出，不记作supervisor failure；ACK_WAITING在P12完成前仍可续租。
- 决策：`PayloadAccessDecisionEvidence`改为`init=False`并由module-private issuer签发，公共直接构造与`dataclasses.replace()`不能生成新evidence。生产reference组合只接受`IamDeliveryPayloadReferenceValidator`，该validator仅在真实`IamClient.revalidate_payload_ref()`返回精确typed backend decision后签发；runtime删除本地IAM decision-reference生成函数。专用revalidation request绑定object/version/checksum/size、payload tenant、目标opaque principal、target fingerprint、permission snapshot/version和opaque admission authority reference，不再使用source/owner identity digest。backend先验证payload provider metadata，再以`payload_ref.read`对精确object id执行Resource ACL/policy，返回backend-issued decision reference/version；无provider、provider unavailable、旧snapshot、跨tenant/object或ACL deny均fail closed。
- 后果：该决策不改变ENV-1、PC-1、IAM-R1核心握手/权限合同、RP-1、P08 authority owner或P10 admission语义；P11仍只到ack_waiting。没有新增event loop、TaskSupervisor、shutdown owner、P17 leader lease/fencing、remote ownership或重发。P12 ACK/NACK/Defer/timeout/retry、P13 DLQ/replay、P14 health/fair scheduling和production task.dispatch仍被阻塞。
- 关联阶段/工作包：`P08`、`P10`、`P11-W01`至`P11-W11`、`P11-FIX-04`、`P11-FIX-05`、`P12`、`P13`、`P14`、`P17`。

## ADR-045

- ADR 编号：`ADR-045`
- 状态：`ACCEPTED`（production authority seal、transport write uncertainty、原子 cursor 与长期运行入口校准）
- 背景：P09/P11复核发现三类公开值仍可能被当作 authority：`AuthorizationDecisionEvidence`可由普通调用方按公开SHA-256公式构造；payload validator可接收绕过初始化或覆写方法的`IamClient`子类；`StateAccessScope`可由调用方直接声明caller与capability。与此同时，transport在write已经开始后的timeout、取消或异常仍落到`WRITE_FAILED`，默认`python -m ns_runtime.main`又在self-check后退出；Redis ordered-index分页和provider transaction result cardinality也缺少同一权威操作内的完整闭合。
- 决策（production/test authority隔离）：authorization evidence、payload access evidence和StateStore scope都改为issuer-sealed exact production type。production issuer由composition root或每个StateStore唯一拥有，contract-test issuer使用显式test-only contract和独立realm，二者不可互认。seal绑定全部公开authority字段；公开构造、`dataclasses.replace()`、字段复制、subclass、未初始化对象、fake issuer或手工capability不能生成或扩大production authority。SHA-256/fingerprint继续用于可复算内容绑定，但不再被解释为backend IAM许可证明。production RoutingPlan/admission仅接受真实`IamProcessorAuthorization -> MessageAuthorizationService`调用链签发的evidence；payload validator只接受完整初始化、未覆写且由composition root装配的精确production `IamClient`。
- 决策（StateStore authority）：每个StateStore实例拥有唯一scope issuer，并把store realm、caller、domain、namespace、tenant、runtime/plugin/partition及精确capability一起签名。所有read/transact/append/index操作都复验同一store issuer；跨store scope、公开dataclass、replace、subclass和手工枚举稳定拒绝。production repository/authority service与显式contract-test model分别调用各自窄issuer；不增加global registry、service locator、第二个authority owner，也不允许Processor、plugin、EventBus subscriber或connection code直接签发。
- 决策（transport uncertainty，取代ADR-039及P11-W08中“所有timeout/error均为write_failed”的旧表述）：transport write返回`NOT_STARTED`、`UNCERTAIN`或`SUCCEEDED` typed result。只有adapter能证明bytes未开始时，SendWorker才写`WRITE_FAILED`；write已经开始后的timeout、取消、connection close、未知异常或非法provider result统一写`WRITE_UNCERTAIN`、释放owner并停止发送/renew。shutdown interruption不得降格为确定失败。`SUCCEEDED`后的StateStore completion继续使用ADR-042/043既有reconcile，不能解释为delivery success。
- 决策（provider结果与ordered-index分页）：StateTransaction provider result必须精确满足`records == mutations`和`log_positions == log appends`，多项、缺项、错误类型在zip或状态投影前fail closed。Redis ordered-index用一次Lua执行原子验证cursor member/score、定位、分页和total；并发在cursor前插入/删除不造成已消费项重复，cursor本身删除或score改变返回typed conflict，避免静默跳项或饥饿。公共read assertion从model与facade保持一致导出。
- 决策（默认入口）：`python -m ns_runtime.main`默认打开唯一StateStore、启动唯一RuntimeService并等待同一RuntimeShutdownCoordinator的SIGINT、SIGTERM、critical task failure或显式shutdown。`diagnose`与`self-check`是显式有界命令；启动异常保留原对象并执行best-effort cleanup。不得创建第二event loop、TaskSupervisor、RuntimeService或shutdown coordinator。
- 后果：P09/P10/P11既有local-only范围不扩大，P12仍`BLOCKED`。该校准只关闭authority伪造、write uncertainty、provider atomicity/cardinality与入口生命周期缺口；不实现ACK/NACK/Defer、timeout retry、DLQ/replay、P14公平/健康调度、P17 cluster ownership或production `task.dispatch`。
- 关联阶段/工作包：`P02-FIX-06`、`P08-FIX-01`、`P09-FIX-05`、`P10`、`P11-FIX-06`、`P12`、`P13`、`P14`、`P17`、`P22`。

## ADR-046

- ADR 编号：`ADR-046`
- 状态：`ACCEPTED`（composition-owned instance authority 与 transaction-bound provider result 校准）
- 背景：ADR-045关闭了公开hash自签名，但复核发现module-private自由函数仍可按caller提供的任意字段签发production authorization/payload evidence；production IamClient也仍可由自由factory创建，且精确NsAsyncHttpClient实例的`post/request/send`可被instance monkey patch。StateStore又保留接收任意caller/capability的实例scope issuer，delivery自由函数一次签发全部权限；StateTransactionResult只在Store返回后检查数量，自身可直接构造、复制并在同长度transaction间复用。
- 决策（IAM authorization）：删除module-level production evidence signer。每个production `IamProcessorAuthorization`持有绑定精确`MessageAuthorizationService`的实例issuer，只直接消费该service签发的`MessageAuthorizationResult`和当前`ProcessorContext`。typed result以service实例密钥绑定原始request、session/effective snapshot、backend decision和risk；evidence内部派生message/tenant/target、permission、config/policy和decision binding。自由字段签发、direct construction、replace/copy、subclass、未初始化/fake service、test realm和cross-service replay均不能形成production authority。
- 决策（IAM client/HTTP provenance）：删除module-level production IamClient factory。实例`IamClientFactory`由composition root持有并一次性绑定精确`NsHttpClientOwner`、该owner创建的精确`NsAsyncHttpClient`、底层`httpx.AsyncClient`及runtime composition；同一HTTP client不能重复绑定第二composition。production provenance逐次复验exact type、owner/client/binding identity、class method identity及instance `post/request/send` substitution；copy、subclass、未初始化对象和monkey patch一律失去production资格。测试backend response通过composition以下的真实本地HTTP transport输入，不伪装production adapter。
- 决策（payload authority）：删除module-level payload production signer。每个production reference validator私有持有绑定同一production IamClient与Clock的实例issuer；IamClient记录并一次性消费精确的原始`PayloadRefRevalidationRequest`/backend `PayloadRefRevalidationDecision`对象对，issuer内部再与DeliveryRecord、target、permission snapshot、admission authority、object/version/checksum/size、decision reference/time/expiry交叉校验。fake validator/client、replace/copy、subclass、错误typed result或cross-request replay不能生成production evidence。
- 决策（StateStore repositories）：删除StateStore接收caller/authority/capability集合的scope issuer和delivery自由签发函数。唯一StateStore composition capability只能创建固定role的strong-audit、admission、scheduler、payload和registry repository；每个repository固定caller、authority、runtime/audit namespace规则和精确最小capability。业务调用只提供tenant/bucket/layout等维度；raw store、错误owner、其他repository、copy/subclass/public scope均不能扩权。各repository仍共享同一StateStore lifecycle owner，不引入global registry、service locator或第二authority owner。
- 决策（transaction result）：`StateTransactionResult`只能通过与精确transaction共同验证的factory形成；result保存transaction identity、per-instance binding、canonical fingerprint和HMAC，records逐项绑定mutation key/document/order，log_positions严格绑定append cardinality与正整数类型。provider在zip前验证响应容器、类型和数量；direct constructor、replace/copy/subclass、字段替换、错序和同形状cross-transaction replay均fail closed。
- 后果：ADR-046取代ADR-044/045中“module-private自由issuer/factory足以构成production authority”以及“Store只需事后检查transaction cardinality”的较弱解释；hash/fingerprint继续只是内容绑定。运行范围仍只到`prepared -> queued -> sending -> ack_waiting`和WRITE_UNCERTAIN不重发；P12继续BLOCKED，未实现ACK/NACK/Defer、retry、DLQ、P14公平/健康调度、P17 cluster ownership或production `task.dispatch`。
- 关联阶段/工作包：`P08-FIX-02`、`P09-FIX-06`、`P11-FIX-07`、`P12`、`P13`、`P14`、`P17`、`P22`。

## ADR-047

- ADR 编号：`ADR-047`
- 状态：`ACCEPTED`（IAM transport provenance 与 StateStore resource authority closure）
- 背景：ADR-046删除了自由 IAM factory/signer 和任意 StateStore capability issuer，但复核发现普通代码仍可尝试用新建 HTTP owner/client/composition 组装 production IAM client；HTTP binding 未覆盖底层 `_transport`、`_mounts` 与 mount/proxy handler substitution。production StateStore 又把 repository owner 保存在 raw store，并只按 capability/namespace 而未按 object/schema/index/log 资源隔离 repository role。
- 决策（IAM composition）：`IamClient`公共构造永久 fail closed，module 不提供 `IamClientFactory`、production factory或依赖 frame/path 的 composition proof。普通 `NsHttpClientOwner` 不再具有 IAM handle 签发 API；唯一 production graph 在 process bootstrap 的同一局部组装区间内一次性绑定 exact owner/client/handle/client identity。contract-test adapter保持独立类型与 realm；伪造 filename、替换 `sys._getframe`、普通构造、`object.__new__`、copy、subclass和method override均不能建立production adapter。
- 决策（HTTP provenance）：IAM只持有 narrow POST handle。binding冻结 client/request/httpx base URL、scheme/host/port/path prefix、timeout、默认routing/auth headers、TLS context、proxy、主transport、mount/proxy transport及其handler identity；IAM endpoint使用exact allowlist并拒绝绝对URL、跨host、编码/路径穿越。请求构造与执行使用绑定时捕获的绝对backend和transport callable，不再依赖“await前后复验”，因此await期间替换后恢复transport/base URL/mount也不能改变请求去向。
- 决策（StateStore creation）：public facade只提供provider-only创建与显式contract-test composition；production composition由bootstrap一次性组装，并用endpoint/namespace/runtime进程锁拒绝平行authority。raw StateStore不保存repository owner、private issuer/private key或repository creation capability；固定repository set安装一次即关闭。production repository只保存固定role/binding和Store窄endpoint，不保存签发closure；contract-test issuer仍是独立realm。
- 决策（resource allowlist）：production scope只携带opaque canonical policy ID，不携带可被`object.__setattr__`扩权的policy dataclass。StateStore内部只读policy表按ID/version解析，并以scope实例快照绑定atomic scope、role/caller/capabilities/policy ID/repository binding；复制、字段替换和cross-role replay均fail closed。Store在provider调用前逐项验证exact operation、`object_type/schema_name`、ordered-index `name/bucket`和namespace/partition，角色最小权限边界保持不变。
- 后果：上述closure不新增global registry、service locator、第二StateStore owner、event loop、TaskSupervisor、RuntimeService或shutdown owner。状态机仍严格停在`prepared -> queued -> sending -> ack_waiting`；WRITE_UNCERTAIN不恢复、不重发。P12继续BLOCKED，ACK/NACK/Defer、retry、DLQ、cluster ownership和production `task.dispatch`仍未实现。
- 关联阶段/工作包：`P08-FIX-03`、`P09-FIX-07`、`P11-FIX-08`、`P12`、`P13`、`P14`、`P17`、`P22`。

## ADR-048

- ADR 编号：`ADR-048`
- 状态：`ACCEPTED`（production authority OS isolation broker）
- 背景：ADR-045至ADR-047仍尝试在普通runtime Python解释器内用pending token、private object、closure、不可变dataclass、HTTP binding或mutable validator证明production provenance。普通代码若能任意import内部module、读取对象/closure并用`object.__new__`或`object.__setattr__`改写字段，这些机制都不能形成不可伪造的authority边界；继续叠加proof、frame/path或调用点白名单只会产生新的同解释器绕过面。
- 决策（进程隔离）：production authority迁入bootstrap以`multiprocessing` spawn启动的独立broker进程。broker独占IAM HTTP client/backend安全配置与transport、Ed25519 decision私钥、production StateStore provider、repository resource policy/scope authority及物理数据域lease。主runtime进程只持有duplex IPC endpoint、broker公钥和instance ID、固定IAM operation proxy、固定repository role proxy及不含私钥的typed result；不持有production HTTP client/transport、raw Store、scope signer或mutable production validator。
- 决策（IAM signed result）：IAM IPC操作闭集为introspect、runtime access check、permission snapshot、payload validate与payload revalidate，不接受URL/path/HTTP method。broker result签名绑定broker instance、operation、canonical request fingerprint与request projection、backend typed decision、permission snapshot/version、tenant/target/message type、issued/expiry、nonce和sequence。主进程只验证签名并让authorization/payload evidence直接保留同一个broker authority；跨broker、跨request/operation、过期、copy/replace/字段修改或旧instance replay全部fail closed。
- 决策（StateStore proxy与lease）：production Redis/Valkey provider只在broker child打开。repository handle由broker签名并固定instance、opaque handle ID、role、runtime/lifecycle generation；服务端按handle选择admission、scheduler、payload、registry、audit固定role与exact resource policy，不接收caller/authority/capability/policy ID。physical-domain OS lock identity绑定backend、normalized endpoint、database、namespace和principal/credential reference，不绑定runtime ID，也不允许调用方选择锁目录；相同物理域的第二broker跨进程拒绝，异常退出由OS释放锁。
- 决策（test与failure）：contract-test StateStore composition只能构造deterministic in-memory test realm，拒绝Redis/Valkey endpoint、credential与network provider；真实Redis验证必须启动production broker fixture。broker crash、IPC close/timeout、签名错误或instance变化时IAM返回unavailable/denied，StateStore write返回indeterminate；不得退回本地production graph、重签decision、自动重试delivery或恢复WRITE_UNCERTAIN。
- 后果：ADR-048取代ADR-045至ADR-047中“同一runtime解释器内bootstrap局部proof/handle/scope validator足以证明production provenance”的表述；其余typed binding、resource allowlist、transaction cardinality和长期运行入口决策继续有效。broker只承担authority隔离与IPC，不新增global registry、service locator、第二RuntimeService、TaskSupervisor或shutdown coordinator。状态机仍严格为`prepared -> queued -> sending -> ack_waiting`，P12继续BLOCKED；未实现ACK/NACK/Defer、retry、DLQ、cluster ownership或production `task.dispatch`。
- 关联阶段/工作包：`P08-FIX-05`、`P09-FIX-09`、`P11-FIX-10`、`P12`、`P13`、`P14`、`P17`、`P22`。

## ADR-049

- ADR 编号：`ADR-049`
- 状态：`ACCEPTED`（fixed deployment trust root、pickle-free IPC与delivery persistence proxy closure）
- 背景：ADR-048虽建立spawn broker，但runtime仍信任ready消息自报的session公钥，普通模块又能调用任意config starter；`multiprocessing.Connection.send/recv`会pickle应用对象，StateTransaction codec使用了错误字段且ordered-index把cursor传给不存在的参数。P10/P11业务服务仍依赖raw Store/repository，broker proxy没有成为production composition的实际持久化依赖。
- 决策（trust root）：production root公钥预置于broker构建，不能由握手、普通config或环境明文替换。root私钥只通过最外层launcher继承的一次性FD进入spawn child；child在创建IAM/StateStore backend前验证私钥导出的公钥与预置root相同。ready消息必须携带root签名的instance certificate，绑定trust realm、broker instance、session公钥、runtime、lifecycle generation、issued/expiry和nonce；runtime验证certificate后才接受session result与handle签名。公开production starter永久拒绝，contract和真实Redis integration各自使用非production test trust realm。
- 决策（wire）：应用IPC只使用`send_bytes/recv_bytes`承载canonical UTF-8 JSON，禁止pickle Python object。decoder限制frame/string/container/depth，只允许JSON primitive tree并拒绝duplicate/unknown field和NaN/Infinity。IAM、State read/write、transaction、assertion、ordered-index cursor/result、append及health各有exact codec。client不得发送scope/caller/capability/policy；broker按handle role创建内部scope并以`record_assertions`、`ordered_index_assertions`等正确字段重建新StateTransaction。cursor绑定member/score/index name/bucket并传入provider `start_after`。
- 决策（business persistence）：P10/P11只依赖`DeliveryAdmissionPersistence`、`DeliverySchedulerPersistence`、`DeliveryPayloadPersistence`、`DeliveryRegistryPersistence`和`StrongAuditPersistence`窄接口。production RuntimeContext显式注入五个fixed-role broker proxy；contract-test adapter只接受contract-test repository。业务事务使用不含authority的typed DTO，response按原请求fingerprint及mutation/log cardinality绑定。
- 决策（failure与secret）：write request在send前只允许local handle/schema/role拒绝；send后发生timeout、EOF、malformed payload/envelope、sequence/signature错误或broker退出统一为indeterminate。close依次shutdown、bounded wait、terminate、kill并确认child退出，child finally关闭HTTP、Store和physical-domain lease。IAM/StateStore credential仅由broker secret FD读取；主RuntimeContext只保留redacted状态和不含credential的broker proxies。
- 后果：production root私钥不得进入源码或test fixture，因此无deployment key的本地/CI只能验证错误root拒绝、test realm及全部非secret composition，不能把production `main`正向启动记为已验证。该限制不允许退回可由Python调用方提供的新root。状态机仍严格为`prepared -> queued -> sending -> ack_waiting`，WRITE_UNCERTAIN不恢复、不重发；P12、P13、P14、P17、P22及production `task.dispatch`保持未实现/禁用。
- 关联阶段/工作包：`P08-FIX-06`、`P09-FIX-10`、`P11-FIX-11`、`P12`、`P13`、`P14`、`P17`、`P22`。

## ADR-050

- ADR 编号：`ADR-050`
- 状态：`ACCEPTED`（certificate-bound realm types、signed StateStore responses与canonical transaction binding）
- 背景：ADR-049已建立fixed root与pickle-free wire，但channel的production身份仍能被可写realm字符串伪装，production/test/integration复用channel与repository proxy类型，StateStore response没有session签名，transaction fingerprint又依赖不包含document payload的`repr()`。此外IPC异常先标closed会跳过child cleanup，实际StateStore secret FD仍可能传递父进程可解析的credential source，physical-domain lease包含可改名reference。
- 决策（certificate chain与exact realm）：channel必须保存ready阶段验签通过的`BrokerInstanceCertificate`，每次production adapter/repository使用时都以编译时root重新验证realm、instance、session public key、runtime、generation和时效。production、contract-test、integration-test分别使用exact channel、IAM proxy、repository proxy与StateStore proxy类型，exact type-to-role映射不读取可写class attribute；字符串realm、字段补写、subclass或`object.__new__`不能把test对象变为production authority。
- 决策（signed response）：broker对IAM外层与所有StateStore success/error response使用session private key签名。canonical claims绑定certificate/session identity、instance/generation、request ID和request sequence、operation、handle ID/role、完整request fingerprint、完整result/error canonical JSON、response sequence、issued_at与nonce。主进程在DTO decode和typed error映射前验证certificate chain与签名，并要求response sequence精确加一；unsigned、跨operation/handle/broker/request replay、result/error修改、sequence重复/跳号/倒退均关闭channel。write请求发出后的任一response provenance错误映射为indeterminate。
- 决策（IAM与transaction）：`MessageAuthorizationService`、authorization evidence与payload evidence必须同时验证production root certificate chain及certificate session key签发的request-bound IAM result，且certificate/result均未过期。delivery transaction fingerprint使用canonical JSON覆盖partition、mutation顺序/key/kind/assertion、document metadata与payload SHA-256、record/index assertions、index mutations和log appends；result由`for_transaction`逐项验证record顺序、key/document/delete语义、log cardinality与正整数position，并绑定精确request identity/fingerprint，拒绝copy/replace/字段修改与同shape cross-request replay。
- 决策（cleanup、secret与lease）：channel把“拒绝新请求”和“child已reap”分为两个状态；任何IPC/decode/signature failure都执行close connection、bounded join、terminate、kill并确认child死亡，之后`close()`仍幂等复验。launcher向secret FD写实际StateStore password bytes而非`env:`/`file:` source，broker一次消费并尽力清零buffer；普通RuntimeContext/config/environment只保留redacted状态。physical-domain lock仅绑定backend、normalized scheme/host/port/database、namespace及稳定ACL principal/username，不绑定runtime ID、secret、环境变量名、文件路径或调用方credential reference。
- 后果：production deployment root正向启动仍必须由仓库外部署私钥验证，本地test/integration root不能冒充production证据。broker session certificate有界过期并在失效后fail closed，部署必须在证书生命周期内完成受控broker轮换。本决策不新增global registry、service locator、第二RuntimeService、TaskSupervisor或shutdown owner；状态机仍为`prepared -> queued -> sending -> ack_waiting`，WRITE_UNCERTAIN不恢复、不重发，P12及production `task.dispatch`继续BLOCKED/disabled。
- 关联阶段/工作包：`P08-FIX-07`、`P09-FIX-11`、`P11-FIX-12`、`P12`、`P13`、`P14`、`P17`、`P22`。

## ADR-051

- ADR 编号：`ADR-051`
- 状态：`ACCEPTED`（OS-isolated attestation、delegated session rotation 与 IPC send uncertainty closure）
- 背景：ADR-050仍让普通runtime解释器用可修改的root global、certificate/channel字段和helper函数决定production provenance；request前后读取当前channel字段又存在connection/certificate swap TOCTOU。root直接签发的300秒session certificate没有在线生命周期，长期runtime必然永久unavailable；`send_bytes()`返回前的部分写异常也会被误判为未发送。
- 决策（独立attestor）：最外层launcher在导入processor、delivery、routing和plugin前以`spawn`启动最小authority attestor。attestor自行重新导入编译时production root，只接受bytes-only strict canonical JSON；它独占保存root批准的delegation/session chain、broker instance/runtime、generation、session key及handle集合，并验证production identity、IAM signed result、StateStore signed response和session rotation。普通runtime中的root global、realm/helper、certificate/public key字段只能作为待验证数据，不能单独形成production authority。
- 决策（delegation与rotation）：deployment root仅在bootstrap签发长期但有界的broker delegation certificate；root private key随即销毁。broker持有delegated key，并在session进入最后三分之一有效期时，凭attestor签发且绑定current/next generation的rotation ticket生成新session key、certificate及全部role handles。delegation签名的rotation response由attestor验证后原子批准；现有proxy对象保持稳定并刷新到新generation，旧handle、IAM result与StateStore response立即失效。delegation到期仍fail closed，部署必须在该上限前受控重启或提供后续独立授权的delegation renewal。
- 决策（request snapshot与TOCTOU）：channel在同一序列化临界区内取得attestor request ticket并冻结identity ID、connection generation、instance/runtime/session generation、session/certificate fingerprint、handle/role、request ID/sequence/fingerprint及expected response sequence。broker验证同一ticket后才执行operation；response到达后attestor以该snapshot验证session签名与全部request/response binding，runtime再确认connection/process/attestor/generation未变化。await期间替换connection、certificate、public key、instance或runtime只能导致fail closed；已发送StateStore write返回indeterminate。
- 决策（send failure）：在调用`send_bytes()`前即设置send-attempted。调用开始后的`BrokenPipeError`、`EOFError`、`OSError`、malformed response、attestor/signature/sequence failure均关闭原始与当前connection并bounded join、terminate、kill、join broker child，禁止复用旧sequence/channel；StateStore write统一映射为`NsRuntimeStateStoreIndeterminateWriteError`，read/IAM映射为unavailable。该语义不触发delivery retry，也不恢复或重发WRITE_UNCERTAIN。
- 后果：attestor和broker各自只有隔离IPC用途的内部event loop/process lifecycle，不是第二RuntimeService、TaskSupervisor、service locator或shutdown coordinator。状态机继续严格为`prepared -> queued -> sending -> ack_waiting`；P12 ACK/NACK/Defer/timeout/retry、P13 DLQ/replay、P14公平/health、P17 cluster ownership及production `task.dispatch`保持BLOCKED/disabled。
- 关联阶段/工作包：`P08-FIX-08`、`P09-FIX-12`、`P11-FIX-13`、`P12`、`P13`、`P14`、`P17`、`P22`。

## ADR-052

- ADR 编号：`ADR-052`
- 状态：`ACCEPTED`（P11 threat-model freeze、root-bound attestor与role-specific OS endpoints）
- 背景：ADR-051把最终production验签移入独立attestor，但root delegation没有绑定attestor identity，broker仍可能只因父进程传入的32字节attestor key接受ticket；主runtime又通过一个共享channel保存全部role handle并允许caller选择handle/role。该结构把proxy Python类型和主解释器对象不可修改性误当成安全边界，也让attestor失效后的所有role connection cleanup及长期session公开元数据难以解释。
- 决策（威胁模型）：P11保证普通runtime业务模块不能让broker执行超出其被授予role endpoint的IAM或StateStore操作；production私钥、raw Store、IAM transport、resource policy和最终role判定位于broker，跨role、跨endpoint、跨broker与跨generation调用由broker拒绝。P11不声称防御主runtime进程已被任意代码执行完全攻陷，也不声称阻止攻击者在同一解释器替换业务方法、调用栈或本地Python返回值；这些行为可以伪造本地结果，但不能据此让broker执行其他endpoint的operation。proxy exact type、`_ROLE`和dependency slot检查仅防误配，不构成authority proof。
- 决策（root-bound attestor）：root-signed delegation精确绑定broker instance/runtime、delegation public key、attestor instance ID/public key、允许用途、有效期与nonce。broker只使用delegation内的attestor key/identity验证request和rotation ticket；attestor批准identity时又必须确认delegation绑定的attestor就是自身。父进程单独提供的key、自建exact attestor client、攻击者Pipe/process或格式正确但由其他attestor签发的ticket一律拒绝。
- 决策（role-specific endpoints）：bootstrap分别为`iam`、`admission`、`scheduler`、`payload`、`registry`、`audit`和`lifecycle`创建独立duplex OS endpoint；broker持有connection到role的固定映射，request envelope不携带caller-selected role或handle。attestor ticket绑定endpoint ID、固定role、operation、generation、request ID/sequence/fingerprint；每个production proxy只接收自身endpoint和该role当前handle。启动握手临时聚合材料只供attestor一次批准，bootstrap消费后清空；稳定main对象图不保留全部role channel/handle表，不提供`current_handle(role)`或generic caller-handle request API。当前broker是单执行队列，因此process custodian只共享一个不携带endpoint/role/handle/authority的调度锁，把各独立endpoint的send、rotation与attestor response verification串行化，防止旧generation在途响应与另一endpoint的新generation rotation交叉；该锁不是共享authority channel。
- 决策（failure与session metadata）：attestor死亡或identity/rotation/response attestation失效时，所有新请求fail closed并触发connection close、bounded join、terminate、kill、确认broker退出及attestor close；broker finally关闭IAM/Store/lease和全部server endpoints。只有已尝试`send_bytes()`的StateStore write返回indeterminate，preflight失效返回unavailable。`ProductionAuthorityBroker.public_key`和`current_session_identity()`动态读取lifecycle endpoint当前attestor-approved generation/session key/fingerprint，session轮换后不暴露启动快照。
- 后果：authority路径由root→delegation(attestor-bound)→attestor ticket→broker endpoint-role/resource policy组成；Python proxy类型不再承担broker安全职责。没有新增verifier进程、global registry、service locator、第二RuntimeService、TaskSupervisor或shutdown coordinator。状态仍为`prepared -> queued -> sending -> ack_waiting`，WRITE_UNCERTAIN不恢复、不重发；P12 ACK/NACK/Defer/timeout/retry、P13 DLQ/replay、P14公平/health、P17 cluster ownership及production `task.dispatch`保持BLOCKED/disabled。
- 关联阶段/工作包：`P08-FIX-09`、`P09-FIX-13`、`P11-FIX-14`、`P12`、`P13`、`P14`、`P17`、`P22`。

## ADR-053

- ADR 编号：`ADR-053`
- 状态：`ACCEPTED`（authority 稳定性 barrier、test-only 诊断与 service cleanup lease）
- 背景：全仓高负载执行中，短至0.6秒的test session certificate、固定0.42秒sleep、固定0.25秒worker delay与3秒broker channel timeout发生竞争，偶发把测试调度抖动表现为`attestor_unavailable`或`broker_unavailable`；单项复跑不能区分certificate边界、child退出、提前cleanup、IPC timeout或attestor timeout。composition在service cleanup连续无进展后又无条件清空`service_lifecycle_owner`，让仍持有transport/task/logger/StateStore资源的service只能依赖局部变量、traceback或GC存活，并可能被外层第二owner重复关闭。
- 决策（authority test稳定性）：rotation测试以当前certificate的`issued_at`/`expires_at`计算最后三分之一barrier，并以实际generation增长驱动至少3次切换；不使用固定毫秒余量或精确generation墙钟假设。event-loop非阻塞测试使用`threading.Event`明确标记worker进入阻塞点，ticker证明推进后显式释放worker；测试过程中同时证明operation完成、broker/attestor存活。contract/integration test starter提供显式test-only broker operation timeout；production channel timeout计算、默认值、certificate/attestation/IAM receipt/StateStore response验证和fail-closed语义不变。
- 决策（安全诊断）：只有contract-test与integration-test channel启用失败诊断。异常details只允许process alive、exitcode、operation、endpoint role、request stage、rotation generation和timeout stage，以及同结构attestor状态；禁止credential、payload、FD、handle、certificate、endpoint对象和异常原文。诊断在reap前取样，用于区分真实child exit与broker/attestor response timeout；诊断不触发retry、不吞错且不改变production异常语义。
- 决策（service owner）：StateStore成功open且service即将尝试start时，composition把transport、task supervisor、logger和共享broker-backed StateStore/repository graph一次性移交给service。未完成cleanup由绑定原event loop的close-only lifecycle lease持有；稳定`cleanup_pending_no_progress`异常显式持有该lease及`cleanup_pending`、`service_stopped`和owner-active事实。只有coordinator不再pending且service为`STOPPED`时才能释放owner；外层同步composition cleanup不在新`asyncio.run()`中调用service.stop，也不重复关闭service已接管资源。同一原loop解除失败条件后可通过lease只继续未完成phase。普通operation failure作为cleanup-incomplete的cause保留，进程级`BaseException`优先级不变。
- 后果：authority broker/attestor仍保持原有隔离、bounded reap、WRITE_UNCERTAIN和role endpoint边界；service只新增close-only生命周期所有权，不新增第二RuntimeService、event loop、TaskSupervisor或shutdown coordinator。P12继续`BLOCKED / NOT_STARTED / F0`，未实现ACK/NACK/Defer、retry、DLQ、cluster ownership或production `task.dispatch`。
