...
- 决策：`RTC-1` 的“显式依赖”同时约束 API 与模块导入边界。`import ns_runtime.context` 不得仅为解析类型而导入 `ns_common` package，也不得经 `config.model` 的全局 `ns_config` 或 `http_client -> logger` 链读取环境、访问配置文件、创建运行目录、初始化 logger handler、创建 HTTP client、task、thread 或 event loop。类型引用使用 postponed annotations 和 `TYPE_CHECKING` 保留具体公共类型；运行时类型验证只在调用方显式构造 context，或为可选槽位实际注入非 `None` 对象时延迟解析对应类型。`NsConfig`、`NsHttpClientOwner` 等字段不得因此退化为 `Any`、dict、任意 object 或字符串 registry；默认 `None` 的 HTTP owner 槽位不得加载 HTTP/logger/config 链。
