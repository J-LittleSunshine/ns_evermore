...
- 后果：并发重复认证最多一个进入 authenticated；active 上的 drain/close 按锁顺序线性化，close 可从 draining 继续完成且无法恢复 active。普通断网不立即删除可恢复 logical context，但连接在 grace 全程不可路由且无 transport/path binding；SC-1 的 grace view 已增量冻结，resume/reauth/snapshot 部分仍由 W11-W14 完成。
- 阶段状态：`P05-W01` 至 `P05-W10 VERIFIED`；SC-1 为 `IN_PROGRESS`，resume、不可恢复 close、reauth 与 snapshot/audit 尚未完成，P06 真实 IAM backend/client 保持 `NOT_STARTED`。
