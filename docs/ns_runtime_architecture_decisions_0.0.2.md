...
- 决策：进程内 `EventBus` 只接受 typed event，event 精确只含 object id、safe summary 和 trace reference。subscriber task复用同一 TaskSupervisor；单 subscriber timeout/exception只形成 publish report，不传播为主链路失败，也不提供状态修改或 authority API。事件丢失不得影响任何当前状态；后续 worker 被唤醒后仍必须重新读取 P08 authority。
...
- 阶段冻结：`P07-W01` 至 `P07-W10` 经 blocker 修复专项 18 项、processor/connection/session 相关联合 89 项、runtime 683 项和 backend 694 项全量回归，以及 cold import、compileall、pip check、显式类型与禁止边界扫描后重新达到 `VERIFIED/F3`；P08 StateStore/Redis/Valkey authority/lease/fencing/CAS、DeliveryRecord/AckRecord/NackRecord/DeferRecord/retry/dead-letter、RoutingPlan/target selection/reliable dispatch 均未实现。
