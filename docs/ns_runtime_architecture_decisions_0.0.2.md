...
- 后果：并发重复认证最多一个进入 authenticated；active 上的 drain/close 按锁顺序线性化，close 可从 draining 继续完成且无法恢复 active。logical connection可跨transport恢复但旧transport/session/epoch均无法重新取得owner；SC-1的resume/epoch view已增量冻结，不可恢复close/reauth/snapshot仍由W12-W14完成。
- 阶段状态：`P05-W01` 至 `P05-W11 VERIFIED`；SC-1 为 `IN_PROGRESS`，不可恢复 close、reauth 与 snapshot/audit 尚未完成，P06 真实 IAM backend/client 保持 `NOT_STARTED`。
