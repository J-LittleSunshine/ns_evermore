...
- 决策：pipeline 顺序固定为 security validation、authorization、rate-limit entry、idempotency precheck、audit marker、routing preparation、message processor、response finalize。`ProcessorRegistry` 只通过显式 registration 按 message.type、stage、同 major 有界 protocol version 和 feature flag/state 解析；duplicate exact registration、重叠 version 和缺失 flag resolution 均 fail-closed。当前 rate-limit、idempotency 与 routing 只冻结无状态接口，不建立 limiter store、idempotency store、target selection 或 RoutingPlan。
...
- 阶段冻结：`P07-W01` 至 `P07-W10` 经 14 项专项、60 项 processor/connection/session composition 联合、runtime 679 项和 backend 690 项全量回归，以及 cold import、compileall、pip check、敏感字段与禁止边界扫描后达到 `VERIFIED/F3`；P08 StateStore/Redis/Valkey authority/lease/fencing/CAS、DeliveryRecord/AckRecord/NackRecord/DeferRecord/retry/dead-letter、RoutingPlan/target selection/reliable dispatch 均未实现。
