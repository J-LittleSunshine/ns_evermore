...
- 后果：并发重复认证最多一个进入 authenticated；active 上的 drain/close 按锁顺序线性化，close 可从 draining 继续完成且无法恢复 active。安全关闭一经标记不能被grace或新token恢复；reauth失败或绝对TTL到期会先撤销target并关闭，成功续期只原子替换同一logical session的权限快照。SC-1的reauth/expiry view已增量冻结，最终safe snapshot/audit聚合仍由W14完成。
- 阶段状态：`P05-W01` 至 `P05-W13 VERIFIED`；SC-1 为 `IN_PROGRESS`，最终snapshot/audit聚合尚未完成，P06真实IAM backend/client保持 `NOT_STARTED`。
