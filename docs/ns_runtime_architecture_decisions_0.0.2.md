...
- 后果：并发重复认证最多一个进入 authenticated；active 上的 drain/close 按锁顺序线性化，close 可从 draining 继续完成且无法恢复 active。drain 立即阻止新目标选择，同时 W08 health heartbeat 仍可工作；SC-1 的 drain view 已增量冻结，grace/resume/reauth/snapshot 部分仍由 W10-W14 完成。
- 阶段状态：`P05-W01` 至 `P05-W09 VERIFIED`；SC-1 为 `IN_PROGRESS`，grace/resume、不可恢复 close、reauth 与 snapshot/audit 尚未完成，P06 真实 IAM backend/client 保持 `NOT_STARTED`。
