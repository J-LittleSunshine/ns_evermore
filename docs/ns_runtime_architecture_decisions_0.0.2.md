...
- 后果：并发重复认证最多一个进入 authenticated；active 上的 drain/close 按锁顺序线性化，close 可从 draining 继续完成且无法恢复 active。handshake completion 被先到 close fencing 后稳定拒绝；hello-first/deadline 不建立 IAM、SessionContext、active/index 或业务 processor，W03 必须在相同受控握手内存中消费并释放 token。
- 阶段状态：`P05-W01` 至 `P05-W02 VERIFIED`；SC-1、IAM 测试 adapter、协商、索引、accepted、heartbeat、grace/resume、reauth 与 snapshot/audit 尚未冻结，P06 保持 `NOT_STARTED`。
