...
- 决策（IAM composition）：`IamClient`公共构造永久 fail closed，module 不再提供 `IamClientFactory`或 production client factory。唯一 production graph 只在 process composition 中组装 exact `IamClient`、owner-issued narrow HTTP authority handle 与 exact-client composition proof；proof、handle、owner和client均绑定实例 identity 且不可复制，contract-test adapter永不满足 production校验。普通构造、任意 runtime object、`object.__new__`、copy、subclass和method override不能建立 production adapter。
- 决策（HTTP provenance）：IAM不再直接持有公开 `NsAsyncHttpClient`，只持有 narrow POST handle。owner binding记录 exact `NsAsyncHttpClient`、底层 exact `httpx.AsyncClient`、主 `_transport`、原 `_mounts`容器及逐项mount/proxy transport identity，并记录 `NsAsyncHttpClient.request/get/post/put/delete`、`httpx.AsyncClient.request/send/stream`及每个transport `handle_async_request`的class identity。请求前后都复验完整binding；任一instance substitution、transport/mount替换、handler monkey patch或owner/client关闭都使handle失效。
- 决策（StateStore creation）：raw StateStore不保存repository owner、private scope issuer、repository对象或`_create_repository(owner=...)`入口。composition一次性创建admission、scheduler、payload、registry及显式audit repository后冻结binding map并永久关闭creation。production repository使用Ed25519私钥签发绑定caller、authority、namespace、partition和policy的scope；raw store只持公钥验证器与冻结policy映射，属性遍历、复制或pickle式字段复制不能恢复签发私钥。contract-test model继续使用独立issuer/composition，不复用production creation path。
- 决策（resource allowlist）：scope seal包含不可变resource policy。StateStore在provider调用前对每个read/scan/CAS/transaction mutation/read assertion/index mutation/index assertion/log append及append逐项验证exact operation、`object_type/schema_name`、ordered-index `name/bucket`和namespace/partition。admission仅访问dedup、payload/summary/delivery初始化、prepared index与transition log；scheduler仅访问delivery/summary/attempt/cursor、明确scheduler indexes/logs；payload仅READ payload_body；registry仅layout/tenant registration与registry index；audit仅指定audit log。未知资源、错误schema/bucket和跨role访问全部fail closed。
