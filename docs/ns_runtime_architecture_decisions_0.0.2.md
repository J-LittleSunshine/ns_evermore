...
- 决策：tenant + raw message_id + RP-1 selected target fingerprint形成dedup key，登记包含schema、TTL、summary、in_progress/acked/dead/expired/cancelled闭集及自校验fingerprint。dedup、root/shard Summary与prepared DeliveryRecord由`StateStoreDeliveryAdmissionStore`在同一tenant namespace transaction以absent assertions原子创建；冲突后只做linearizable dedup read。普通cache、模块dict和transient memory不实现该production接口；无provider使用typed unavailable，write outcome未知保持indeterminate且不自动retry。
...
- 决策：fanout `<=5000`只有一个shard；`>5000`按1000 bucket分片，初始化构造固定500 batch且最终由一个StateStore transaction提交。all accepted、partial rejected与all rejected分别形成accepted/partial/failed root/shard Summary；all rejected仍创建dedup与failed Summary但零DeliveryRecord。rejection evidence只保存低基数typed reason。
