...
- 决策：fallback scorer固定为`runtime_fallback/fallback.v1`，只使用selector exactness、trusted affinity、runtime policy static weight、capability surplus与canonical binding tie-break；不使用random、调用顺序、sender message ID/weight、transport identity或P14 health/latency/pressure。decision fingerprint覆盖normalized request、唯一snapshot mutation、policy/scorer版本与决策证据，但排除plan ID、创建时间和message reference；相同权威输入产生相同fingerprint和selected targets。
- 决策：`RoutingDecision`显式区分deeply immutable `ResolvedRoutingPlan`与`RoutingFailureReport`，失败不能伪装为空plan。plan保存原target、候选/过滤/评分证据、selected binding、策略与rebind、IAM/config/policy/scorer引用、index mutation、local/stale事实和显式previous-plan context；version只从调用方提供的previous context推导，Router不保存message→version map。敏感执行值不进入repr；日志、指标和普通审计只能消费`SafeRoutingProjection`。
...
- 决策：资源上限固定由配置提供，默认candidate 10000、selected 10000、evidence 20000，必须为正且完整round-trip；超限unavailable而不截断，并支持5001 selected bindings。policy reject映射`RUNTIME_ROUTE_REJECTED/200177`，权威connection missing映射`RUNTIME_TARGET_NOT_FOUND`，tenant/IAM使用既有精确错误，其余no candidate、eligibility、epoch、limit、remote和authority unavailable映射`RUNTIME_ROUTE_UNAVAILABLE`。later action只是纯数据，不创建retry、queue、timer、task、worker或subscription。
