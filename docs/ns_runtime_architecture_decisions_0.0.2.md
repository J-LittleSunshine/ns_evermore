...
- 后果：并发重复认证最多一个进入 authenticated；active 上的 drain/close 按锁顺序线性化，close 可从 draining 继续完成且无法恢复 active。客户端未收到 accepted 时服务端不会发布 active；已成功发送后若 activation fencing 失败也会关闭而非保留模糊 active。SC-1 的 admission path 已增量冻结，heartbeat/grace/resume/reauth/snapshot 部分仍由 W08-W14 完成。
- 阶段状态：`P05-W01` 至 `P05-W07 VERIFIED`；SC-1 为 `IN_PROGRESS`，heartbeat、drain、grace/resume、reauth 与 snapshot/audit 尚未完成，P06 真实 IAM backend/client 保持 `NOT_STARTED`。
